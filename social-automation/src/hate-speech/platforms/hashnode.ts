...
export class HashnodeHateSpeechMonitor {
  private readonly platform = 'hashnode';
  ...
  async run(): Promise<HateSpeechMonitorResult> {
    ...
    try {
      ...
      console.error('[HashnodeHateSpeechMonitor] getSocialContext failed:', error);
      result.error = true;
    ...
    try {
      console.log(`[HashnodeHateSpeechMonitor] Starting run for ${this.profileUrl}`);
      const page = context.pages()[0] ?? await context.newPage();
      console.log(`[HashnodeHateSpeechMonitor] Navigating...`);
      await page.goto(this.profileUrl, { waitUntil: 'domcontentloaded', timeout: 60000 });
      console.log(`[HashnodeHateSpeechMonitor] Navigated. Waiting for posts...`);
      try {
        await page.waitForSelector('article', { timeout: 15000 });
      } catch {
        console.log(`[HashnodeHateSpeechMonitor] Timeout waiting for posts, will proceed anyway.`);
      }
      ...
      console.log(`[HashnodeHateSpeechMonitor] Wait done. Start scrolling...`);
      ...
      while (result.posts.length < this.postCount && emptyScrolls < 10) {
        ...
        console.log(`[HashnodeHateSpeechMonitor] Found ${count} posts on screen. Empty scrolls: ${emptyScrolls}, Total extracted: ${result.posts.length}`);
        ...
        for (let j = 0; j < count; j++) {
          ...
          try {
            ...
            if (links.length > 0) {
              ...
              postUrl = href.startsWith('http') ? href : ('https://hashnode.com' + href).split('?')[0];
            }
        ...
        if (addedThisScroll === 0) {
          emptyScrolls++;
          console.log(`[HashnodeHateSpeechMonitor] Added 0 posts this scroll. Empty scrolls incremented to ${emptyScrolls}`);
        } else {
          emptyScrolls = 0;
          console.log(`[HashnodeHateSpeechMonitor] Added ${addedThisScroll} posts this scroll. Reset empty scrolls.`);
        }

        if (result.posts.length < this.postCount) {
          console.log(`[HashnodeHateSpeechMonitor] Scrolling down...`);
      ...
      console.log(`[HashnodeHateSpeechMonitor] Run completed successfully with ${result.total_posts} posts.`);

    } catch (error) {
      console.error('[HashnodeHateSpeechMonitor] Fatal error during run:', error);
      result.error = true;
      result.error_reason = errorReason(error);
      result.session_expired = isSessionExpired(error);
    } finally {
