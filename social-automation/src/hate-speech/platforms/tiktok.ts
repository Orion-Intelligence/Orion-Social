...
export class TikTokHateSpeechMonitor {
  ...
  async run(): Promise<HateSpeechMonitorResult> {
    ...
    try {
      ...
      console.error('[TikTokHateSpeechMonitor] getSocialContext failed:', error);
      result.error = true;
    ...
    try {
      console.log(`[TikTokHateSpeechMonitor] Starting run for ${this.profileUrl}`);
      const page = context.pages()[0] ?? await context.newPage();
      console.log(`[TikTokHateSpeechMonitor] Navigating...`);
      await page.goto(this.profileUrl, { waitUntil: 'domcontentloaded', timeout: 60000 });
      console.log(`[TikTokHateSpeechMonitor] Navigated. Waiting for timeline...`);
      try {
        await page.waitForSelector('a[href*="/video/"], [data-e2e="user-post-item"]', { timeout: 15000 });
      } catch {
        console.log(`[TikTokHateSpeechMonitor] Timeout waiting for items, will proceed anyway.`);
      }

      if (page.url().includes('/login') || (await page.locator('[data-e2e="login-button"], #loginContainer').count()) > 0) {
        throw new SocialAutomationError('Session expired: Redirected to login page.', 'SESSION_EXPIRED');
      }

      console.log(`[TikTokHateSpeechMonitor] Wait done. Start scrolling...`);
      ...
      while (result.posts.length < this.postCount && emptyScrolls < 10) {
        const items = page.locator('a[href*="/video/"]');
        const count = await items.count();
        let addedThisScroll = 0;
        console.log(`[TikTokHateSpeechMonitor] Found ${count} video links on screen. Empty scrolls: ${emptyScrolls}, Total extracted: ${result.posts.length}`);
        ...
        for (let j = 0; j < count; j++) {
          ...
          try {
            ...
            const postUrl = (href.startsWith('http') ? href : 'https://www.tiktok.com' + href).split('?')[0];
        ...
        if (addedThisScroll === 0) {
          emptyScrolls++;
          console.log(`[TikTokHateSpeechMonitor] Added 0 posts this scroll. Empty scrolls incremented to ${emptyScrolls}`);
        } else {
          emptyScrolls = 0;
          console.log(`[TikTokHateSpeechMonitor] Added ${addedThisScroll} posts this scroll. Reset empty scrolls.`);
        }

        if (result.posts.length < this.postCount) {
          console.log(`[TikTokHateSpeechMonitor] Scrolling down...`);
      ...
      console.log(`[TikTokHateSpeechMonitor] Run completed successfully with ${result.total_posts} posts.`);

    } catch (error) {
      console.error('[TikTokHateSpeechMonitor] Fatal error during run:', error);
      result.error = true;
      result.error_reason = errorReason(error);
      result.session_expired = isSessionExpired(error);
    } finally {
