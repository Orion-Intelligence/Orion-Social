...
export class YouTubeHateSpeechMonitor {
  ...
  async run(): Promise<HateSpeechMonitorResult> {
    ...
    try {
      ...
      console.error('[YouTubeHateSpeechMonitor] getSocialContext failed:', error);
      result.error = true;
    ...
    try {
      console.log(`[YouTubeHateSpeechMonitor] Starting run for ${this.profileUrl}`);
      const page = context.pages()[0] ?? await context.newPage();
      console.log(`[YouTubeHateSpeechMonitor] Navigating...`);
      await page.goto(this.profileUrl, { waitUntil: 'domcontentloaded', timeout: 60000 });
      console.log(`[YouTubeHateSpeechMonitor] Navigated. Waiting for timeline...`);
      try {
        await page.waitForSelector('ytd-rich-item-renderer', { timeout: 15000 });
      } catch {
        console.log(`[YouTubeHateSpeechMonitor] Timeout waiting for posts, will proceed anyway.`);
      }

      if (page.url().includes('accounts.google.com') || page.url().includes('/signin') || (await page.locator('a[aria-label="Sign in"]').count()) > 0) {
        throw new SocialAutomationError('Session expired: Redirected to login page.', 'SESSION_EXPIRED');
      }

      console.log(`[YouTubeHateSpeechMonitor] Wait done. Start scrolling...`);

      const extractedUrls = new Set<string>();
      let emptyScrolls = 0;

      while (result.posts.length < this.postCount && emptyScrolls < 10) {
        const items = page.locator('ytd-rich-item-renderer');
        const count = await items.count();
        let addedThisScroll = 0;
        console.log(`[YouTubeHateSpeechMonitor] Found ${count} posts on screen. Empty scrolls: ${emptyScrolls}, Total extracted: ${result.posts.length}`);
        ...
        for (let j = 0; j < count; j++) {
          ...
          try {
            const links = await item.locator('a[href*="/watch"], a[href^="http"]').all();
            let postUrl = 'Unknown URL';
            if (links.length > 0) {
              const href = await links[0].getAttribute('href') || '';
              postUrl = href.startsWith('http') ? href : 'https://www.youtube.com' + href;
            }
            ...
            if (postUrl !== 'Unknown URL' && !extractedUrls.has(postUrl)) {
              ...
              const isAd = (await item.locator('ytd-ad-slot-renderer, ytd-in-feed-ad-layout-renderer').count()) > 0 || lines.some(line => ['Sponsored', 'Ad'].includes(line));
              if (isAd) continue;

              const author = lines.find(line => !['Sponsored', 'Ad'].includes(line)) ?? 'Unknown';
        ...
        if (addedThisScroll === 0) {
          emptyScrolls++;
          console.log(`[YouTubeHateSpeechMonitor] Added 0 posts this scroll. Empty scrolls incremented to ${emptyScrolls}`);
        } else {
          emptyScrolls = 0;
          console.log(`[YouTubeHateSpeechMonitor] Added ${addedThisScroll} posts this scroll. Reset empty scrolls.`);
        }

        if (result.posts.length < this.postCount) {
          console.log(`[YouTubeHateSpeechMonitor] Scrolling down...`);
      ...
      console.log(`[YouTubeHateSpeechMonitor] Run completed successfully with ${result.total_posts} posts.`);

    } catch (error) {
      console.error('[YouTubeHateSpeechMonitor] Fatal error during run:', error);
      result.error = true;
      result.error_reason = errorReason(error);
      result.session_expired = isSessionExpired(error);
    } finally {
