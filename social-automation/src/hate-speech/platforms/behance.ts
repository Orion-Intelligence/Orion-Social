...
export class BehanceHateSpeechMonitor {
  ...
  async run(): Promise<HateSpeechMonitorResult> {
    ...
    try {
      ...
      console.error('[BehanceHateSpeechMonitor] getSocialContext failed:', error);
      result.error = true;
    ...
    try {
      console.log(`[BehanceHateSpeechMonitor] Starting run for ${this.profileUrl}`);
      const page = context.pages()[0] ?? await context.newPage();
      console.log(`[BehanceHateSpeechMonitor] Navigating...`);
      await page.goto(this.profileUrl, { waitUntil: 'domcontentloaded', timeout: 60000 });
      console.log(`[BehanceHateSpeechMonitor] Navigated. Waiting for posts...`);
      try {
        await page.waitForSelector('a[href*="/gallery/"], [class*="ProjectCoverNeue"]', { timeout: 15000 });
      } catch {
        console.log(`[BehanceHateSpeechMonitor] Timeout waiting for posts, will proceed anyway.`);
      }

      if (page.url().includes('login') || page.url().includes('signin') || (await page.locator('input[type="password"]').count()) > 0) {
        throw new SocialAutomationError('Session expired: Redirected to login page.', 'SESSION_EXPIRED');
      }

      console.log(`[BehanceHateSpeechMonitor] Wait done. Start scrolling...`);
      ...
      while (result.posts.length < this.postCount && emptyScrolls < 10) {
        const items = page.locator('a[href*="/gallery/"]');
        const count = await items.count();
        let addedThisScroll = 0;
        console.log(`[BehanceHateSpeechMonitor] Found ${count} gallery links on screen. Empty scrolls: ${emptyScrolls}, Total extracted: ${result.posts.length}`);
        ...
        for (let j = 0; j < count; j++) {
          ...
          try {
            ...
            if (!href.includes('/gallery/')) continue;
            const postUrl = (href.startsWith('http') ? href : 'https://www.behance.net' + href).split('?')[0];
        ...
        if (addedThisScroll === 0) {
          emptyScrolls++;
          console.log(`[BehanceHateSpeechMonitor] Added 0 posts this scroll. Empty scrolls incremented to ${emptyScrolls}`);
        } else {
          emptyScrolls = 0;
          console.log(`[BehanceHateSpeechMonitor] Added ${addedThisScroll} posts this scroll. Reset empty scrolls.`);
        }

        if (result.posts.length < this.postCount) {
          console.log(`[BehanceHateSpeechMonitor] Scrolling down...`);
      ...
      console.log(`[BehanceHateSpeechMonitor] Run completed successfully with ${result.total_posts} posts.`);

    } catch (error) {
      console.error('[BehanceHateSpeechMonitor] Fatal error during run:', error);
      result.error = true;
      result.error_reason = errorReason(error);
      result.session_expired = isSessionExpired(error);
    } finally {
