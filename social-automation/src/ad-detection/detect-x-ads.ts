...
export class XAdDetector {
          ...
          try {
            ...
            if (isAd) {
              ...
              if (tweetUrl !== 'Unknown URL' && !detectedAds.has(tweetUrl)) {
                ...
                const details = await this.extractAdDetails(context, tweetUrl);
  ...
  private async extractAdDetails(context: BrowserContext, tweetUrl: string) {
...
async function main(): Promise<void> {
  ...
  const result = await new XAdDetector(sessionFile).run();
