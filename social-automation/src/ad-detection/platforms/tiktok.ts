  ...
  async run(): Promise<AdDetectionResult> {
    ...
    try {
      ...
      for (let i = 0; i < MAX_SCROLLS; i++) {
        ...
        const items = page.locator('[data-e2e="recommend-list-item-container"], article');
        ...
        for (let j = 0; j < count; j++) {
          ...
          try {
            ...
            const isAd = lines.some(line => ['Sponsored', 'Ad'].includes(line));

            if (isAd) {
              const links = await item.locator('a[href*="/video/"], a[href*="/@"]').all();
              let adUrl = 'Unknown URL';
              if (links.length > 0) {
                const href = await links[0].getAttribute('href') || '';
                adUrl = href.startsWith('http') ? href : 'https://www.tiktok.com' + href;
                adUrl = adUrl.split('?')[0];
              }
              ...
              if (uniqueId && !detectedAds.has(uniqueId)) {
                ...
                console.log(`[TikTokAdDetector] Ad detected: ${author} ${adUrl}`);
