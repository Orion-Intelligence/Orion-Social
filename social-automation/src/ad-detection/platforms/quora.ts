  ...
  async run(): Promise<AdDetectionResult> {
    ...
    try {
      ...
      for (let i = 0; i < MAX_SCROLLS; i++) {
        ...
        const items = page.locator('.q-box.qu-borderAll');
        ...
        for (let j = 0; j < count; j++) {
          ...
          try {
            ...
            const isAd = lines.length > 1 && lines.some(line => ['Promoted', 'Sponsored', 'Advertisement'].includes(line));
            ...
            if (isAd) {
              ...
              if (links.length > 0) {
                ...
                adUrl = href.startsWith('http') ? href : ('https://www.quora.com' + href).split('?')[0];
              }
              ...
              if (uniqueId && !detectedAds.has(uniqueId)) {
                ...
                const author = lines.find(line => !['Promoted', 'Sponsored', 'Advertisement'].includes(line)) ?? 'Unknown';
                ...
                console.log(`[QuoraAdDetector] Ad detected: ${author} ${adUrl}`);
