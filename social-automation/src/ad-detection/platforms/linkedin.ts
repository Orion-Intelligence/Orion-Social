...
export class LinkedInAdDetector {
      ...
      for (let i = 0; i < MAX_SCROLLS; i++) {
        ...
        for (let j = 0; j < count; j++) {
          ...
          try {
            ...
            const isAd = lines.some(line => ['Promoted'].includes(line));
            ...
            if (isAd) {
              ...
              if (links.length > 0) {
                ...
                adUrl = href.startsWith('http') ? href : ('https://www.linkedin.com' + href).split('?')[0];
              }
              ...
              if (uniqueId && !detectedAds.has(uniqueId)) {
                ...
                const author = lines.find(line => !['Promoted'].includes(line)) ?? 'Unknown';
                ...
                console.log(`[LinkedInAdDetector] Ad detected: ${author} ${adUrl}`);
