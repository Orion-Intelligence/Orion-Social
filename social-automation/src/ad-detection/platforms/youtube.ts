          ...
          try {
            ...
            if (isAd) {
              ...
              if (links.length > 0) {
                ...
                adUrl = href.startsWith('http') ? href : ('https://www.youtube.com' + href).split('?')[0];
              }

              const uniqueId = lines.slice(0, 3).join(' | ');
              ...
              if (uniqueId && !detectedAds.has(uniqueId)) {
                ...
                console.log(`[YouTubeAdDetector] Ad detected: ${author} ${adUrl}`);
