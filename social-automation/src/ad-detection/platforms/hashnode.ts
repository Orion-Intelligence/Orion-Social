...
export class HashnodeAdDetector {
  ...
  async run(): Promise<AdDetectionResult> {
    ...
    try {
      ...
      await page.goto('https://hashnode.com/feed', { waitUntil: 'domcontentloaded' });
          ...
          try {
            ...
            if (isAd) {
              ...
              if (links.length > 0) {
                ...
                adUrl = href.startsWith('http') ? href : ('https://hashnode.com' + href).split('?')[0];
              }
              ...
              if (uniqueId && !detectedAds.has(uniqueId)) {
                ...
                const author = lines.find(line => !['Sponsored', 'Promoted'].includes(line)) ?? 'Unknown';
                ...
                console.log(`[HashnodeAdDetector] Ad detected: ${author} ${adUrl}`);
