...
from crawler.constants.constant import HTTP_SCHEME


def init_services():
    RAW_PATH_CONSTANTS.MICROSERVER = HTTP_SCHEME + "localhost:8010"
...
def check_services_status():
    try:
        ...
        if not response.json().get("IsTor"):
            print("Tor is reachable but traffic is not routed through Tor.")
            sys.exit(1)
