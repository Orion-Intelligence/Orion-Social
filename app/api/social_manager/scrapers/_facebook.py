...
class _facebook(extraction_interface, ABC):
    ...
    def _extract_profile_assets(self, page) -> dict:
        try:
            ...
            return page.evaluate("""() => {
                ...
                const followers = pageText.match(/[\\d.,]+[KMB]?\\s+followers/i)?.[0] || '';
                const members = pageText.match(/[\\d.,]+[KMB]?\\s+members/i)?.[0] || '';
                return {title, description, followers, members, profileIcon, coverpage, unavailable};
            }""") or {}
        except Exception:
            return {}
    ...
    def _append_profile_info(self, page, data_type: SocialDataType):
        ...
        group_info = " | ".join(
            item for item in [
                helper_method.scalar_text(profile_assets.get("followers")),
    ...
    def parse_leak_data(self, page):
        ...
        for post in posts:
            ...
            raw_comments = self._collect_comments(page, post_url, self._comment_limit(), self._comment_offset()) if data_type == SocialDataType.COMMENTS and post_url else []
