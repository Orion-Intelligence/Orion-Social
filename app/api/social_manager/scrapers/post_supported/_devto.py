...
class _devto(extraction_interface, ABC):
    _instance = None
    ...
    def supports(cls, platform: str) -> bool:
        return (platform or "").strip().lower() == "devto"
    ...
    def rule_config(self) -> RuleModel:
        return RuleModel(
            ...
            m_threat_type=ThreatType.DEVTO,
            m_rule_type=RuleType.DEVTO,
        ...
        try:
            return page.evaluate("""async () => {
                ...
                const target = () => {
                    ...
                    if (parts[0] === 't' && parts[1]) return {type: 'tag', value: decodeURIComponent(parts[1])};
                    if (parts[0]) return {type: 'user', value: decodeURIComponent(parts[0]).replace(/^@/, '')};
                ...
                if (selected.type === 'user' && selected.value) {
                    try {
                        const res = await fetch(`https://dev.to/api/users/by_username?url=${encodeURIComponent(selected.value)}`, {
                        ...
                        if (res.ok) {
                            ...
                            return {
                                title: clean(user.name || user.username || selected.value),
                                bio: clean(user.summary || ''),
                                canonical: `https://dev.to/${user.username || selected.value}`,
                                profileIcon: user.profile_image || user.profile_image_90 || '',
                                coverpage: '',
                                stats: [
                ...
                if (selected.type === 'tag' && selected.value) {
                    return {
                        title: `DEV tag: ${selected.value}`,
                        bio: clean(meta('og:description', 'twitter:description', 'description') || text('main p')),
                        canonical: `https://dev.to/t/${selected.value}`,
                        profileIcon: absolutize(meta('og:image', 'twitter:image')) || '',
                        coverpage: '',
                        stats: `TAG: ${selected.value}`,
                    };
                }
    ...
    def _extract_posts(page, limit: int) -> list[dict]:
        try:
            return page.evaluate("""async limit => {
                ...
                const selected = target();
                const apiUrl = selected.type === 'tag'
                    ? `https://dev.to/api/articles?tag=${encodeURIComponent(selected.value)}&per_page=${Math.max(1, Math.min(limit, 100))}`
                    : `https://dev.to/api/articles?username=${encodeURIComponent(selected.value)}&per_page=${Math.max(1, Math.min(limit, 100))}`;
                const rows = [];
                if (selected.value) {
                    try {
                        const res = await fetch(apiUrl, {headers: {accept: 'application/json'}, credentials: 'omit'});
                        if (res.ok) {
                            ...
                            for (const article of Array.isArray(articles) ? articles : []) {
                                rows.push({
                                    ...
                                    caption: clean(article.description || article.title || ''),
                                    image: article.cover_image || article.social_image || '',
                                    timestamp: article.published_timestamp || article.published_at || article.created_at || '',
                                    author: clean(article.user?.name || article.user?.username || ''),
                                    handle: clean(article.user?.username || ''),
                                    messageId: String(article.id || article.url || ''),
                                    likeCount: article.public_reactions_count ?? article.positive_reactions_count ?? '',
                                    tags: Array.isArray(article.tag_list) ? article.tag_list : String(article.tags || '').split(',').map(tag => tag.trim()).filter(Boolean),
                ...
                const roots = Array.from(document.querySelectorAll([
                    'article',
                    '.crayons-story',
                    '[class*="crayons-story" i]'
                ].join(','))).slice(0, Math.max(limit * 6, 30));
                const fromDom = roots.map(root => {
                    const linkNode = Array.from(root.querySelectorAll('a[href*="/"]')).find(a => /dev\\.to\\/[^/]+\\/.+/.test(absolutize(a.getAttribute('href'))));
                    const mediaNode = root.querySelector('img');
                    const title = clean(root.querySelector('h2,h3,[class*="title" i]')?.innerText || linkNode?.innerText || '');
                    const caption = clean(root.querySelector('[class*="description" i], p')?.innerText || title || root.innerText || '');
