...
PLATFORM_CONFIG: Dict[str, Any] = {
    ...
    "page_url": "https://medium.com/@{username}",
}


class _medium(extraction_interface, ABC):
    _instance = None

    def __init__(self, callback=None):
        super().__init__()
        self.callback = callback
        self.platform = "medium"
        self._card_data = []
        self._entity_data = []
        self.soup = None
        self.m_seed_url = ""
    ...
    def supports(cls, platform: str) -> bool:
        return (platform or "").strip().lower() == "medium"
    ...
    def rule_config(self) -> RuleModel:
        return RuleModel(
            ...
            m_threat_type=ThreatType.MEDIUM,
            m_rule_type=RuleType.MEDIUM,
    ...
    def _extract_profile(page) -> dict:
        try:
            return page.evaluate("""async () => {
                ...
                const feedUrl = () => {
                    if (location.hostname.replace(/^www\\./, '') === 'medium.com') return `https://medium.com/feed${location.pathname.replace(/\\/$/, '')}`;
                    return `${location.origin}/feed`;
                };
                const childText = (node, names) => {
                    const wanted = names.map(name => name.toLowerCase());
                    const child = Array.from(node?.children || []).find(el => wanted.includes(String(el.localName || '').toLowerCase()) || wanted.includes(String(el.tagName || '').toLowerCase()));
                ...
                try {
                    const res = await fetch(feedUrl(), {headers: {accept: 'application/rss+xml,application/xml,text/xml,*/*'}, credentials: 'omit'});
                    if (res.ok) {
                        ...
                        if (channel) {
                            feedProfile = {
                                ...
                                bio: stripHtml(childText(channel, ['description'])),
                                canonical: childText(channel, ['link']),
                                profileIcon: childText(channel.querySelector('image'), ['url']),
                            };
                        }
                    }
                } catch (_) {}
                ...
                const profileIcon = absolutize(feedProfile?.profileIcon)
                ...
                return {
                    title: feedProfile?.title || clean(meta('og:title', 'twitter:title') || document.title || text('h1')),
                    bio: feedProfile?.bio || clean(meta('og:description', 'twitter:description', 'description') || text('[data-testid*="bio" i], .bio, .description, header p, main p')),
                    canonical: absolutize(feedProfile?.canonical || document.querySelector('link[rel="canonical"]')?.getAttribute('href')) || location.href,
                    profileIcon,
                    coverpage,
                    stats: 'RSS: available',
                };
    ...
    def _extract_posts(page, limit: int) -> list[dict]:
        try:
            return page.evaluate("""async limit => {
                ...
                const child = (node, names) => {
                    const wanted = names.map(name => name.toLowerCase());
                    return Array.from(node?.children || []).find(el => wanted.includes(String(el.localName || '').toLowerCase()) || wanted.includes(String(el.tagName || '').toLowerCase()));
                };
                const childText = (node, names) => clean(child(node, names)?.textContent);
                const imageFromHtml = html => {
                    const doc = new DOMParser().parseFromString(String(html || ''), 'text/html');
                    const img = doc.querySelector('img[src]');
                    return absolutize(img?.getAttribute('src') || '');
                };
                const rows = [];
                try {
                    const res = await fetch(feedUrl(), {headers: {accept: 'application/rss+xml,application/xml,text/xml,*/*'}, credentials: 'omit'});
                    if (res.ok) {
                        const xml = new DOMParser().parseFromString(await res.text(), 'text/xml');
                        for (const item of Array.from(xml.querySelectorAll('item')).slice(0, limit * 2)) {
                            const html = childText(item, ['encoded', 'content:encoded', 'description']);
                            const link = childText(item, ['link']);
                            const guid = childText(item, ['guid']);
                            rows.push({
                                url: link,
                                title: childText(item, ['title']),
                                caption: clean(html).slice(0, 1200),
                                image: imageFromHtml(html),
                                timestamp: childText(item, ['pubDate', 'updated', 'atom:updated']),
                                author: childText(item, ['creator', 'dc:creator']),
                                messageId: guid || link,
                                tags: Array.from(item.children || []).filter(el => String(el.localName || '').toLowerCase() === 'category').map(el => clean(el.textContent)).filter(Boolean),
                            });
                        }
                    }
                } catch (_) {}
                ...
                const roots = Array.from(document.querySelectorAll('article, [role="article"]')).slice(0, Math.max(limit * 6, 30));
                const fromDom = roots.map(root => {
                    const linkNode = Array.from(root.querySelectorAll('a[href]')).find(a => {
                        ...
                        return href && /medium\\.com|infosecwriteups\\.com/i.test(href) && !/\\/tag\\//i.test(href) && href !== location.href;
                    });
