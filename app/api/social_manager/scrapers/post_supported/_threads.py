...
class _threads(extraction_interface, ABC):
    _instance = None
    ...
    def supports(cls, platform: str) -> bool:
        return (platform or "").strip().lower() == "threads"
    ...
    def rule_config(self) -> RuleModel:
        return RuleModel(
            ...
            m_threat_type=ThreatType.THREADS,
            m_rule_type=RuleType.THREADS,
    ...
    def _extract_profile(page) -> dict:
        try:
            return page.evaluate("""() => {
                ...
                const username = (location.pathname.match(/@([^/?#]+)/)?.[1] || '').replace(/^@/, '');
                const displayName = bodyLines.find((line, index) => index > 0 && line.toLowerCase() !== username.toLowerCase() && !/^\\d/.test(line)) || '';
                const usernameIndex = bodyLines.findIndex((line, index) => index > 0 && line.toLowerCase() === username.toLowerCase());
                const bio = usernameIndex >= 0 ? bodyLines.slice(usernameIndex + 1).find(line => {
                    const lower = line.toLowerCase();
                    return line && lower !== username.toLowerCase() && !/^follow(?:er)?s?$/.test(lower) && !/^follow$|^mention$|^threads$|^replies$|^media$|^reposts$/.test(lower);
                }) || '' : '';
                const description = clean(meta('og:description', 'twitter:description', 'description'));
                const postCount = description.match(/([0-9][0-9.,]*\\s*[KMBkmb]?)\\s+Threads\\b/)?.[1] || '';
                return {
                    title: clean(displayName || meta('og:title', 'twitter:title') || document.title).replace(/\\s*\\(@[^)]+\\).*$/i, ''),
                    bio: clean(bio || description.replace(/^.*?Threads\\s+•\\s*/i, '')),
                    ...
                    coverpage: '',
                    stats: postCount ? `POSTS: ${postCount}` : '',
                };
            }""") or {}
        except Exception:
            return {}
    ...
    def _extract_posts(page, limit: int) -> list[dict]:
        try:
            return page.evaluate("""limit => {
                ...
                const username = (location.pathname.match(/@([^/?#]+)/)?.[1] || '').replace(/^@/, '');
                const decodeThreadsLink = value => {
                    try {
                        const parsed = new URL(value, location.href);
                        return parsed.hostname === 'l.threads.com' ? parsed.searchParams.get('u') || parsed.href : parsed.href;
                    } catch (_) {
                        return value || '';
                    }
                };
                const seen = new Set();
                return Array.from(document.querySelectorAll('a[href*="/post/"]')).filter(link => {
                    const href = link.getAttribute('href') || '';
                    return href && !href.endsWith('/media');
                }).map(link => {
                    let root = link;
                    for (let i = 0; i < 9 && root?.parentElement; i += 1) {
                        root = root.parentElement;
                        const lines = (root.innerText || '').split('\\n').map(clean).filter(Boolean);
                        if (lines.length >= 3 && lines.some(line => line.toLowerCase() === username.toLowerCase())) break;
                    }
                    const lines = (root?.innerText || '').split('\\n').map(clean).filter(Boolean);
                    const timeNode = link.querySelector('time[datetime]') || root?.querySelector(`a[href="${link.getAttribute('href')}"] time[datetime]`) || root?.querySelector('time[datetime]');
                    const dateText = clean(timeNode?.innerText || link.innerText || '');
                    const startIndex = lines.findIndex(line => line === dateText);
                    const textLines = (startIndex >= 0 ? lines.slice(startIndex + 1) : lines.slice(2)).filter(line => !/^\\d+[.,]?\\d*[KMBkmb]?$/.test(line));
                    const stopWords = new Set(['Follow', 'Mention', 'Threads', 'Replies', 'Media', 'Reposts', 'Log in']);
                    const caption = clean(textLines.filter(line => !stopWords.has(line)).join(' '));
                    const mediaImage = root?.querySelector(`a[href$="/media"] img`) || null;
                    const linkedUrls = Array.from(root?.querySelectorAll('a[href^="https://l.threads.com/"]') || []).map(a => decodeThreadsLink(a.href)).filter(Boolean);
                    const postUrl = absolutize(link.getAttribute('href'));
                    return {
                        url: postUrl,
                        title: caption.slice(0, 180),
                        caption,
                        image: absolutize(mediaImage?.currentSrc || mediaImage?.src || mediaImage?.getAttribute('src') || ''),
                        timestamp: timeNode?.getAttribute('datetime') || '',
                        author: username,
                        username,
                        messageId: postUrl.split('/post/')[1]?.split(/[/?#]/)[0] || postUrl,
                        linkedUrls,
                    };
                }).filter(item => {
