...
PLATFORM_CONFIG: Dict[str, Any] = {
    "name": "Primal",
    "profile_url": "https://primal.net/p/{username}",
    "posts_url": "https://primal.net/search/%23{username}",
    "domains": [
        "primal.net"
    ]
}
...
class _primal(extraction_interface, ABC):
    _instance = None

    def __init__(self, callback=None):
        super().__init__()
        self.callback = callback
        self.platform = "primal"
        self._card_data = []
        self._entity_data = []
        self.soup = None
        self.m_seed_url = ""
    ...
    def supports(cls, platform: str) -> bool:
        return (platform or "").strip().lower() == "primal"
    ...
    def developer_signature(self) -> str:
        return "Orion Primal/Nostr post scraper"
    ...
    def rule_config(self) -> RuleModel:
        return RuleModel(
            ...
            m_threat_type=ThreatType.PRIMAL,
            m_rule_type=RuleType.PRIMAL,
        ...
        try:
            return page.evaluate("""async () => {
                ...
                const extractTag = () => {
                    ...
                    return clean(queryTag || (raw.match(/(?:\\/search\\/%23|\\/search\\/#|\\/search\\/|#\\/t\\/|\\/t\\/|[#?&]q=%23)([A-Za-z0-9_+-]+)/i) || [])[1] || '').replace(/^#/, '').toLowerCase();
                };
                ...
                return {
                    title: tag ? `Primal Nostr tag: ${tag}` : clean(meta('og:title', 'twitter:title') || document.title || 'Primal'),
        ...
        try:
            return page.evaluate("""async limit => {
                ...
                return events.map(event => {
                    ...
                    return {
                        url: `https://primal.net/e/${event.id}`,
                        title,
                        caption,
