...
class _hashnode(extraction_interface, ABC):
    _instance = None

    def __init__(self, callback=None):
        super().__init__()
        self.callback = callback
        self.platform = "hashnode"
        self._card_data = []
        self._entity_data = []
        self.soup = None
        self.m_seed_url = ""
    ...
    def supports(cls, platform: str) -> bool:
        return (platform or "").strip().lower() == "hashnode"
    ...
    def rule_config(self) -> RuleModel:
        return RuleModel(
            ...
            m_threat_type=ThreatType.HASHNODE,
            m_rule_type=RuleType.HASHNODE,
    ...
    def _extract_profile(page) -> dict:
        try:
            return page.evaluate("""() => {
                ...
                const count = description.match(/Explore\\s+([0-9][0-9,]*)\\s+articles/i)?.[1] || '';
                const statMatches = [tag ? `TAG: ${tag}` : '', count ? `POSTS: ${count}` : ''].filter(Boolean);
                return {
                    title: clean(meta('og:title', 'twitter:title') || document.title || text('h1')),
                    bio: description || text('[data-testid*="bio" i], .bio, .description, header p, main p'),
            ...
            return page.evaluate("""limit => {
                ...
                const isArticleUrl = value => {
                    try {
                        ...
                        if (/^hashnode\\.com$/i.test(host) && /^\\/@|^\\/tag\\/|^\\/posts\\//i.test(url.pathname)) return false;
                        if (/github\\.com|linkedin\\.com|twitter\\.com|x\\.com|facebook\\.com|instagram\\.com|youtube\\.com|discord\\.|t\\.me/i.test(host)) return false;
                        return /^https?:$/.test(url.protocol) && url.pathname.replace(/\\/$/, '').split('/').filter(Boolean).length >= 1;
                    } catch (_) {
                        return false;
                    }
                };
                const tag = decodeURIComponent(location.pathname.split('/').filter(Boolean).pop() || '');
                const fromDom = Array.from(document.querySelectorAll('article')).slice(0, Math.max(limit * 4, 24)).map(root => {
                    const titleNode = root.querySelector('h1,h2,h3');
                    const linkNode = Array.from(root.querySelectorAll('a[href]')).find(a => a.contains(titleNode) || a.getAttribute('aria-label') === clean(titleNode?.innerText));
                    const mediaNode = root.querySelector('img');
                    const title = clean(titleNode?.innerText || linkNode?.innerText || '');
                    const caption = clean(root.querySelector('p')?.innerText || title || root.innerText || '');
                    const text = clean(root.innerText || '');
                    const timestamp = text.match(/\\b(?:\\d+[hmwd]\\s+ago|\\d+\\s+(?:min|hour|day|week|month)s?\\s+ago|[A-Z][a-z]{2}\\s+\\d{1,2},\\s+\\d{4})\\b/)?.[0] || '';
                    const url = normalizeArticleUrl(linkNode?.getAttribute('href') || '');
                    const image = absolutize(mediaNode?.currentSrc || mediaNode?.src || mediaNode?.getAttribute('src') || '');
                    const author = clean(root.querySelector('a[href^="/@"] span:last-child, a[href^="/@"]')?.innerText || '');
                    const discussion = absolutize(root.querySelector('a[href^="/posts/"]')?.getAttribute('href') || '');
                    return {url, title, caption, image, timestamp, author, messageId: discussion || url, tags: tag ? [tag] : []};
                });
                const seen = new Set();
                return fromDom.filter(item => {
                    item.url = normalizeArticleUrl(item.url);
                    item.image = absolutize(item.image);
                    item.title = clean(item.title);
                    item.caption = clean(item.caption || item.title);
                    if (!isArticleUrl(item.url) || !item.title || !item.caption) return false;
