...
PLATFORM_CONFIG: Dict[str, Any] = {
    "name": "Pleroma",
    "profile_url": "https://pleroma.io/users/{username}",
    "domains": [
        "pleroma.io"
    ]
}
...
class _pleroma(extraction_interface, ABC):
    _instance = None

    def __init__(self, callback=None):
        super().__init__()
        self.callback = callback
        self.platform = "pleroma"
        self._card_data = []
        self._entity_data = []
        self.soup = None
        self.m_seed_url = ""
    ...
    def supports(cls, platform: str) -> bool:
        return (platform or "").strip().lower() == "pleroma"
    ...
    def rule_config(self) -> RuleModel:
        return RuleModel(
            ...
            m_threat_type=ThreatType.PLEROMA,
            m_rule_type=RuleType.PLEROMA,
    ...
    def _extract_profile(page) -> dict:
        try:
            return page.evaluate("""async () => {
                ...
                const tag = pathParts[0] === 'tag' ? pathParts[1] || '' : '';
                const accountPart = pathParts[0] === 'users' ? pathParts[1] || '' : (pathParts.find(part => part.startsWith('@')) || '').replace(/^@/, '');
                const instance = await getJson('/api/v1/instance') || {};
                let account = null;
                let postsCount = '';
                if (tag) {
                    const statuses = await getJson(`/api/v1/timelines/tag/${encodeURIComponent(tag)}?limit=20`);
                    postsCount = Array.isArray(statuses) ? statuses.filter(status => !status.in_reply_to_id && !status.reblog).length : '';
                } else if (accountPart) {
                    account = await getJson(`/api/v1/accounts/lookup?acct=${encodeURIComponent(accountPart)}`);
                    postsCount = account?.statuses_count ?? '';
                }
                const title = tag ? `Pleroma tag: ${tag}` : clean(account?.display_name || account?.acct || account?.username || instance.title || document.title);
                const stats = [
                    tag ? `TAG: ${tag}` : '',
                ...
                return {
                    title,
                    bio: htmlToText(account?.note || instance.description || ''),
                    canonical: absolutize(account?.url || location.href),
                    profileIcon: absolutize(account?.avatar || account?.avatar_static || instance.thumbnail || document.querySelector('link[rel*="icon"]')?.getAttribute('href')),
                    coverpage: absolutize(account?.header || account?.header_static || instance.background_image || ''),
                    stats,
                };
            }""") or {}
        except Exception:
    ...
    def _extract_posts(page, limit: int) -> list[dict]:
        try:
            return page.evaluate("""async limit => {
                ...
                let statuses = [];
                if (tag) {
                    const result = await getJson(`/api/v1/timelines/tag/${encodeURIComponent(tag)}?limit=${Math.min(Math.max(limit * 2, 10), 40)}`);
                    statuses = Array.isArray(result) ? result : [];
                } else if (accountPart) {
                    const account = await getJson(`/api/v1/accounts/lookup?acct=${encodeURIComponent(accountPart)}`);
                    if (account?.id) {
                        const result = await getJson(`/api/v1/accounts/${encodeURIComponent(account.id)}/statuses?limit=${Math.min(Math.max(limit * 2, 10), 40)}&exclude_replies=true&exclude_reblogs=true`);
                        statuses = Array.isArray(result) ? result : [];
                    }
                }
                const seen = new Set();
                return statuses.filter(status => !status.in_reply_to_id && !status.reblog).map(status => {
                    const account = status.account || {};
                    const text = clean(status.pleroma?.content?.['text/plain'] || htmlToText(status.content || status.spoiler_text || ''));
                    const card = status.card || {};
                    const media = Array.isArray(status.media_attachments) ? status.media_attachments[0] : null;
                    const title = clean(text.split('\\n')[0] || card.title || text).slice(0, 180);
                    return {
                        url: absolutize(status.url || status.uri || `/notice/${status.id}`),
                        title,
                        caption: text || clean(card.description || card.title || ''),
                        image: absolutize(media?.preview_url || media?.url || card.image || account.avatar || ''),
                        timestamp: status.created_at || '',
                        author: clean(account.display_name || account.acct || account.username || ''),
                        username: clean(account.acct || account.username || ''),
                        messageId: clean(status.id || status.url || status.uri || ''),
                        likeCount: status.favourites_count,
                        repostCount: status.reblogs_count,
                        tags: Array.isArray(status.tags) ? status.tags.map(tag => clean(tag.name)).filter(Boolean) : [],
                    };
                }).filter(item => {
