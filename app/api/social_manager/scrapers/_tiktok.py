...
class _tiktok(extraction_interface, ABC):
    ...
    def _public_metadata_item(row: dict, username: str) -> dict:
        ...
        return {
            ...
            "stats": {
                "diggCount": row.get("digg_count") or row.get("like_count"),
    ...
    def _append_video_items(self, items: list[dict], username: str, data_type: SocialDataType) -> int:
        ...
        for item in items:
            ...
            self._append_video_card(
                ...
                likes=self._tiktok_count(stats.get("diggCount") or stats.get("digg_count") or stats.get("likeCount")),
                comments=self._tiktok_count(stats.get("commentCount") or stats.get("comment_count")),
                shares=self._tiktok_count(stats.get("shareCount") or stats.get("share_count")),
                views=self._tiktok_count(stats.get("playCount") or stats.get("play_count") or stats.get("viewCount")),
                media_url=media_url,
                data_type=data_type,
            )
    ...
    def _append_profile_card(self, username: str, user_info: dict, data_type: SocialDataType):
        ...
        follower_count = self._stat_count(stats, "followerCount", "follower_count")
        following_count = self._stat_count(stats, "followingCount", "following_count")
    ...
    def parse_leak_data(self, page=None):
        ...
        try:
            ...
            if username and data_type in (SocialDataType.POSTS, SocialDataType.VIDEOS, SocialDataType.SHORTS):
                ...
                self._profile_missing = self._page_shows_missing_account(page)
                if self._profile_missing:
                    log.g().e(f"Skipping TikTok {data_type.value} for {username}: account not found.")
                    return
                self._profile_metadata = self._extract_profile_assets(page)
                self._profile_user_detail = self._extract_user_detail_from_page(page)
            ...
            if username and self._profile_user_detail and data_type in (
                ...
                self._append_profile_card(username, self._profile_user_detail, data_type)
                return
    ...
    async def _parse_leak_data_async(self):
        ...
        async with TikTokApi() as api:
            await api.create_sessions(ms_tokens=[self.MS_TOKEN], num_sessions=1, sleep_after=random.randint(1, 10))
            ...
            if data_type in (SocialDataType.PROFILE, SocialDataType.CHANNEL, SocialDataType.FOLLOWERS, SocialDataType.FOLLOWING):
                try:
                    ...
                    user_info = {}
                self._append_profile_card(username, user_info, data_type)
                return
