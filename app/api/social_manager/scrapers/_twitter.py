...
class _twitter(extraction_interface, ABC):
    ...
    def _extract_profile_assets(page, username: str) -> dict:
        ...
        try:
            return page.evaluate("""(username) => {
                ...
                const followersText = countText('Followers');
                const followingText = countText('Following');
                ...
                return {
                    ...
                    followers: countFromLabel(followersText, 'Followers'),
                    following: countFromLabel(followingText, 'Following')
                };
            }""", username)
        except Exception:
            return {}
    ...
    def _append_profile_info(self, page, username: str):
        ...
        followers = self._normalize_profile_count(profile_assets.get("followers"))
        following = self._normalize_profile_count(profile_assets.get("following"))
        group_info = " | ".join(
            item for item in [
                f"FOLLOWERS: {followers}" if followers else "",
                f"FOLLOWING: {following}" if following else "",
