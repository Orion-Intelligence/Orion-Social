...
from api.social_manager.social_recon.constants.platform_constants import AvoneMeConstants

constants = AvoneMeConstants


def probe_url(username: str) -> str:
    return AvoneMeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(AvoneMeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(AvoneMeConstants, "AVATAR_KEYS", ()), getattr(AvoneMeConstants, "COVER_KEYS", ()))
