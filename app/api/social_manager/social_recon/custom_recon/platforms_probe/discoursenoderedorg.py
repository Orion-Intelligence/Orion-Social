...
from api.social_manager.social_recon.constants.platform_constants import DiscourseNoderedOrgConstants

constants = DiscourseNoderedOrgConstants


def probe_url(username: str) -> str:
    return DiscourseNoderedOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DiscourseNoderedOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DiscourseNoderedOrgConstants, "AVATAR_KEYS", ()), getattr(DiscourseNoderedOrgConstants, "COVER_KEYS", ()))
