...
from api.social_manager.social_recon.constants.platform_constants import TruelancerConstants

constants = TruelancerConstants


def probe_url(username: str) -> str:
    return TruelancerConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TruelancerConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TruelancerConstants, "AVATAR_KEYS", ()), getattr(TruelancerConstants, "COVER_KEYS", ()))
