...
def probe_url(username: str) -> str:
    return WarriorForumConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WarriorForumConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WarriorForumConstants, "AVATAR_KEYS", ()), getattr(WarriorForumConstants, "COVER_KEYS", ()))
