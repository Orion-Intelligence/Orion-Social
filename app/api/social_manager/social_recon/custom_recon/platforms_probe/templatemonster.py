...
def probe_url(username: str) -> str:
    return TemplateMonsterConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TemplateMonsterConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TemplateMonsterConstants, "AVATAR_KEYS", ()), getattr(TemplateMonsterConstants, "COVER_KEYS", ()))
