...
from api.social_manager.social_recon.constants.platform_constants import ExploretalentComConstants

constants = ExploretalentComConstants


def probe_url(username: str) -> str:
    return ExploretalentComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ExploretalentComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ExploretalentComConstants, "AVATAR_KEYS", ()), getattr(ExploretalentComConstants, "COVER_KEYS", ()))
