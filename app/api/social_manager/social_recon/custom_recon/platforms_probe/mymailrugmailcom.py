...
from api.social_manager.social_recon.constants.platform_constants import MyMailRuGmailComConstants

constants = MyMailRuGmailComConstants


def probe_url(username: str) -> str:
    return MyMailRuGmailComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MyMailRuGmailComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MyMailRuGmailComConstants, "AVATAR_KEYS", ()), getattr(MyMailRuGmailComConstants, "COVER_KEYS", ()))
