...
from api.social_manager.social_recon.constants.platform_constants import BiggerpocketsConstants

constants = BiggerpocketsConstants


def probe_url(username: str) -> str:
    return BiggerpocketsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(BiggerpocketsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(BiggerpocketsConstants, "AVATAR_KEYS", ()), getattr(BiggerpocketsConstants, "COVER_KEYS", ()))
    ...
    if info.get("display_name"):
        info["display_name"] = parse.clean(re.split(r"\s*\|\s*BiggerPockets", info["display_name"], maxsplit=1)[0])
