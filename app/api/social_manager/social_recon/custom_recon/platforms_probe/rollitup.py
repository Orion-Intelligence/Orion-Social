...
def probe_url(username: str) -> str:
    return RollitupConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(RollitupConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(RollitupConstants, "AVATAR_KEYS", ()), getattr(RollitupConstants, "COVER_KEYS", ()))
