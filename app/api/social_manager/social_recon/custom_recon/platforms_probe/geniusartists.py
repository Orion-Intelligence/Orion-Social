...
from api.social_manager.social_recon.constants.platform_constants import GeniusArtistsConstants

constants = GeniusArtistsConstants


def probe_url(username: str) -> str:
    return GeniusArtistsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(GeniusArtistsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(GeniusArtistsConstants, "AVATAR_KEYS", ()), getattr(GeniusArtistsConstants, "COVER_KEYS", ()))
