...
from api.social_manager.social_recon.constants.platform_constants import Aminus3Constants

constants = Aminus3Constants


def probe_url(username: str) -> str:
    return Aminus3Constants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(Aminus3Constants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(Aminus3Constants, "AVATAR_KEYS", ()), getattr(Aminus3Constants, "COVER_KEYS", ()))
