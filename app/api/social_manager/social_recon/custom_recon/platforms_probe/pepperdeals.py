...
from api.social_manager.social_recon.constants.platform_constants import PepperdealsConstants

constants = PepperdealsConstants


def probe_url(username: str) -> str:
    return PepperdealsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PepperdealsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PepperdealsConstants, "AVATAR_KEYS", ()), getattr(PepperdealsConstants, "COVER_KEYS", ()))
