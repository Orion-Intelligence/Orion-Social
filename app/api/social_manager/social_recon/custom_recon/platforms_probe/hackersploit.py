...
from api.social_manager.social_recon.constants.platform_constants import HackerSploitConstants

constants = HackerSploitConstants


def probe_url(username: str) -> str:
    return HackerSploitConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(HackerSploitConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(HackerSploitConstants, "AVATAR_KEYS", ()), getattr(HackerSploitConstants, "COVER_KEYS", ()))
