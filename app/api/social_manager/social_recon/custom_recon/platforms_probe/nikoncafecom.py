...
def probe_url(username: str) -> str:
    return NikoncafeComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(NikoncafeComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(NikoncafeComConstants, "AVATAR_KEYS", ()), getattr(NikoncafeComConstants, "COVER_KEYS", ()))
