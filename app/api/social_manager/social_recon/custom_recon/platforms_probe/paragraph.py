...
def probe_url(username: str) -> str:
    return ParagraphConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ParagraphConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ParagraphConstants, "AVATAR_KEYS", ()), getattr(ParagraphConstants, "COVER_KEYS", ()))
