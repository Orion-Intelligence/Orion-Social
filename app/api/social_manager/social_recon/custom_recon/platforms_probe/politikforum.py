...
def probe_url(username: str) -> str:
    return PolitikforumConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PolitikforumConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PolitikforumConstants, "AVATAR_KEYS", ()), getattr(PolitikforumConstants, "COVER_KEYS", ()))
