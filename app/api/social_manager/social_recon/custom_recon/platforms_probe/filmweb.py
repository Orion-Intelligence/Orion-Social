...
from api.social_manager.social_recon.constants.platform_constants import FilmWebConstants

constants = FilmWebConstants


def probe_url(username: str) -> str:
    return FilmWebConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FilmWebConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FilmWebConstants, "AVATAR_KEYS", ()), getattr(FilmWebConstants, "COVER_KEYS", ()))
