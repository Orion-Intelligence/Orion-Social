...
def probe_url(username: str) -> str:
    return WeChatConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WeChatConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WeChatConstants, "AVATAR_KEYS", ()), getattr(WeChatConstants, "COVER_KEYS", ()))
