...
from api.social_manager.social_recon.constants.platform_constants import FinforumConstants

constants = FinforumConstants


def probe_url(username: str) -> str:
    return FinforumConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FinforumConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FinforumConstants, "AVATAR_KEYS", ()), getattr(FinforumConstants, "COVER_KEYS", ()))
