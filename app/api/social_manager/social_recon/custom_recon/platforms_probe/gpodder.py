...
def probe_url(username: str) -> str:
    return GpodderConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(GpodderConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(GpodderConstants, "AVATAR_KEYS", ()), getattr(GpodderConstants, "COVER_KEYS", ()))
