...
from api.social_manager.social_recon.constants.platform_constants import DribbbleConstants

constants = DribbbleConstants


def probe_url(username: str) -> str:
    return DribbbleConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in DribbbleConstants.GENERIC:
        return VerdictConstants.UNKNOWN, {}
    meta = parse.meta(body)
    person = parse.ld_entity(body, "Person", "ProfilePage")
    avatar = parse.text(person.get("image"))
    info = {
        "display_name": parse.clean(person.get("name") or meta.get("og:title")),
        "username": parse.text(meta.get("og:url")).rstrip("/").split("/")[-1],
        "description": parse.clean(person.get("description") or meta.get("og:description")),
