...
from api.social_manager.social_recon.constants.platform_constants import BulbappComConstants

constants = BulbappComConstants


def probe_url(username: str) -> str:
    return BulbappComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(BulbappComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(BulbappComConstants, "AVATAR_KEYS", ()), getattr(BulbappComConstants, "COVER_KEYS", ()))
