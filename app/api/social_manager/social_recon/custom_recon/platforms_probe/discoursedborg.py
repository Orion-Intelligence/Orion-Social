...
from api.social_manager.social_recon.constants.platform_constants import DiscoursedbOrgConstants

constants = DiscoursedbOrgConstants


def probe_url(username: str) -> str:
    return DiscoursedbOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DiscoursedbOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DiscoursedbOrgConstants, "AVATAR_KEYS", ()), getattr(DiscoursedbOrgConstants, "COVER_KEYS", ()))
