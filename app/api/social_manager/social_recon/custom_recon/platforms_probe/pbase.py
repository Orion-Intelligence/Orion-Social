...
def probe_url(username: str) -> str:
    return PbaseConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PbaseConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PbaseConstants, "AVATAR_KEYS", ()), getattr(PbaseConstants, "COVER_KEYS", ()))
