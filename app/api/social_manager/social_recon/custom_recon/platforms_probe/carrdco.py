...
from api.social_manager.social_recon.constants.platform_constants import CarrdCoConstants

constants = CarrdCoConstants


def probe_url(username: str) -> str:
    return CarrdCoConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CarrdCoConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CarrdCoConstants, "AVATAR_KEYS", ()), getattr(CarrdCoConstants, "COVER_KEYS", ()))
