...
def probe_url(username: str) -> str:
    return IcheckmoviesConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(IcheckmoviesConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(IcheckmoviesConstants, "AVATAR_KEYS", ()), getattr(IcheckmoviesConstants, "COVER_KEYS", ()))
