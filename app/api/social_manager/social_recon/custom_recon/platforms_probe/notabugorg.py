...
def probe_url(username: str) -> str:
    return NotabugOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(NotabugOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(NotabugOrgConstants, "AVATAR_KEYS", ()), getattr(NotabugOrgConstants, "COVER_KEYS", ()))
