...
def probe_url(username: str) -> str:
    return ImageShackConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ImageShackConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ImageShackConstants, "AVATAR_KEYS", ()), getattr(ImageShackConstants, "COVER_KEYS", ()))
