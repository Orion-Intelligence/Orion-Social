...
from api.social_manager.social_recon.constants.platform_constants import CodePenConstants

constants = CodePenConstants


def probe_url(username: str) -> str:
    return CodePenConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in CodePenConstants.GENERIC:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, CodePenConstants.AVATAR_KEYS, CodePenConstants.COVER_KEYS)
    name = parse.clean(info.get("display_name"))
    if name.endswith(CodePenConstants.NAME_SUFFIX):
        name = name[: -len(CodePenConstants.NAME_SUFFIX)].strip()
    info["display_name"] = name
    if info.get("description"):
        info["description"] = parse.clean(info["description"])
    handle = parse.text(parse.meta(body).get("og:url")).rstrip("/").split("/")[-1]
    if handle:
