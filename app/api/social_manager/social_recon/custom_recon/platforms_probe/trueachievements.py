...
def probe_url(username: str) -> str:
    return TrueAchievementsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TrueAchievementsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TrueAchievementsConstants, "AVATAR_KEYS", ()), getattr(TrueAchievementsConstants, "COVER_KEYS", ()))
