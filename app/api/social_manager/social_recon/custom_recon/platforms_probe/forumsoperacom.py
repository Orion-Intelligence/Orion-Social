...
from api.social_manager.social_recon.constants.platform_constants import ForumsOperaComConstants

constants = ForumsOperaComConstants


def probe_url(username: str) -> str:
    return ForumsOperaComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForumsOperaComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumsOperaComConstants, "AVATAR_KEYS", ()), getattr(ForumsOperaComConstants, "COVER_KEYS", ()))
