...
from api.social_manager.social_recon.constants.platform_constants import PaypalConstants

constants = PaypalConstants


def probe_url(username: str) -> str:
    return PaypalConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PaypalConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PaypalConstants, "AVATAR_KEYS", ()), getattr(PaypalConstants, "COVER_KEYS", ()))
