...
from api.social_manager.social_recon.constants.platform_constants import GeodesistConstants

constants = GeodesistConstants


def probe_url(username: str) -> str:
    return GeodesistConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(GeodesistConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(GeodesistConstants, "AVATAR_KEYS", ()), getattr(GeodesistConstants, "COVER_KEYS", ()))
