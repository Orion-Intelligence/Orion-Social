...
def probe_url(username: str) -> str:
    return OdyseeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(OdyseeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(OdyseeConstants, "AVATAR_KEYS", ()), getattr(OdyseeConstants, "COVER_KEYS", ()))
