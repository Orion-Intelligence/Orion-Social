...
from api.social_manager.social_recon.constants.platform_constants import WwwKinokopilkaProConstants

constants = WwwKinokopilkaProConstants


def probe_url(username: str) -> str:
    return WwwKinokopilkaProConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WwwKinokopilkaProConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WwwKinokopilkaProConstants, "AVATAR_KEYS", ()), getattr(WwwKinokopilkaProConstants, "COVER_KEYS", ()))
