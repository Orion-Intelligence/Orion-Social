...
def probe_url(username: str) -> str:
    return ThemeForestConstants.PROFILE_URL.format(username=username)


def fetch(username: str) -> tuple[int, str, str]:
    return http_client.fetch(probe_url(username), impersonate=ThemeForestConstants.IMPERSONATE)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ThemeForestConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ThemeForestConstants, "AVATAR_KEYS", ()), getattr(ThemeForestConstants, "COVER_KEYS", ()))
