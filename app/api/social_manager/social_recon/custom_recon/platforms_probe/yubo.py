...
from api.social_manager.social_recon.constants.platform_constants import YuboConstants

constants = YuboConstants


def probe_url(username: str) -> str:
    return YuboConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(YuboConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(YuboConstants, "AVATAR_KEYS", ()), getattr(YuboConstants, "COVER_KEYS", ()))
