...
from api.social_manager.social_recon.constants.platform_constants import SuzuriJpConstants

constants = SuzuriJpConstants


def probe_url(username: str) -> str:
    return SuzuriJpConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SuzuriJpConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SuzuriJpConstants, "AVATAR_KEYS", ()), getattr(SuzuriJpConstants, "COVER_KEYS", ()))
