...
def probe_url(username: str) -> str:
    return BitBucketConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(BitBucketConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(BitBucketConstants, "AVATAR_KEYS", ()), getattr(BitBucketConstants, "COVER_KEYS", ()))
