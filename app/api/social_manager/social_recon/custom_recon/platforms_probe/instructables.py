...
def probe_url(username: str) -> str:
    return InstructablesConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(InstructablesConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(InstructablesConstants, "AVATAR_KEYS", ()), getattr(InstructablesConstants, "COVER_KEYS", ()))
