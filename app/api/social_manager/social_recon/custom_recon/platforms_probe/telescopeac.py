...
from api.social_manager.social_recon.constants.platform_constants import TelescopeAcConstants

constants = TelescopeAcConstants


def probe_url(username: str) -> str:
    return TelescopeAcConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TelescopeAcConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TelescopeAcConstants, "AVATAR_KEYS", ()), getattr(TelescopeAcConstants, "COVER_KEYS", ()))
