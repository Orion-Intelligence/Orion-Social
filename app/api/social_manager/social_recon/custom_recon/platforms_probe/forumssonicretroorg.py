...
from api.social_manager.social_recon.constants.platform_constants import ForumsSonicretroOrgConstants

constants = ForumsSonicretroOrgConstants


def probe_url(username: str) -> str:
    return ForumsSonicretroOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForumsSonicretroOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumsSonicretroOrgConstants, "AVATAR_KEYS", ()), getattr(ForumsSonicretroOrgConstants, "COVER_KEYS", ()))
