...
from api.social_manager.social_recon.constants.platform_constants import SocialLibremOneConstants

constants = SocialLibremOneConstants


def probe_url(username: str) -> str:
    return SocialLibremOneConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SocialLibremOneConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SocialLibremOneConstants, "AVATAR_KEYS", ()), getattr(SocialLibremOneConstants, "COVER_KEYS", ()))
