...
def probe_url(username: str) -> str:
    return CTANConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CTANConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CTANConstants, "AVATAR_KEYS", ()), getattr(CTANConstants, "COVER_KEYS", ()))
