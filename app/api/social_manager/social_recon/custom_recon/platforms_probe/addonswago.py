...
from api.social_manager.social_recon.constants.platform_constants import AddonsWagoConstants

constants = AddonsWagoConstants


def probe_url(username: str) -> str:
    return AddonsWagoConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(AddonsWagoConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(AddonsWagoConstants, "AVATAR_KEYS", ()), getattr(AddonsWagoConstants, "COVER_KEYS", ()))
