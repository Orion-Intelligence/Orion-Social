...
from api.social_manager.social_recon.constants.platform_constants import CommunityPix4dComConstants

constants = CommunityPix4dComConstants


def probe_url(username: str) -> str:
    return CommunityPix4dComConstants.API_URL.format(username=username)
...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    if not isinstance(user, dict) or not user.get("username"):
        return VerdictConstants.UNKNOWN, {}
    base = CommunityPix4dComConstants.PROFILE_URL.split("/u/")[0]
