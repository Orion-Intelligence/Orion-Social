...
def probe_url(username: str) -> str:
    return JigidiConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(JigidiConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(JigidiConstants, "AVATAR_KEYS", ()), getattr(JigidiConstants, "COVER_KEYS", ()))
