...
from api.social_manager.social_recon.constants.platform_constants import AuthorTodayConstants

constants = AuthorTodayConstants


def probe_url(username: str) -> str:
    return AuthorTodayConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(AuthorTodayConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(AuthorTodayConstants, "AVATAR_KEYS", ()), getattr(AuthorTodayConstants, "COVER_KEYS", ()))
