...
from api.social_manager.social_recon.constants.platform_constants import BelgaeSocialConstants

constants = BelgaeSocialConstants


def probe_url(username: str) -> str:
    return BelgaeSocialConstants.PROFILE_URL.format(username=username)


def fetch(username: str) -> tuple[int, str, str]:
    return http_client.fetch(BelgaeSocialConstants.API_URL.format(username=username))
...
def evaluate_resource(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if community:
        code, payload, _ = http_client.fetch(BelgaeSocialConstants.COMMUNITY_API.format(id=community.group(1)))
