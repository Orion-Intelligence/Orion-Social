...
def probe_url(username: str) -> str:
    return SniperforumsComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SniperforumsComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SniperforumsComConstants, "AVATAR_KEYS", ()), getattr(SniperforumsComConstants, "COVER_KEYS", ()))
