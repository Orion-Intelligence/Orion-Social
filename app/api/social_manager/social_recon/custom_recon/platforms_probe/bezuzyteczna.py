...
from api.social_manager.social_recon.constants.platform_constants import BezuzytecznaConstants

constants = BezuzytecznaConstants


def probe_url(username: str) -> str:
    return BezuzytecznaConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(BezuzytecznaConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(BezuzytecznaConstants, "AVATAR_KEYS", ()), getattr(BezuzytecznaConstants, "COVER_KEYS", ()))
