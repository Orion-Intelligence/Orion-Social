...
def probe_url(username: str) -> str:
    return XgmGuruConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(XgmGuruConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(XgmGuruConstants, "AVATAR_KEYS", ()), getattr(XgmGuruConstants, "COVER_KEYS", ()))
