...
from api.social_manager.social_recon.constants.platform_constants import MstdnSocialConstants

constants = MstdnSocialConstants


def probe_url(username: str) -> str:
    return MstdnSocialConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MstdnSocialConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MstdnSocialConstants, "AVATAR_KEYS", ()), getattr(MstdnSocialConstants, "COVER_KEYS", ()))
