...
from api.social_manager.social_recon.constants.platform_constants import SlackConstants

constants = SlackConstants


def probe_url(username: str) -> str:
    return SlackConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SlackConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SlackConstants, "AVATAR_KEYS", ()), getattr(SlackConstants, "COVER_KEYS", ()))
