...
def probe_url(username: str) -> str:
    return WeforumConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WeforumConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WeforumConstants, "AVATAR_KEYS", ()), getattr(WeforumConstants, "COVER_KEYS", ()))
