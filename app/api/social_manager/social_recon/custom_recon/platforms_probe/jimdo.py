...
from api.social_manager.social_recon.constants.platform_constants import JimdoConstants

constants = JimdoConstants


def probe_url(username: str) -> str:
    return JimdoConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(JimdoConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(JimdoConstants, "AVATAR_KEYS", ()), getattr(JimdoConstants, "COVER_KEYS", ()))
...
HOSTS = ('jimdosite.com',)
SUBDOMAIN = ('jimdosite.com', "profile")
ROUTES = (
    ("(?P<id>[^/]+)", "profile"),
)
