...
from api.social_manager.social_recon.constants.platform_constants import DiscussFlarumOrgConstants

constants = DiscussFlarumOrgConstants


def probe_url(username: str) -> str:
    return DiscussFlarumOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DiscussFlarumOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DiscussFlarumOrgConstants, "AVATAR_KEYS", ()), getattr(DiscussFlarumOrgConstants, "COVER_KEYS", ()))
