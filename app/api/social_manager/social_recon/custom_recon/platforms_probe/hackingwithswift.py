...
def probe_url(username: str) -> str:
    return HackingWithSwiftConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(HackingWithSwiftConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(HackingWithSwiftConstants, "AVATAR_KEYS", ()), getattr(HackingWithSwiftConstants, "COVER_KEYS", ()))
