...
from api.social_manager.social_recon.constants.platform_constants import TwitterConstants

constants = TwitterConstants


def probe_url(username: str) -> str:
    return TwitterConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TwitterConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TwitterConstants, "AVATAR_KEYS", ()), getattr(TwitterConstants, "COVER_KEYS", ()))
