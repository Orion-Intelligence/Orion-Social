...
from api.social_manager.social_recon.constants.platform_constants import AntiscamSpaceConstants

constants = AntiscamSpaceConstants


def probe_url(username: str) -> str:
    return AntiscamSpaceConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if "?username=" in (final_url or ""):
        return VerdictConstants.ABSENT, {}
    heading = parse.title(body)
    if not heading or heading.casefold() in getattr(AntiscamSpaceConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(AntiscamSpaceConstants, "AVATAR_KEYS", ()), getattr(AntiscamSpaceConstants, "COVER_KEYS", ()))
