...
from api.social_manager.social_recon.constants.platform_constants import EnIllogicopediaOrgConstants

constants = EnIllogicopediaOrgConstants


def probe_url(username: str) -> str:
    return EnIllogicopediaOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(EnIllogicopediaOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(EnIllogicopediaOrgConstants, "AVATAR_KEYS", ()), getattr(EnIllogicopediaOrgConstants, "COVER_KEYS", ()))
