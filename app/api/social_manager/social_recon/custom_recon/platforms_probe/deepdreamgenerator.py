...
def probe_url(username: str) -> str:
    return DeepDreamGeneratorConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DeepDreamGeneratorConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DeepDreamGeneratorConstants, "AVATAR_KEYS", ()), getattr(DeepDreamGeneratorConstants, "COVER_KEYS", ()))
