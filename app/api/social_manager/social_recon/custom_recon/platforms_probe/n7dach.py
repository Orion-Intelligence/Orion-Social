...
from api.social_manager.social_recon.constants.platform_constants import N7dachConstants

constants = N7dachConstants


def probe_url(username: str) -> str:
    return N7dachConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(N7dachConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(N7dachConstants, "AVATAR_KEYS", ()), getattr(N7dachConstants, "COVER_KEYS", ()))
