...
from api.social_manager.social_recon.constants.platform_constants import SparkConstants

constants = SparkConstants


def probe_url(username: str) -> str:
    return SparkConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SparkConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SparkConstants, "AVATAR_KEYS", ()), getattr(SparkConstants, "COVER_KEYS", ()))
