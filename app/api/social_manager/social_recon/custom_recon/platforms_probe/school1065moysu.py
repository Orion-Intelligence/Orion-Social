...
from api.social_manager.social_recon.constants.platform_constants import School1065MoySuConstants

constants = School1065MoySuConstants


def probe_url(username: str) -> str:
    return School1065MoySuConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(School1065MoySuConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(School1065MoySuConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(School1065MoySuConstants, "AVATAR_KEYS", ()), getattr(School1065MoySuConstants, "COVER_KEYS", ()))
