...
from api.social_manager.social_recon.constants.platform_constants import SsbwikiComConstants

constants = SsbwikiComConstants


def probe_url(username: str) -> str:
    return SsbwikiComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SsbwikiComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SsbwikiComConstants, "AVATAR_KEYS", ()), getattr(SsbwikiComConstants, "COVER_KEYS", ()))
