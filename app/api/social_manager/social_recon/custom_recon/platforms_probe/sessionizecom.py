...
from api.social_manager.social_recon.constants.platform_constants import SessionizeComConstants

constants = SessionizeComConstants


def probe_url(username: str) -> str:
    return SessionizeComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SessionizeComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SessionizeComConstants, "AVATAR_KEYS", ()), getattr(SessionizeComConstants, "COVER_KEYS", ()))
