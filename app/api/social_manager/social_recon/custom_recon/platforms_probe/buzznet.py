...
from api.social_manager.social_recon.constants.platform_constants import BuzznetConstants

constants = BuzznetConstants


def probe_url(username: str) -> str:
    return BuzznetConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(BuzznetConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(BuzznetConstants, "AVATAR_KEYS", ()), getattr(BuzznetConstants, "COVER_KEYS", ()))
