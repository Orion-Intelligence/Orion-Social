...
from api.social_manager.social_recon.constants.platform_constants import DestructoidConstants

constants = DestructoidConstants


def probe_url(username: str) -> str:
    return DestructoidConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DestructoidConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DestructoidConstants, "AVATAR_KEYS", ()), getattr(DestructoidConstants, "COVER_KEYS", ()))
