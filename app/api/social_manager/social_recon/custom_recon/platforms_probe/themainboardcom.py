...
from api.social_manager.social_recon.constants.platform_constants import TheMainboardComConstants

constants = TheMainboardComConstants


def probe_url(username: str) -> str:
    return TheMainboardComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TheMainboardComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TheMainboardComConstants, "AVATAR_KEYS", ()), getattr(TheMainboardComConstants, "COVER_KEYS", ()))
