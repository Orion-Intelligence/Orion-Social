...
def probe_url(username: str) -> str:
    return CreativeMarketConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CreativeMarketConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CreativeMarketConstants, "AVATAR_KEYS", ()), getattr(CreativeMarketConstants, "COVER_KEYS", ()))
