...
from api.social_manager.social_recon.constants.platform_constants import ReligiousForumsConstants

constants = ReligiousForumsConstants


def probe_url(username: str) -> str:
    return ReligiousForumsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ReligiousForumsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ReligiousForumsConstants, "AVATAR_KEYS", ()), getattr(ReligiousForumsConstants, "COVER_KEYS", ()))
