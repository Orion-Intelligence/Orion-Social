...
from api.social_manager.social_recon.constants.platform_constants import FacultyOfMedicineConstants

constants = FacultyOfMedicineConstants


def probe_url(username: str) -> str:
    return FacultyOfMedicineConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FacultyOfMedicineConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FacultyOfMedicineConstants, "AVATAR_KEYS", ()), getattr(FacultyOfMedicineConstants, "COVER_KEYS", ()))
