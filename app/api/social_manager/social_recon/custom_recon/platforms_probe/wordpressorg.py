...
def probe_url(username: str) -> str:
    return WordPressOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WordPressOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WordPressOrgConstants, "AVATAR_KEYS", ()), getattr(WordPressOrgConstants, "COVER_KEYS", ()))
