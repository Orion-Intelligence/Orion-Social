...
def probe_url(username: str) -> str:
    return VintageMustangComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(VintageMustangComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(VintageMustangComConstants, "AVATAR_KEYS", ()), getattr(VintageMustangComConstants, "COVER_KEYS", ()))
