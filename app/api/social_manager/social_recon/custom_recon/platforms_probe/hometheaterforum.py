...
def probe_url(username: str) -> str:
    return HometheaterforumConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(HometheaterforumConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(HometheaterforumConstants, "AVATAR_KEYS", ()), getattr(HometheaterforumConstants, "COVER_KEYS", ()))
