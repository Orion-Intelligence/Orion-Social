...
def probe_url(username: str) -> str:
    return MineplexComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MineplexComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MineplexComConstants, "AVATAR_KEYS", ()), getattr(MineplexComConstants, "COVER_KEYS", ()))
