...
from api.social_manager.social_recon.constants.platform_constants import CastingcallclubConstants

constants = CastingcallclubConstants


def probe_url(username: str) -> str:
    return CastingcallclubConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CastingcallclubConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CastingcallclubConstants, "AVATAR_KEYS", ()), getattr(CastingcallclubConstants, "COVER_KEYS", ()))
