...
from api.social_manager.social_recon.constants.platform_constants import FlarumEsConstants

constants = FlarumEsConstants


def probe_url(username: str) -> str:
    return FlarumEsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FlarumEsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FlarumEsConstants, "AVATAR_KEYS", ()), getattr(FlarumEsConstants, "COVER_KEYS", ()))
