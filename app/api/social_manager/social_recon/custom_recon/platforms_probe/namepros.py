...
def probe_url(username: str) -> str:
    return NameprosConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(NameprosConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(NameprosConstants, "AVATAR_KEYS", ()), getattr(NameprosConstants, "COVER_KEYS", ()))
