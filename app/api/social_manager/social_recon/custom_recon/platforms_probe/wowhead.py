...
def probe_url(username: str) -> str:
    return WowheadConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WowheadConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WowheadConstants, "AVATAR_KEYS", ()), getattr(WowheadConstants, "COVER_KEYS", ()))
