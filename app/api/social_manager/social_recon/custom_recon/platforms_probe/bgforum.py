...
from api.social_manager.social_recon.constants.platform_constants import BgforumConstants

constants = BgforumConstants


def probe_url(username: str) -> str:
    return BgforumConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(BgforumConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(BgforumConstants, "AVATAR_KEYS", ()), getattr(BgforumConstants, "COVER_KEYS", ()))
