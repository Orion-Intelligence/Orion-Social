...
def probe_url(username: str) -> str:
    return TvGamesConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TvGamesConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TvGamesConstants, "AVATAR_KEYS", ()), getattr(TvGamesConstants, "COVER_KEYS", ()))
