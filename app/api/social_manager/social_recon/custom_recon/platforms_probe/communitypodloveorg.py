...
from api.social_manager.social_recon.constants.platform_constants import CommunityPodloveOrgConstants

constants = CommunityPodloveOrgConstants


def probe_url(username: str) -> str:
    return CommunityPodloveOrgConstants.API_URL.format(username=username)
...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    if not isinstance(user, dict) or not user.get("username"):
        return VerdictConstants.UNKNOWN, {}
    base = CommunityPodloveOrgConstants.PROFILE_URL.split("/u/")[0]
