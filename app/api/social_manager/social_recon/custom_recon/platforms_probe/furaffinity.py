...
def probe_url(username: str) -> str:
    return FurAffinityConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FurAffinityConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FurAffinityConstants, "AVATAR_KEYS", ()), getattr(FurAffinityConstants, "COVER_KEYS", ()))
