...
from api.social_manager.social_recon.constants.platform_constants import SplatoonwikiOrgConstants

constants = SplatoonwikiOrgConstants


def probe_url(username: str) -> str:
    return SplatoonwikiOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SplatoonwikiOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SplatoonwikiOrgConstants, "AVATAR_KEYS", ()), getattr(SplatoonwikiOrgConstants, "COVER_KEYS", ()))
