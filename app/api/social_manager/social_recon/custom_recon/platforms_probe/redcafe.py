...
def probe_url(username: str) -> str:
    return RedcafeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(RedcafeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(RedcafeConstants, "AVATAR_KEYS", ()), getattr(RedcafeConstants, "COVER_KEYS", ()))
