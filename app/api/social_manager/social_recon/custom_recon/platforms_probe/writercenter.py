...
from api.social_manager.social_recon.constants.platform_constants import WritercenterConstants

constants = WritercenterConstants


def probe_url(username: str) -> str:
    return WritercenterConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WritercenterConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WritercenterConstants, "AVATAR_KEYS", ()), getattr(WritercenterConstants, "COVER_KEYS", ()))
