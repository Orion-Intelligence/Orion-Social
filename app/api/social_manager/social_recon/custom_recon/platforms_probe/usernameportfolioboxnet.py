...
from api.social_manager.social_recon.constants.platform_constants import UsernamePortfolioboxNetConstants

constants = UsernamePortfolioboxNetConstants


def probe_url(username: str) -> str:
    return UsernamePortfolioboxNetConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(UsernamePortfolioboxNetConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(UsernamePortfolioboxNetConstants, "AVATAR_KEYS", ()), getattr(UsernamePortfolioboxNetConstants, "COVER_KEYS", ()))
...
HOSTS = ("portfoliobox.net",)
SUBDOMAIN = ("portfoliobox.net", "profile")
ROUTES = ()
