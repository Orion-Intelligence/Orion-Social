...
def probe_url(username: str) -> str:
    return UstreamConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(UstreamConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(UstreamConstants, "AVATAR_KEYS", ()), getattr(UstreamConstants, "COVER_KEYS", ()))
