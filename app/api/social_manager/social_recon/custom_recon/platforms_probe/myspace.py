...
def probe_url(username: str) -> str:
    return MyspaceConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MyspaceConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MyspaceConstants, "AVATAR_KEYS", ()), getattr(MyspaceConstants, "COVER_KEYS", ()))
