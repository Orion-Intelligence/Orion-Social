...
def probe_url(username: str) -> str:
    return StihiRuConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(StihiRuConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(StihiRuConstants, "AVATAR_KEYS", ()), getattr(StihiRuConstants, "COVER_KEYS", ()))
