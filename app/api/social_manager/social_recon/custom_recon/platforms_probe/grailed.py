...
from api.social_manager.social_recon.constants.platform_constants import GrailedConstants

constants = GrailedConstants


def probe_url(username: str) -> str:
    return GrailedConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(GrailedConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(GrailedConstants, "AVATAR_KEYS", ()), getattr(GrailedConstants, "COVER_KEYS", ()))
