...
from api.social_manager.social_recon.constants.platform_constants import HudsonRockConstants

constants = HudsonRockConstants


def probe_url(username: str) -> str:
    return HudsonRockConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(HudsonRockConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(HudsonRockConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(HudsonRockConstants, "AVATAR_KEYS", ()), getattr(HudsonRockConstants, "COVER_KEYS", ()))
...
ROUTES = (
    (r"api/json/v2/osint\-tools/search\-by\-username/?\?(?:.*&)?username=(?P<id>[^&]+)(?:&.*)?", "profile"),
)
