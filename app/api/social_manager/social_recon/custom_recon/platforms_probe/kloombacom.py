...
from api.social_manager.social_recon.constants.platform_constants import KloombaComConstants

constants = KloombaComConstants


def probe_url(username: str) -> str:
    return KloombaComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(KloombaComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(KloombaComConstants, "AVATAR_KEYS", ()), getattr(KloombaComConstants, "COVER_KEYS", ()))
