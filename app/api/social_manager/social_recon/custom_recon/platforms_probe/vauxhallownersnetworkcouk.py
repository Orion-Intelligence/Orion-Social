...
def probe_url(username: str) -> str:
    return VauxhallownersnetworkCoUkConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(VauxhallownersnetworkCoUkConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(VauxhallownersnetworkCoUkConstants, "AVATAR_KEYS", ()), getattr(VauxhallownersnetworkCoUkConstants, "COVER_KEYS", ()))
