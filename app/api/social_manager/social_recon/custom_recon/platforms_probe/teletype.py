...
def probe_url(username: str) -> str:
    return TeletypeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TeletypeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TeletypeConstants, "AVATAR_KEYS", ()), getattr(TeletypeConstants, "COVER_KEYS", ()))
