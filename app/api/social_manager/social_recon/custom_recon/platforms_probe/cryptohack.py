...
def probe_url(username: str) -> str:
    return CryptoHackConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CryptoHackConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CryptoHackConstants, "AVATAR_KEYS", ()), getattr(CryptoHackConstants, "COVER_KEYS", ()))
