...
from api.social_manager.social_recon.constants.platform_constants import PolymarketConstants

constants = PolymarketConstants


def probe_url(username: str) -> str:
    return PolymarketConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PolymarketConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PolymarketConstants, "AVATAR_KEYS", ()), getattr(PolymarketConstants, "COVER_KEYS", ()))
