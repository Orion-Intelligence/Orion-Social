...
def probe_url(username: str) -> str:
    return YouPicConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(YouPicConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(YouPicConstants, "AVATAR_KEYS", ()), getattr(YouPicConstants, "COVER_KEYS", ()))
