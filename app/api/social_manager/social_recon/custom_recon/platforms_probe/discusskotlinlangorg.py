...
from api.social_manager.social_recon.constants.platform_constants import DiscussKotlinlangOrgConstants

constants = DiscussKotlinlangOrgConstants


def probe_url(username: str) -> str:
    return DiscussKotlinlangOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DiscussKotlinlangOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DiscussKotlinlangOrgConstants, "AVATAR_KEYS", ()), getattr(DiscussKotlinlangOrgConstants, "COVER_KEYS", ()))
