...
from api.social_manager.social_recon.constants.platform_constants import MastodonCloudConstants

constants = MastodonCloudConstants


def probe_url(username: str) -> str:
    return MastodonCloudConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MastodonCloudConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MastodonCloudConstants, "AVATAR_KEYS", ()), getattr(MastodonCloudConstants, "COVER_KEYS", ()))
