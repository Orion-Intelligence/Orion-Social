...
from api.social_manager.social_recon.constants.platform_constants import Sony1273dnRuConstants

constants = Sony1273dnRuConstants


def probe_url(username: str) -> str:
    return Sony1273dnRuConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(Sony1273dnRuConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(Sony1273dnRuConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(Sony1273dnRuConstants, "AVATAR_KEYS", ()), getattr(Sony1273dnRuConstants, "COVER_KEYS", ()))
