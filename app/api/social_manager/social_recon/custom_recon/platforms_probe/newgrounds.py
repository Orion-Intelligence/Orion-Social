...
from api.social_manager.social_recon.constants.platform_constants import NewgroundsConstants

constants = NewgroundsConstants


def probe_url(username: str) -> str:
    return NewgroundsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(NewgroundsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(NewgroundsConstants, "AVATAR_KEYS", ()), getattr(NewgroundsConstants, "COVER_KEYS", ()))
...
SUBDOMAIN = ("newgrounds.com", "profile")
ROUTES = ()
