...
from api.social_manager.social_recon.constants.platform_constants import UncUaConstants

constants = UncUaConstants


def probe_url(username: str) -> str:
    return UncUaConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(UncUaConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(UncUaConstants, "AVATAR_KEYS", ()), getattr(UncUaConstants, "COVER_KEYS", ()))
