...
def probe_url(username: str) -> str:
    return KofiConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(KofiConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(KofiConstants, "AVATAR_KEYS", ()), getattr(KofiConstants, "COVER_KEYS", ()))
