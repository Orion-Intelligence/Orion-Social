...
from api.social_manager.social_recon.constants.platform_constants import CalendlyConstants

constants = CalendlyConstants


def probe_url(username: str) -> str:
    return CalendlyConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CalendlyConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CalendlyConstants, "AVATAR_KEYS", ()), getattr(CalendlyConstants, "COVER_KEYS", ()))
