...
from api.social_manager.social_recon.constants.platform_constants import MyMailRuOKConstants

constants = MyMailRuOKConstants


def probe_url(username: str) -> str:
    return MyMailRuOKConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MyMailRuOKConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MyMailRuOKConstants, "AVATAR_KEYS", ()), getattr(MyMailRuOKConstants, "COVER_KEYS", ()))
