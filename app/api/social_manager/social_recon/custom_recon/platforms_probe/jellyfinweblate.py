...
from api.social_manager.social_recon.constants.platform_constants import JellyfinWeblateConstants

constants = JellyfinWeblateConstants


def probe_url(username: str) -> str:
    return JellyfinWeblateConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(JellyfinWeblateConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(JellyfinWeblateConstants, "AVATAR_KEYS", ()), getattr(JellyfinWeblateConstants, "COVER_KEYS", ()))
