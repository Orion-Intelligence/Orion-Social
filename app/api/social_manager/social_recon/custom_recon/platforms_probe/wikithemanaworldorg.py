...
from api.social_manager.social_recon.constants.platform_constants import WikiThemanaworldOrgConstants

constants = WikiThemanaworldOrgConstants


def probe_url(username: str) -> str:
    return WikiThemanaworldOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WikiThemanaworldOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WikiThemanaworldOrgConstants, "AVATAR_KEYS", ()), getattr(WikiThemanaworldOrgConstants, "COVER_KEYS", ()))
