...
from api.social_manager.social_recon.constants.platform_constants import AknigaConstants

constants = AknigaConstants


def probe_url(username: str) -> str:
    return AknigaConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(AknigaConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(AknigaConstants, "AVATAR_KEYS", ()), getattr(AknigaConstants, "COVER_KEYS", ()))
