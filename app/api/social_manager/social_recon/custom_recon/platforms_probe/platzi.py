...
def probe_url(username: str) -> str:
    return PlatziConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PlatziConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PlatziConstants, "AVATAR_KEYS", ()), getattr(PlatziConstants, "COVER_KEYS", ()))
