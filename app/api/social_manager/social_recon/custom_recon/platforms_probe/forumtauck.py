...
from api.social_manager.social_recon.constants.platform_constants import ForumTauckConstants

constants = ForumTauckConstants


def probe_url(username: str) -> str:
    return ForumTauckConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForumTauckConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumTauckConstants, "AVATAR_KEYS", ()), getattr(ForumTauckConstants, "COVER_KEYS", ()))
