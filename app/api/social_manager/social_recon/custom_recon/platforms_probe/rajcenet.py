...
from api.social_manager.social_recon.constants.platform_constants import RajceNetConstants

constants = RajceNetConstants


def probe_url(username: str) -> str:
    return RajceNetConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(RajceNetConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(RajceNetConstants, "AVATAR_KEYS", ()), getattr(RajceNetConstants, "COVER_KEYS", ()))
...
HOSTS = ('rajce.idnes.cz',)
SUBDOMAIN = ('rajce.idnes.cz', "profile")
ROUTES = (
    ("(?P<id>[^/]+)", "profile"),
)
