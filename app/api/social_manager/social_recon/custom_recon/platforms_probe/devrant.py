...
def probe_url(username: str) -> str:
    return DevRantConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DevRantConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DevRantConstants, "AVATAR_KEYS", ()), getattr(DevRantConstants, "COVER_KEYS", ()))
