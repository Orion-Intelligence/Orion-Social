...
from api.social_manager.social_recon.constants.platform_constants import PurephotoConstants

constants = PurephotoConstants


def probe_url(username: str) -> str:
    return PurephotoConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PurephotoConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PurephotoConstants, "AVATAR_KEYS", ()), getattr(PurephotoConstants, "COVER_KEYS", ()))
