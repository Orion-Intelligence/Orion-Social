...
def probe_url(username: str) -> str:
    return WolpyConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WolpyConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WolpyConstants, "AVATAR_KEYS", ()), getattr(WolpyConstants, "COVER_KEYS", ()))
