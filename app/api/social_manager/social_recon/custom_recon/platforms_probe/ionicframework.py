...
from api.social_manager.social_recon.constants.platform_constants import IonicFrameworkConstants

constants = IonicFrameworkConstants


def probe_url(username: str) -> str:
    return IonicFrameworkConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(IonicFrameworkConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(IonicFrameworkConstants, "AVATAR_KEYS", ()), getattr(IonicFrameworkConstants, "COVER_KEYS", ()))
