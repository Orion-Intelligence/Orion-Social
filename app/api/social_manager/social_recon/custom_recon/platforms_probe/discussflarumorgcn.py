...
from api.social_manager.social_recon.constants.platform_constants import DiscussFlarumOrgCnConstants

constants = DiscussFlarumOrgCnConstants


def probe_url(username: str) -> str:
    return DiscussFlarumOrgCnConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DiscussFlarumOrgCnConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DiscussFlarumOrgCnConstants, "AVATAR_KEYS", ()), getattr(DiscussFlarumOrgCnConstants, "COVER_KEYS", ()))
