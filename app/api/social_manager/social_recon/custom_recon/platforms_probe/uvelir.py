...
def probe_url(username: str) -> str:
    return UvelirConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(UvelirConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(UvelirConstants, "AVATAR_KEYS", ()), getattr(UvelirConstants, "COVER_KEYS", ()))
