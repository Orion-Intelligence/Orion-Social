...
from api.social_manager.social_recon.constants.platform_constants import AllhockeyConstants

constants = AllhockeyConstants


def probe_url(username: str) -> str:
    return AllhockeyConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(AllhockeyConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(AllhockeyConstants, "AVATAR_KEYS", ()), getattr(AllhockeyConstants, "COVER_KEYS", ()))
