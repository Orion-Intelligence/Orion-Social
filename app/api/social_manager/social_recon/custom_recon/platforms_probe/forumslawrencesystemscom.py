...
from api.social_manager.social_recon.constants.platform_constants import ForumsLawrencesystemsComConstants

constants = ForumsLawrencesystemsComConstants


def probe_url(username: str) -> str:
    return ForumsLawrencesystemsComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForumsLawrencesystemsComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumsLawrencesystemsComConstants, "AVATAR_KEYS", ()), getattr(ForumsLawrencesystemsComConstants, "COVER_KEYS", ()))
