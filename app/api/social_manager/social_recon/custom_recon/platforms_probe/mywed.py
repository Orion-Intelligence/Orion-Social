...
from api.social_manager.social_recon.constants.platform_constants import MywedConstants

constants = MywedConstants


def probe_url(username: str) -> str:
    return MywedConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MywedConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MywedConstants, "AVATAR_KEYS", ()), getattr(MywedConstants, "COVER_KEYS", ()))
...
ROUTES = (
    (r"ru/photographer/(?P<id>[^/]+)(?:/.*)?", "profile"),
)
