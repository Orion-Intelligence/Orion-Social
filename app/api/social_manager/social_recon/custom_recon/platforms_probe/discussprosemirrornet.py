...
from api.social_manager.social_recon.constants.platform_constants import DiscussProsemirrorNetConstants

constants = DiscussProsemirrorNetConstants


def probe_url(username: str) -> str:
    return DiscussProsemirrorNetConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DiscussProsemirrorNetConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DiscussProsemirrorNetConstants, "AVATAR_KEYS", ()), getattr(DiscussProsemirrorNetConstants, "COVER_KEYS", ()))
