...
from api.social_manager.social_recon.constants.platform_constants import ChaturbatorSuConstants

constants = ChaturbatorSuConstants


def probe_url(username: str) -> str:
    return ChaturbatorSuConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ChaturbatorSuConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ChaturbatorSuConstants, "AVATAR_KEYS", ()), getattr(ChaturbatorSuConstants, "COVER_KEYS", ()))
