...
from api.social_manager.social_recon.constants.platform_constants import SmugmugConstants

constants = SmugmugConstants


def probe_url(username: str) -> str:
    return SmugmugConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SmugmugConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SmugmugConstants, "AVATAR_KEYS", ()), getattr(SmugmugConstants, "COVER_KEYS", ()))
