...
from api.social_manager.social_recon.constants.platform_constants import CryptomatorForumConstants

constants = CryptomatorForumConstants


def probe_url(username: str) -> str:
    return CryptomatorForumConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CryptomatorForumConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CryptomatorForumConstants, "AVATAR_KEYS", ()), getattr(CryptomatorForumConstants, "COVER_KEYS", ()))
