...
def probe_url(username: str) -> str:
    return BigsoccerConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(BigsoccerConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(BigsoccerConstants, "AVATAR_KEYS", ()), getattr(BigsoccerConstants, "COVER_KEYS", ()))
