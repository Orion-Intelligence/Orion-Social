...
from api.social_manager.social_recon.constants.platform_constants import BarkVideoConstants

constants = BarkVideoConstants


def probe_url(username: str) -> str:
    return BarkVideoConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(BarkVideoConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(BarkVideoConstants, "AVATAR_KEYS", ()), getattr(BarkVideoConstants, "COVER_KEYS", ()))
