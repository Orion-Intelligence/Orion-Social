...
def probe_url(username: str) -> str:
    return OpenSourceConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(OpenSourceConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(OpenSourceConstants, "AVATAR_KEYS", ()), getattr(OpenSourceConstants, "COVER_KEYS", ()))
