...
from api.social_manager.social_recon.constants.platform_constants import CommunityBlynkCcConstants

constants = CommunityBlynkCcConstants


def probe_url(username: str) -> str:
    return CommunityBlynkCcConstants.API_URL.format(username=username)
...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    if not isinstance(user, dict) or not user.get("username"):
        return VerdictConstants.UNKNOWN, {}
    base = CommunityBlynkCcConstants.PROFILE_URL.split("/u/")[0]
