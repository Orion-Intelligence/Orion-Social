...
def probe_url(username: str) -> str:
    return ExophaseConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ExophaseConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ExophaseConstants, "AVATAR_KEYS", ()), getattr(ExophaseConstants, "COVER_KEYS", ()))
