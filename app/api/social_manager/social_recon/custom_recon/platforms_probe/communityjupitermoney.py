...
from api.social_manager.social_recon.constants.platform_constants import CommunityJupiterMoneyConstants

constants = CommunityJupiterMoneyConstants


def probe_url(username: str) -> str:
    return CommunityJupiterMoneyConstants.API_URL.format(username=username)
...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    if not isinstance(user, dict) or not user.get("username"):
        return VerdictConstants.UNKNOWN, {}
    base = CommunityJupiterMoneyConstants.PROFILE_URL.split("/u/")[0]
