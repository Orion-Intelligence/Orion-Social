...
def probe_url(username: str) -> str:
    return DMOJConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DMOJConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DMOJConstants, "AVATAR_KEYS", ()), getattr(DMOJConstants, "COVER_KEYS", ()))
