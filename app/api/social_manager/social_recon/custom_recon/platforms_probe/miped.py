...
from api.social_manager.social_recon.constants.platform_constants import MipedConstants

constants = MipedConstants


def probe_url(username: str) -> str:
    return MipedConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MipedConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MipedConstants, "AVATAR_KEYS", ()), getattr(MipedConstants, "COVER_KEYS", ()))
