...
def probe_url(username: str) -> str:
    return WindowsforumConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WindowsforumConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WindowsforumConstants, "AVATAR_KEYS", ()), getattr(WindowsforumConstants, "COVER_KEYS", ()))
