...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    name = parse.clean(parse.text(meta.get("og:title")))
    if name.endswith(PexelsConstants.NAME_SUFFIX):
        name = name[: -len(PexelsConstants.NAME_SUFFIX)].strip()
    info = {
        "display_name": name,
        "username": parse.text(meta.get("og:url")).rstrip("/").split("/")[-1].lstrip("@"),
    }
    followers = re.search(PexelsConstants.FOLLOWERS, body)
    if followers:
        info["followers"] = followers.group(1)
    photos = re.search(PexelsConstants.PHOTOS, body)
    if photos:
        info["photos"] = photos.group(1)
