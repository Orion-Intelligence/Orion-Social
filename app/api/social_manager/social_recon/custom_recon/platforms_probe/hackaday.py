...
from api.social_manager.social_recon.constants.platform_constants import HackadayConstants

constants = HackadayConstants


def probe_url(username: str) -> str:
    return HackadayConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(HackadayConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(HackadayConstants, "AVATAR_KEYS", ()), getattr(HackadayConstants, "COVER_KEYS", ()))
