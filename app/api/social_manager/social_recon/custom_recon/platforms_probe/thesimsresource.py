...
def probe_url(username: str) -> str:
    return TheSimsResourceConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TheSimsResourceConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TheSimsResourceConstants, "AVATAR_KEYS", ()), getattr(TheSimsResourceConstants, "COVER_KEYS", ()))
