...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in GumroadConstants.GENERIC:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, GumroadConstants.AVATAR_KEYS, GumroadConstants.COVER_KEYS)
    name = parse.clean(info.get("display_name"))
    if name.startswith(GumroadConstants.NAME_PREFIX):
        name = name[len(GumroadConstants.NAME_PREFIX):].strip()
    info["display_name"] = name
    if heading and heading.casefold() not in GumroadConstants.GENERIC:
        info["username"] = parse.clean(heading)
