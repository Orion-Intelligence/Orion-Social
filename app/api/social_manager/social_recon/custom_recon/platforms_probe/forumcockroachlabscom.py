...
from api.social_manager.social_recon.constants.platform_constants import ForumCockroachlabsComConstants

constants = ForumCockroachlabsComConstants


def probe_url(username: str) -> str:
    return ForumCockroachlabsComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForumCockroachlabsComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumCockroachlabsComConstants, "AVATAR_KEYS", ()), getattr(ForumCockroachlabsComConstants, "COVER_KEYS", ()))
