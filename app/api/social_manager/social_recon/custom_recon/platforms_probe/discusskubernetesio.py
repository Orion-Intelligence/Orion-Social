...
from api.social_manager.social_recon.constants.platform_constants import DiscussKubernetesIoConstants

constants = DiscussKubernetesIoConstants


def probe_url(username: str) -> str:
    return DiscussKubernetesIoConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DiscussKubernetesIoConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DiscussKubernetesIoConstants, "AVATAR_KEYS", ()), getattr(DiscussKubernetesIoConstants, "COVER_KEYS", ()))
