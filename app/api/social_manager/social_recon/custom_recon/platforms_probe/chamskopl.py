...
from api.social_manager.social_recon.constants.platform_constants import ChamskoPlConstants

constants = ChamskoPlConstants


def probe_url(username: str) -> str:
    return ChamskoPlConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ChamskoPlConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ChamskoPlConstants, "AVATAR_KEYS", ()), getattr(ChamskoPlConstants, "COVER_KEYS", ()))
