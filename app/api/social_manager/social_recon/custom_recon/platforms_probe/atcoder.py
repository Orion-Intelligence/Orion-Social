...
def probe_url(username: str) -> str:
    return AtcoderConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(AtcoderConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(AtcoderConstants, "AVATAR_KEYS", ()), getattr(AtcoderConstants, "COVER_KEYS", ()))
