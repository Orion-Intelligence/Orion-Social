...
from api.social_manager.social_recon.constants.platform_constants import ContentlyConstants

constants = ContentlyConstants


def probe_url(username: str) -> str:
    return ContentlyConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ContentlyConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ContentlyConstants, "AVATAR_KEYS", ()), getattr(ContentlyConstants, "COVER_KEYS", ()))
...
HOSTS = ("contently.com",)
SUBDOMAIN = ("contently.com", "profile")
ROUTES = ()
