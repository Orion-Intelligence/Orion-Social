...
def probe_url(username: str) -> str:
    return MaxConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MaxConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MaxConstants, "AVATAR_KEYS", ()), getattr(MaxConstants, "COVER_KEYS", ()))
