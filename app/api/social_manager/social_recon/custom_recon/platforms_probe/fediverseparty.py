...
def probe_url(username: str) -> str:
    return FediversePartyConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FediversePartyConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FediversePartyConstants, "AVATAR_KEYS", ()), getattr(FediversePartyConstants, "COVER_KEYS", ()))
