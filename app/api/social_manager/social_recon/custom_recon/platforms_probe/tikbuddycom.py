...
from api.social_manager.social_recon.constants.platform_constants import TikbuddyComConstants

constants = TikbuddyComConstants


def probe_url(username: str) -> str:
    return TikbuddyComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TikbuddyComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TikbuddyComConstants, "AVATAR_KEYS", ()), getattr(TikbuddyComConstants, "COVER_KEYS", ()))
