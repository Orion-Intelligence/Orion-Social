...
def probe_url(username: str) -> str:
    return DcinsideConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DcinsideConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DcinsideConstants, "AVATAR_KEYS", ()), getattr(DcinsideConstants, "COVER_KEYS", ()))
