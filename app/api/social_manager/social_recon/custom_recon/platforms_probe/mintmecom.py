...
from api.social_manager.social_recon.constants.platform_constants import MintmeComConstants

constants = MintmeComConstants


def probe_url(username: str) -> str:
    return MintmeComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MintmeComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MintmeComConstants, "AVATAR_KEYS", ()), getattr(MintmeComConstants, "COVER_KEYS", ()))
