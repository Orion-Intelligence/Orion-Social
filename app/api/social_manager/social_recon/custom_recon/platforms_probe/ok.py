...
from api.social_manager.social_recon.constants.platform_constants import OKConstants

constants = OKConstants


def probe_url(username: str) -> str:
    return OKConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(OKConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(OKConstants, "AVATAR_KEYS", ()), getattr(OKConstants, "COVER_KEYS", ()))
