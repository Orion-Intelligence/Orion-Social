...
def probe_url(username: str) -> str:
    return CoderwallConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CoderwallConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CoderwallConstants, "AVATAR_KEYS", ()), getattr(CoderwallConstants, "COVER_KEYS", ()))
