...
from api.social_manager.social_recon.constants.platform_constants import GlobalvoicesConstants

constants = GlobalvoicesConstants


def probe_url(username: str) -> str:
    return GlobalvoicesConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(GlobalvoicesConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(GlobalvoicesConstants, "AVATAR_KEYS", ()), getattr(GlobalvoicesConstants, "COVER_KEYS", ()))
