...
from api.social_manager.social_recon.constants.platform_constants import HolopinConstants

constants = HolopinConstants


def probe_url(username: str) -> str:
    return HolopinConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(HolopinConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(HolopinConstants, "AVATAR_KEYS", ()), getattr(HolopinConstants, "COVER_KEYS", ()))
