...
def probe_url(username: str) -> str:
    return EthereumMagiciansConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(EthereumMagiciansConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(EthereumMagiciansConstants, "AVATAR_KEYS", ()), getattr(EthereumMagiciansConstants, "COVER_KEYS", ()))
