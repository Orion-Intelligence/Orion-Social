...
from api.social_manager.social_recon.constants.platform_constants import DangerousthingsComConstants

constants = DangerousthingsComConstants


def probe_url(username: str) -> str:
    return DangerousthingsComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DangerousthingsComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DangerousthingsComConstants, "AVATAR_KEYS", ()), getattr(DangerousthingsComConstants, "COVER_KEYS", ()))
