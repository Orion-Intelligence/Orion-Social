...
from api.social_manager.social_recon.constants.platform_constants import CavesConstants

constants = CavesConstants


def probe_url(username: str) -> str:
    return CavesConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CavesConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CavesConstants, "AVATAR_KEYS", ()), getattr(CavesConstants, "COVER_KEYS", ()))
