...
def probe_url(username: str) -> str:
    return RusspussRuConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(RusspussRuConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(RusspussRuConstants, "AVATAR_KEYS", ()), getattr(RusspussRuConstants, "COVER_KEYS", ()))
