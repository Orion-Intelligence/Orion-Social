...
def probe_url(username: str) -> str:
    return MobypictureConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MobypictureConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MobypictureConstants, "AVATAR_KEYS", ()), getattr(MobypictureConstants, "COVER_KEYS", ()))
