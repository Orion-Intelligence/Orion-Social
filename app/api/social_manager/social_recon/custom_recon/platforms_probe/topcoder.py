...
def probe_url(username: str) -> str:
    return TopcoderConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TopcoderConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TopcoderConstants, "AVATAR_KEYS", ()), getattr(TopcoderConstants, "COVER_KEYS", ()))
