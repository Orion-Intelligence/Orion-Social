...
def probe_url(username: str) -> str:
    return CoubConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CoubConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CoubConstants, "AVATAR_KEYS", ()), getattr(CoubConstants, "COVER_KEYS", ()))
