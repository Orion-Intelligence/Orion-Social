...
def probe_url(username: str) -> str:
    return WarframeMarketConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WarframeMarketConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WarframeMarketConstants, "AVATAR_KEYS", ()), getattr(WarframeMarketConstants, "COVER_KEYS", ()))
