...
def probe_url(username: str) -> str:
    return FreepikConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FreepikConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FreepikConstants, "AVATAR_KEYS", ()), getattr(FreepikConstants, "COVER_KEYS", ()))
