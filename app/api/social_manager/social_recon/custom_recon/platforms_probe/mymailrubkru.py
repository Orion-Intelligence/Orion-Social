...
from api.social_manager.social_recon.constants.platform_constants import MyMailRuBkRuConstants

constants = MyMailRuBkRuConstants


def probe_url(username: str) -> str:
    return MyMailRuBkRuConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MyMailRuBkRuConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MyMailRuBkRuConstants, "AVATAR_KEYS", ()), getattr(MyMailRuBkRuConstants, "COVER_KEYS", ()))
