...
def probe_url(username: str) -> str:
    return HuntingConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(HuntingConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(HuntingConstants, "AVATAR_KEYS", ()), getattr(HuntingConstants, "COVER_KEYS", ()))
