...
from api.social_manager.social_recon.constants.platform_constants import CommunityOpenconversationalAiConstants

constants = CommunityOpenconversationalAiConstants


def probe_url(username: str) -> str:
    return CommunityOpenconversationalAiConstants.API_URL.format(username=username)
...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    if not isinstance(user, dict) or not user.get("username"):
        return VerdictConstants.UNKNOWN, {}
    base = CommunityOpenconversationalAiConstants.PROFILE_URL.split("/u/")[0]
