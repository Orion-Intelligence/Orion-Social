...
from api.social_manager.social_recon.constants.platform_constants import ListalConstants

constants = ListalConstants


def probe_url(username: str) -> str:
    return ListalConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ListalConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ListalConstants, "AVATAR_KEYS", ()), getattr(ListalConstants, "COVER_KEYS", ()))
