...
def probe_url(username: str) -> str:
    return NeoseekerConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(NeoseekerConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(NeoseekerConstants, "AVATAR_KEYS", ()), getattr(NeoseekerConstants, "COVER_KEYS", ()))
