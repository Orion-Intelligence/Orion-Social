...
def probe_url(username: str) -> str:
    return GovloopConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(GovloopConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(GovloopConstants, "AVATAR_KEYS", ()), getattr(GovloopConstants, "COVER_KEYS", ()))
