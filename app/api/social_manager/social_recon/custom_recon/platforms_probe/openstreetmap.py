...
from api.social_manager.social_recon.constants.platform_constants import OpenStreetMapConstants

constants = OpenStreetMapConstants


def probe_url(username: str) -> str:
    return OpenStreetMapConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(OpenStreetMapConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(OpenStreetMapConstants, "AVATAR_KEYS", ()), getattr(OpenStreetMapConstants, "COVER_KEYS", ()))
