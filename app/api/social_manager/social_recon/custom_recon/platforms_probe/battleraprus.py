...
from api.social_manager.social_recon.constants.platform_constants import BattleraprusConstants

constants = BattleraprusConstants


def probe_url(username: str) -> str:
    return BattleraprusConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(BattleraprusConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(BattleraprusConstants, "AVATAR_KEYS", ()), getattr(BattleraprusConstants, "COVER_KEYS", ()))
...
ROUTES = (
    (r"ru/wiki/%D0%A3%D1%87%D0%B0%D1%81%D1%82%D0%BD%D0%B8%D0%BA:(?P<id>[^/]+)(?:/.*)?", "profile"),
)
