...
from api.social_manager.social_recon.constants.platform_constants import N8wayrunComConstants

constants = N8wayrunComConstants


def probe_url(username: str) -> str:
    return N8wayrunComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(N8wayrunComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(N8wayrunComConstants, "AVATAR_KEYS", ()), getattr(N8wayrunComConstants, "COVER_KEYS", ()))
