...
def probe_url(username: str) -> str:
    return QbnConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(QbnConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(QbnConstants, "AVATAR_KEYS", ()), getattr(QbnConstants, "COVER_KEYS", ()))
