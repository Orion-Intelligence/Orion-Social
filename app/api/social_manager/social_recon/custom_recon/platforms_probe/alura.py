...
from api.social_manager.social_recon.constants.platform_constants import AluraConstants

constants = AluraConstants


def probe_url(username: str) -> str:
    return AluraConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(AluraConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(AluraConstants, "AVATAR_KEYS", ()), getattr(AluraConstants, "COVER_KEYS", ()))
    ...
    if info.get("display_name"):
        info["display_name"] = parse.clean(re.sub(r"^Perfil de\s+", "", info["display_name"]))
