...
def probe_url(username: str) -> str:
    return ZoomirIrConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ZoomirIrConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ZoomirIrConstants, "AVATAR_KEYS", ()), getattr(ZoomirIrConstants, "COVER_KEYS", ()))
