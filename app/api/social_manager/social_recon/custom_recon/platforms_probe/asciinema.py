...
def probe_url(username: str) -> str:
    return AsciinemaConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(AsciinemaConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(AsciinemaConstants, "AVATAR_KEYS", ()), getattr(AsciinemaConstants, "COVER_KEYS", ()))
