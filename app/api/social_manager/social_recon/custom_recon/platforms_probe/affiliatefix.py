...
def probe_url(username: str) -> str:
    return AffiliatefixConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(AffiliatefixConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(AffiliatefixConstants, "AVATAR_KEYS", ()), getattr(AffiliatefixConstants, "COVER_KEYS", ()))
