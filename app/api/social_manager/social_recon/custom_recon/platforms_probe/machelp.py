...
from api.social_manager.social_recon.constants.platform_constants import MacHelpConstants

constants = MacHelpConstants


def probe_url(username: str) -> str:
    return MacHelpConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MacHelpConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MacHelpConstants, "AVATAR_KEYS", ()), getattr(MacHelpConstants, "COVER_KEYS", ()))
