...
from api.social_manager.social_recon.constants.platform_constants import UsernameTildaWsConstants

constants = UsernameTildaWsConstants


def probe_url(username: str) -> str:
    return UsernameTildaWsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(UsernameTildaWsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(UsernameTildaWsConstants, "AVATAR_KEYS", ()), getattr(UsernameTildaWsConstants, "COVER_KEYS", ()))
