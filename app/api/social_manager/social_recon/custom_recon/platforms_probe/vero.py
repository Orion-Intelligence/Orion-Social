...
from api.social_manager.social_recon.constants.platform_constants import VeroConstants

constants = VeroConstants


def probe_url(username: str) -> str:
    return VeroConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(VeroConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(VeroConstants, "AVATAR_KEYS", ()), getattr(VeroConstants, "COVER_KEYS", ()))
