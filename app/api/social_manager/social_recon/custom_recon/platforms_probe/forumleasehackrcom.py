...
from api.social_manager.social_recon.constants.platform_constants import ForumLeasehackrComConstants

constants = ForumLeasehackrComConstants


def probe_url(username: str) -> str:
    return ForumLeasehackrComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForumLeasehackrComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumLeasehackrComConstants, "AVATAR_KEYS", ()), getattr(ForumLeasehackrComConstants, "COVER_KEYS", ()))
