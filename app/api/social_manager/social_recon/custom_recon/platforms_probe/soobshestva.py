...
from api.social_manager.social_recon.constants.platform_constants import SoobshestvaConstants

constants = SoobshestvaConstants


def probe_url(username: str) -> str:
    return SoobshestvaConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SoobshestvaConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SoobshestvaConstants, "AVATAR_KEYS", ()), getattr(SoobshestvaConstants, "COVER_KEYS", ()))
