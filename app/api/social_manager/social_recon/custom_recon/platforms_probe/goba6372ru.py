...
from api.social_manager.social_recon.constants.platform_constants import Goba6372RuConstants

constants = Goba6372RuConstants


def probe_url(username: str) -> str:
    return Goba6372RuConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(Goba6372RuConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(Goba6372RuConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(Goba6372RuConstants, "AVATAR_KEYS", ()), getattr(Goba6372RuConstants, "COVER_KEYS", ()))
