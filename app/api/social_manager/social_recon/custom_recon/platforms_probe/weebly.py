...
from api.social_manager.social_recon.constants.platform_constants import WeeblyConstants

constants = WeeblyConstants


def probe_url(username: str) -> str:
    return WeeblyConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WeeblyConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WeeblyConstants, "AVATAR_KEYS", ()), getattr(WeeblyConstants, "COVER_KEYS", ()))
