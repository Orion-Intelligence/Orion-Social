...
from api.social_manager.social_recon.constants.platform_constants import Nokia6233UcozRuConstants

constants = Nokia6233UcozRuConstants


def probe_url(username: str) -> str:
    return Nokia6233UcozRuConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(Nokia6233UcozRuConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(Nokia6233UcozRuConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(Nokia6233UcozRuConstants, "AVATAR_KEYS", ()), getattr(Nokia6233UcozRuConstants, "COVER_KEYS", ()))
