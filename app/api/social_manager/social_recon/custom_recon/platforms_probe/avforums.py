...
def probe_url(username: str) -> str:
    return AvforumsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(AvforumsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(AvforumsConstants, "AVATAR_KEYS", ()), getattr(AvforumsConstants, "COVER_KEYS", ()))
