...
def probe_url(username: str) -> str:
    return MynicknameComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MynicknameComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MynicknameComConstants, "AVATAR_KEYS", ()), getattr(MynicknameComConstants, "COVER_KEYS", ()))
