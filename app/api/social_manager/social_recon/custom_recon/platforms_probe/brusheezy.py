...
def probe_url(username: str) -> str:
    return BrusheezyConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(BrusheezyConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(BrusheezyConstants, "AVATAR_KEYS", ()), getattr(BrusheezyConstants, "COVER_KEYS", ()))
