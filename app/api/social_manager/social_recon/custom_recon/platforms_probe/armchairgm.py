...
from api.social_manager.social_recon.constants.platform_constants import ArmchairgmConstants

constants = ArmchairgmConstants


def probe_url(username: str) -> str:
    return ArmchairgmConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ArmchairgmConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ArmchairgmConstants, "AVATAR_KEYS", ()), getattr(ArmchairgmConstants, "COVER_KEYS", ()))
