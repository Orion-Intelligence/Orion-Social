...
def probe_url(username: str) -> str:
    return ProzaRuConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ProzaRuConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ProzaRuConstants, "AVATAR_KEYS", ()), getattr(ProzaRuConstants, "COVER_KEYS", ()))
