...
from api.social_manager.social_recon.constants.platform_constants import OmoimotConstants

constants = OmoimotConstants


def probe_url(username: str) -> str:
    return OmoimotConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(OmoimotConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(OmoimotConstants, "AVATAR_KEYS", ()), getattr(OmoimotConstants, "COVER_KEYS", ()))
