...
def probe_url(username: str) -> str:
    return SkyblockConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SkyblockConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SkyblockConstants, "AVATAR_KEYS", ()), getattr(SkyblockConstants, "COVER_KEYS", ()))
