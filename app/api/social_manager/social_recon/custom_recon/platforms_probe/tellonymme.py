...
from api.social_manager.social_recon.constants.platform_constants import TellonymMeConstants

constants = TellonymMeConstants


def probe_url(username: str) -> str:
    return TellonymMeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TellonymMeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TellonymMeConstants, "AVATAR_KEYS", ()), getattr(TellonymMeConstants, "COVER_KEYS", ()))
