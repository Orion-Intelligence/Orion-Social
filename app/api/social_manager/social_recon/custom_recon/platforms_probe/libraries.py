...
from api.social_manager.social_recon.constants.platform_constants import LibrariesConstants

constants = LibrariesConstants


def probe_url(username: str) -> str:
    return LibrariesConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(LibrariesConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(LibrariesConstants, "AVATAR_KEYS", ()), getattr(LibrariesConstants, "COVER_KEYS", ()))
