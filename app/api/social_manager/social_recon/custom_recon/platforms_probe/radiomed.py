...
from api.social_manager.social_recon.constants.platform_constants import RadiomedConstants

constants = RadiomedConstants


def probe_url(username: str) -> str:
    return RadiomedConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(RadiomedConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(RadiomedConstants, "AVATAR_KEYS", ()), getattr(RadiomedConstants, "COVER_KEYS", ()))
