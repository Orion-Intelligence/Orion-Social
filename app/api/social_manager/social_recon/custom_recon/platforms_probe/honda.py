...
from api.social_manager.social_recon.constants.platform_constants import HondaConstants

constants = HondaConstants


def probe_url(username: str) -> str:
    return HondaConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(HondaConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(HondaConstants, "AVATAR_KEYS", ()), getattr(HondaConstants, "COVER_KEYS", ()))
