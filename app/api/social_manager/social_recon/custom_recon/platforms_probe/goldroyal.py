...
def probe_url(username: str) -> str:
    return GoldroyalConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(GoldroyalConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(GoldroyalConstants, "AVATAR_KEYS", ()), getattr(GoldroyalConstants, "COVER_KEYS", ()))
