...
def probe_url(username: str) -> str:
    return WorldofplayersConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WorldofplayersConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WorldofplayersConstants, "AVATAR_KEYS", ()), getattr(WorldofplayersConstants, "COVER_KEYS", ()))
