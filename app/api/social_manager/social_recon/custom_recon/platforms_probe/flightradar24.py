...
from api.social_manager.social_recon.constants.platform_constants import Flightradar24Constants

constants = Flightradar24Constants


def probe_url(username: str) -> str:
    return Flightradar24Constants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(Flightradar24Constants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(Flightradar24Constants, "AVATAR_KEYS", ()), getattr(Flightradar24Constants, "COVER_KEYS", ()))
