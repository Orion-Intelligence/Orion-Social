...
from api.social_manager.social_recon.constants.platform_constants import SchlockConstants

constants = SchlockConstants


def probe_url(username: str) -> str:
    return SchlockConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SchlockConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SchlockConstants, "AVATAR_KEYS", ()), getattr(SchlockConstants, "COVER_KEYS", ()))
