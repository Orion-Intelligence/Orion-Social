...
from api.social_manager.social_recon.constants.platform_constants import MessengerConstants

constants = MessengerConstants


def probe_url(username: str) -> str:
    return MessengerConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MessengerConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MessengerConstants, "AVATAR_KEYS", ()), getattr(MessengerConstants, "COVER_KEYS", ()))
