...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in SmuleConstants.GENERIC:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, SmuleConstants.AVATAR_KEYS, SmuleConstants.COVER_KEYS)
    match = re.match(r"(.+?)'s on Smule", heading)
    if match:
        info["username"] = match.group(1)
        info.setdefault("display_name", match.group(1))
