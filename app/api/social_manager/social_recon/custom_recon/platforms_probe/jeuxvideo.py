...
def probe_url(username: str) -> str:
    return JeuxVideoConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(JeuxVideoConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(JeuxVideoConstants, "AVATAR_KEYS", ()), getattr(JeuxVideoConstants, "COVER_KEYS", ()))
