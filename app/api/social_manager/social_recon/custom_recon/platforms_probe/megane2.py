...
from api.social_manager.social_recon.constants.platform_constants import Megane2Constants

constants = Megane2Constants


def probe_url(username: str) -> str:
    return Megane2Constants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(Megane2Constants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(Megane2Constants, "AVATAR_KEYS", ()), getattr(Megane2Constants, "COVER_KEYS", ()))
