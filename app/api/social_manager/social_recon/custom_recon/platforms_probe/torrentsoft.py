...
from api.social_manager.social_recon.constants.platform_constants import TorrentSoftConstants

constants = TorrentSoftConstants


def probe_url(username: str) -> str:
    return TorrentSoftConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TorrentSoftConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TorrentSoftConstants, "AVATAR_KEYS", ()), getattr(TorrentSoftConstants, "COVER_KEYS", ()))
