...
def probe_url(username: str) -> str:
    return ObservableConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ObservableConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ObservableConstants, "AVATAR_KEYS", ()), getattr(ObservableConstants, "COVER_KEYS", ()))
