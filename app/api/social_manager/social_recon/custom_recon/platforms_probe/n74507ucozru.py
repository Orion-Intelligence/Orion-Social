...
from api.social_manager.social_recon.constants.platform_constants import N74507UcozRuConstants

constants = N74507UcozRuConstants


def probe_url(username: str) -> str:
    return N74507UcozRuConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(N74507UcozRuConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(N74507UcozRuConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(N74507UcozRuConstants, "AVATAR_KEYS", ()), getattr(N74507UcozRuConstants, "COVER_KEYS", ()))
