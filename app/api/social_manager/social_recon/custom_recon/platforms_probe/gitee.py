...
from api.social_manager.social_recon.constants.platform_constants import GiteeConstants

constants = GiteeConstants


def probe_url(username: str) -> str:
    return GiteeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(GiteeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(GiteeConstants, "AVATAR_KEYS", ()), getattr(GiteeConstants, "COVER_KEYS", ()))
