...
def probe_url(username: str) -> str:
    return UltimateGuitarConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(UltimateGuitarConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(UltimateGuitarConstants, "AVATAR_KEYS", ()), getattr(UltimateGuitarConstants, "COVER_KEYS", ()))
