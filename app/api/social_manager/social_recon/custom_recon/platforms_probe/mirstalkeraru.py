...
from api.social_manager.social_recon.constants.platform_constants import MirStalkeraRuConstants

constants = MirStalkeraRuConstants


def probe_url(username: str) -> str:
    return MirStalkeraRuConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MirStalkeraRuConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MirStalkeraRuConstants, "AVATAR_KEYS", ()), getattr(MirStalkeraRuConstants, "COVER_KEYS", ()))
