...
from api.social_manager.social_recon.constants.platform_constants import WordnikConstants

constants = WordnikConstants


def probe_url(username: str) -> str:
    return WordnikConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WordnikConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WordnikConstants, "AVATAR_KEYS", ()), getattr(WordnikConstants, "COVER_KEYS", ()))
