...
from api.social_manager.social_recon.constants.platform_constants import ServeradminConstants

constants = ServeradminConstants


def probe_url(username: str) -> str:
    return ServeradminConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ServeradminConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ServeradminConstants, "AVATAR_KEYS", ()), getattr(ServeradminConstants, "COVER_KEYS", ()))
