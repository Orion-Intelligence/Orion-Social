...
def probe_url(username: str) -> str:
    return WriteAsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WriteAsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WriteAsConstants, "AVATAR_KEYS", ()), getattr(WriteAsConstants, "COVER_KEYS", ()))
