...
from api.social_manager.social_recon.constants.platform_constants import TalkMacpowerusersComConstants

constants = TalkMacpowerusersComConstants


def probe_url(username: str) -> str:
    return TalkMacpowerusersComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TalkMacpowerusersComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TalkMacpowerusersComConstants, "AVATAR_KEYS", ()), getattr(TalkMacpowerusersComConstants, "COVER_KEYS", ()))
