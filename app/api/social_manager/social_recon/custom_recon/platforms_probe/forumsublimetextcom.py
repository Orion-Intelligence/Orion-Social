...
from api.social_manager.social_recon.constants.platform_constants import ForumSublimetextComConstants

constants = ForumSublimetextComConstants


def probe_url(username: str) -> str:
    return ForumSublimetextComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForumSublimetextComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumSublimetextComConstants, "AVATAR_KEYS", ()), getattr(ForumSublimetextComConstants, "COVER_KEYS", ()))
