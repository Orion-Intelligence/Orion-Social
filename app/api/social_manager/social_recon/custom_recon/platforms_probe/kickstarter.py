...
from api.social_manager.social_recon.constants.platform_constants import KickstarterConstants

constants = KickstarterConstants


def probe_url(username: str) -> str:
    return KickstarterConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(KickstarterConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(KickstarterConstants, "AVATAR_KEYS", ()), getattr(KickstarterConstants, "COVER_KEYS", ()))
