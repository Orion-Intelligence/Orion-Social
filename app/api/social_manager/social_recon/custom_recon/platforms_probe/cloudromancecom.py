...
from api.social_manager.social_recon.constants.platform_constants import CloudromanceComConstants

constants = CloudromanceComConstants


def probe_url(username: str) -> str:
    return CloudromanceComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CloudromanceComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CloudromanceComConstants, "AVATAR_KEYS", ()), getattr(CloudromanceComConstants, "COVER_KEYS", ()))
