...
from api.social_manager.social_recon.constants.platform_constants import DonorboxConstants

constants = DonorboxConstants


def probe_url(username: str) -> str:
    return DonorboxConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DonorboxConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DonorboxConstants, "AVATAR_KEYS", ()), getattr(DonorboxConstants, "COVER_KEYS", ()))
