...
def probe_url(username: str) -> str:
    return DomestikaOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DomestikaOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DomestikaOrgConstants, "AVATAR_KEYS", ()), getattr(DomestikaOrgConstants, "COVER_KEYS", ()))
