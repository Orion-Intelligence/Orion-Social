...
def probe_url(username: str) -> str:
    return AreNaConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(AreNaConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(AreNaConstants, "AVATAR_KEYS", ()), getattr(AreNaConstants, "COVER_KEYS", ()))
