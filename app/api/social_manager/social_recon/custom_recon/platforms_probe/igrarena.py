...
from api.social_manager.social_recon.constants.platform_constants import IgrarenaConstants

constants = IgrarenaConstants


def probe_url(username: str) -> str:
    return IgrarenaConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(IgrarenaConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(IgrarenaConstants, "AVATAR_KEYS", ()), getattr(IgrarenaConstants, "COVER_KEYS", ()))
