...
from api.social_manager.social_recon.constants.platform_constants import ContactInBioDomainConstants

constants = ContactInBioDomainConstants


def probe_url(username: str) -> str:
    return ContactInBioDomainConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ContactInBioDomainConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ContactInBioDomainConstants, "AVATAR_KEYS", ()), getattr(ContactInBioDomainConstants, "COVER_KEYS", ()))
...
HOSTS = ('contactin.bio',)
SUBDOMAIN = ('contactin.bio', "profile")
ROUTES = (
    ("(?P<id>[^/]+)", "profile"),
)
