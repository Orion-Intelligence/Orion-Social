...
from api.social_manager.social_recon.constants.platform_constants import N123rfConstants

constants = N123rfConstants


def probe_url(username: str) -> str:
    return N123rfConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if "/profile_" not in (final_url or ""):
        return VerdictConstants.ABSENT, {}
    heading = parse.title(body)
    if not heading or heading.casefold() in getattr(N123rfConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(N123rfConstants, "AVATAR_KEYS", ()), getattr(N123rfConstants, "COVER_KEYS", ()))
