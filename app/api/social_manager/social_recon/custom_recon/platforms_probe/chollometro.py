...
from api.social_manager.social_recon.constants.platform_constants import ChollometroConstants

constants = ChollometroConstants


def probe_url(username: str) -> str:
    return ChollometroConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ChollometroConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ChollometroConstants, "AVATAR_KEYS", ()), getattr(ChollometroConstants, "COVER_KEYS", ()))
