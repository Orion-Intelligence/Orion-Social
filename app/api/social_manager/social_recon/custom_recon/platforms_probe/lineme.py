...
from api.social_manager.social_recon.constants.platform_constants import LineMeConstants

constants = LineMeConstants


def probe_url(username: str) -> str:
    return LineMeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(LineMeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(LineMeConstants, "AVATAR_KEYS", ()), getattr(LineMeConstants, "COVER_KEYS", ()))
