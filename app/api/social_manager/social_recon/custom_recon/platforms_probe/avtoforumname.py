...
def probe_url(username: str) -> str:
    return AvtoForumNameConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(AvtoForumNameConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(AvtoForumNameConstants, "AVATAR_KEYS", ()), getattr(AvtoForumNameConstants, "COVER_KEYS", ()))
