...
def probe_url(username: str) -> str:
    return VgtimesGamesConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(VgtimesGamesConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(VgtimesGamesConstants, "AVATAR_KEYS", ()), getattr(VgtimesGamesConstants, "COVER_KEYS", ()))
