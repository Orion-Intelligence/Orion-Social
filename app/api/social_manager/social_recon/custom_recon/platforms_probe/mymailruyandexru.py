...
from api.social_manager.social_recon.constants.platform_constants import MyMailRuYandexRuConstants

constants = MyMailRuYandexRuConstants


def probe_url(username: str) -> str:
    return MyMailRuYandexRuConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MyMailRuYandexRuConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MyMailRuYandexRuConstants, "AVATAR_KEYS", ()), getattr(MyMailRuYandexRuConstants, "COVER_KEYS", ()))
