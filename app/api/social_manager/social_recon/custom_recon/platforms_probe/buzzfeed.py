...
from api.social_manager.social_recon.constants.platform_constants import BuzzFeedConstants

constants = BuzzFeedConstants


def probe_url(username: str) -> str:
    return BuzzFeedConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(BuzzFeedConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(BuzzFeedConstants, "AVATAR_KEYS", ()), getattr(BuzzFeedConstants, "COVER_KEYS", ()))
