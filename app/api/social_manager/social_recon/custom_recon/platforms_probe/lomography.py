...
def probe_url(username: str) -> str:
    return LomographyConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(LomographyConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(LomographyConstants, "AVATAR_KEYS", ()), getattr(LomographyConstants, "COVER_KEYS", ()))
