...
def probe_url(username: str) -> str:
    return RussianFIConstants.PROFILE_URL.format(username=username)


def fetch(username: str) -> tuple[int, str, str]:
    return http_client.fetch(probe_url(username), impersonate=RussianFIConstants.IMPERSONATE)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(RussianFIConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(RussianFIConstants, "AVATAR_KEYS", ()), getattr(RussianFIConstants, "COVER_KEYS", ()))
...
ROUTES = (
    (r"forumdisplay\.php\?(?:.*&)?f=(?P<id>\d+)(?:&.*)?", "group"),
    ("forums/(?P<id>[^/]+)(?:/.*)?", "group"),
