...
def probe_url(username: str) -> str:
    return ResidentAdvisorConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ResidentAdvisorConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ResidentAdvisorConstants, "AVATAR_KEYS", ()), getattr(ResidentAdvisorConstants, "COVER_KEYS", ()))
