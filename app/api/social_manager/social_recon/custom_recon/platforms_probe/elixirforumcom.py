...
from api.social_manager.social_recon.constants.platform_constants import ElixirforumComConstants

constants = ElixirforumComConstants


def probe_url(username: str) -> str:
    return ElixirforumComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ElixirforumComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ElixirforumComConstants, "AVATAR_KEYS", ()), getattr(ElixirforumComConstants, "COVER_KEYS", ()))
