...
def probe_url(username: str) -> str:
    return JeepgarageOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(JeepgarageOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(JeepgarageOrgConstants, "AVATAR_KEYS", ()), getattr(JeepgarageOrgConstants, "COVER_KEYS", ()))
