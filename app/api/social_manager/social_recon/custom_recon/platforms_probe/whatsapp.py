...
def probe_url(username: str) -> str:
    return WhatsAppConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WhatsAppConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WhatsAppConstants, "AVATAR_KEYS", ()), getattr(WhatsAppConstants, "COVER_KEYS", ()))
