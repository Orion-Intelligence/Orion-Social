...
def probe_url(username: str) -> str:
    return Designs99Constants.PROFILE_URL.format(username=username)


def fetch(username: str) -> tuple[int, str, str]:
    return http_client.fetch(probe_url(username), impersonate=Designs99Constants.IMPERSONATE)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(Designs99Constants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(Designs99Constants, "AVATAR_KEYS", ()), getattr(Designs99Constants, "COVER_KEYS", ()))
