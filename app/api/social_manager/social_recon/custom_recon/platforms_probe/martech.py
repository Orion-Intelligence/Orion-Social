...
from api.social_manager.social_recon.constants.platform_constants import MartechConstants

constants = MartechConstants


def probe_url(username: str) -> str:
    return MartechConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MartechConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MartechConstants, "AVATAR_KEYS", ()), getattr(MartechConstants, "COVER_KEYS", ()))
