...
def probe_url(username: str) -> str:
    return BloggerBloggerComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(BloggerBloggerComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(BloggerBloggerComConstants, "AVATAR_KEYS", ()), getattr(BloggerBloggerComConstants, "COVER_KEYS", ()))
