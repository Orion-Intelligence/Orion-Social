...
from api.social_manager.social_recon.constants.platform_constants import PromodescuentosConstants

constants = PromodescuentosConstants


def probe_url(username: str) -> str:
    return PromodescuentosConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PromodescuentosConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PromodescuentosConstants, "AVATAR_KEYS", ()), getattr(PromodescuentosConstants, "COVER_KEYS", ()))
