...
from api.social_manager.social_recon.constants.platform_constants import DjagiConstants

constants = DjagiConstants


def probe_url(username: str) -> str:
    return DjagiConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DjagiConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DjagiConstants, "AVATAR_KEYS", ()), getattr(DjagiConstants, "COVER_KEYS", ()))
