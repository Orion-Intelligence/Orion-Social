...
from api.social_manager.social_recon.constants.platform_constants import FlickrGroupsConstants

constants = FlickrGroupsConstants


def probe_url(username: str) -> str:
    return FlickrGroupsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FlickrGroupsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FlickrGroupsConstants, "AVATAR_KEYS", ()), getattr(FlickrGroupsConstants, "COVER_KEYS", ()))
