...
from api.social_manager.social_recon.constants.platform_constants import WolframalphaForumConstants

constants = WolframalphaForumConstants


def probe_url(username: str) -> str:
    return WolframalphaForumConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WolframalphaForumConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WolframalphaForumConstants, "AVATAR_KEYS", ()), getattr(WolframalphaForumConstants, "COVER_KEYS", ()))
