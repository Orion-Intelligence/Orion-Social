...
from api.social_manager.social_recon.constants.platform_constants import SzmerInfoConstants

constants = SzmerInfoConstants


def probe_url(username: str) -> str:
    return SzmerInfoConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SzmerInfoConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SzmerInfoConstants, "AVATAR_KEYS", ()), getattr(SzmerInfoConstants, "COVER_KEYS", ()))
