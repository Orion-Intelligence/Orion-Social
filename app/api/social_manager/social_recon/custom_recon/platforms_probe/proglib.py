...
from api.social_manager.social_recon.constants.platform_constants import ProglibConstants

constants = ProglibConstants


def probe_url(username: str) -> str:
    return ProglibConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ProglibConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ProglibConstants, "AVATAR_KEYS", ()), getattr(ProglibConstants, "COVER_KEYS", ()))
