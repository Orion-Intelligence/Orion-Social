...
from api.social_manager.social_recon.constants.platform_constants import NesiditsaConstants

constants = NesiditsaConstants


def probe_url(username: str) -> str:
    return NesiditsaConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(NesiditsaConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(NesiditsaConstants, "AVATAR_KEYS", ()), getattr(NesiditsaConstants, "COVER_KEYS", ()))
