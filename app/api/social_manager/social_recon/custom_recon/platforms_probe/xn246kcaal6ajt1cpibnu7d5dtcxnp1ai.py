...
from api.social_manager.social_recon.constants.platform_constants import Xn246kcaal6ajt1cpibnu7d5dtcXnP1aiConstants

constants = Xn246kcaal6ajt1cpibnu7d5dtcXnP1aiConstants


def probe_url(username: str) -> str:
    return Xn246kcaal6ajt1cpibnu7d5dtcXnP1aiConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(Xn246kcaal6ajt1cpibnu7d5dtcXnP1aiConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(Xn246kcaal6ajt1cpibnu7d5dtcXnP1aiConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(Xn246kcaal6ajt1cpibnu7d5dtcXnP1aiConstants, "AVATAR_KEYS", ()), getattr(Xn246kcaal6ajt1cpibnu7d5dtcXnP1aiConstants, "COVER_KEYS", ()))
