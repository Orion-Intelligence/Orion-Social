...
def probe_url(username: str) -> str:
    return JigsawplanetConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(JigsawplanetConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(JigsawplanetConstants, "AVATAR_KEYS", ()), getattr(JigsawplanetConstants, "COVER_KEYS", ()))
