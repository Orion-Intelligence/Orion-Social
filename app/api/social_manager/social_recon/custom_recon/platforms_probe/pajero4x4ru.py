...
from api.social_manager.social_recon.constants.platform_constants import Pajero4x4RuConstants

constants = Pajero4x4RuConstants


def probe_url(username: str) -> str:
    return Pajero4x4RuConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(Pajero4x4RuConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(Pajero4x4RuConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(Pajero4x4RuConstants, "AVATAR_KEYS", ()), getattr(Pajero4x4RuConstants, "COVER_KEYS", ()))
...
ROUTES = (
    (r"bbs/phpBB2/search\.php/?\?(?:.*&)?author=(?P<id>[^&]+)(?:&.*)?", "profile"),
)
