...
def probe_url(username: str) -> str:
    return DesignspirationConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DesignspirationConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DesignspirationConstants, "AVATAR_KEYS", ()), getattr(DesignspirationConstants, "COVER_KEYS", ()))
