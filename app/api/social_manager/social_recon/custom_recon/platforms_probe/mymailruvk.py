...
from api.social_manager.social_recon.constants.platform_constants import MyMailRuVKConstants

constants = MyMailRuVKConstants


def probe_url(username: str) -> str:
    return MyMailRuVKConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MyMailRuVKConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MyMailRuVKConstants, "AVATAR_KEYS", ()), getattr(MyMailRuVKConstants, "COVER_KEYS", ()))
