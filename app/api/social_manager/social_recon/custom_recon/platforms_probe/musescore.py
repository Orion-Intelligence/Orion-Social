...
def probe_url(username: str) -> str:
    return MuseScoreConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MuseScoreConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MuseScoreConstants, "AVATAR_KEYS", ()), getattr(MuseScoreConstants, "COVER_KEYS", ()))
