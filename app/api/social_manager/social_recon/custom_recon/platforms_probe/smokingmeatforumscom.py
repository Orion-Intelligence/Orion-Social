...
def probe_url(username: str) -> str:
    return SmokingmeatforumsComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SmokingmeatforumsComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SmokingmeatforumsComConstants, "AVATAR_KEYS", ()), getattr(SmokingmeatforumsComConstants, "COVER_KEYS", ()))
