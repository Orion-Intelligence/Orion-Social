...
from api.social_manager.social_recon.constants.platform_constants import KidicaruswikiOrgConstants

constants = KidicaruswikiOrgConstants


def probe_url(username: str) -> str:
    return KidicaruswikiOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(KidicaruswikiOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(KidicaruswikiOrgConstants, "AVATAR_KEYS", ()), getattr(KidicaruswikiOrgConstants, "COVER_KEYS", ()))
