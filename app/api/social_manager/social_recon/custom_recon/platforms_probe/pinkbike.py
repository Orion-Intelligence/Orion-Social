...
def probe_url(username: str) -> str:
    return PinkbikeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PinkbikeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PinkbikeConstants, "AVATAR_KEYS", ()), getattr(PinkbikeConstants, "COVER_KEYS", ()))
