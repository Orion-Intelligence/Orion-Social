...
def probe_url(username: str) -> str:
    return RidemonkeyComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(RidemonkeyComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(RidemonkeyComConstants, "AVATAR_KEYS", ()), getattr(RidemonkeyComConstants, "COVER_KEYS", ()))
