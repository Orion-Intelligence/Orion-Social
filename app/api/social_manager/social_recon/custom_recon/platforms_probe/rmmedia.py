...
from api.social_manager.social_recon.constants.platform_constants import RmmediaConstants

constants = RmmediaConstants


def probe_url(username: str) -> str:
    return RmmediaConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(RmmediaConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(RmmediaConstants, "AVATAR_KEYS", ()), getattr(RmmediaConstants, "COVER_KEYS", ()))
