...
from api.social_manager.social_recon.constants.platform_constants import MydealzConstants

constants = MydealzConstants


def probe_url(username: str) -> str:
    return MydealzConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MydealzConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MydealzConstants, "AVATAR_KEYS", ()), getattr(MydealzConstants, "COVER_KEYS", ()))
