...
from api.social_manager.social_recon.constants.platform_constants import CollectorsComConstants

constants = CollectorsComConstants


def probe_url(username: str) -> str:
    return CollectorsComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CollectorsComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CollectorsComConstants, "AVATAR_KEYS", ()), getattr(CollectorsComConstants, "COVER_KEYS", ()))
