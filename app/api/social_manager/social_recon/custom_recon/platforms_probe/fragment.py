...
def probe_url(username: str) -> str:
    return FragmentConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FragmentConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FragmentConstants, "AVATAR_KEYS", ()), getattr(FragmentConstants, "COVER_KEYS", ()))
