...
from api.social_manager.social_recon.constants.platform_constants import MetroidwikiOrgConstants

constants = MetroidwikiOrgConstants


def probe_url(username: str) -> str:
    return MetroidwikiOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MetroidwikiOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MetroidwikiOrgConstants, "AVATAR_KEYS", ()), getattr(MetroidwikiOrgConstants, "COVER_KEYS", ()))
