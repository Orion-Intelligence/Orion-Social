...
from api.social_manager.social_recon.constants.platform_constants import N999MdConstants

constants = N999MdConstants


def probe_url(username: str) -> str:
    return N999MdConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(N999MdConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(N999MdConstants, "AVATAR_KEYS", ()), getattr(N999MdConstants, "COVER_KEYS", ()))
