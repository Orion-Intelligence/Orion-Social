...
from api.social_manager.social_recon.constants.platform_constants import NglLinkConstants

constants = NglLinkConstants


def probe_url(username: str) -> str:
    return NglLinkConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(NglLinkConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(NglLinkConstants, "AVATAR_KEYS", ()), getattr(NglLinkConstants, "COVER_KEYS", ()))
