...
from api.social_manager.social_recon.constants.platform_constants import N4x4TomskRuConstants

constants = N4x4TomskRuConstants


def probe_url(username: str) -> str:
    return N4x4TomskRuConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(N4x4TomskRuConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(N4x4TomskRuConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(N4x4TomskRuConstants, "AVATAR_KEYS", ()), getattr(N4x4TomskRuConstants, "COVER_KEYS", ()))
