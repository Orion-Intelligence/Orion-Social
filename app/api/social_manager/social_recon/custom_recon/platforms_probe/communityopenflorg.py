...
from api.social_manager.social_recon.constants.platform_constants import CommunityOpenflOrgConstants

constants = CommunityOpenflOrgConstants


def probe_url(username: str) -> str:
    return CommunityOpenflOrgConstants.API_URL.format(username=username)
...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    if not isinstance(user, dict) or not user.get("username"):
        return VerdictConstants.UNKNOWN, {}
    base = CommunityOpenflOrgConstants.PROFILE_URL.split("/u/")[0]
