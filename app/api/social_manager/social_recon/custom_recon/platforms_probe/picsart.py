...
def probe_url(username: str) -> str:
    return PicsartConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PicsartConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PicsartConstants, "AVATAR_KEYS", ()), getattr(PicsartConstants, "COVER_KEYS", ()))
