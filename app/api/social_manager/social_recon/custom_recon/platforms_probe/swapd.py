...
from api.social_manager.social_recon.constants.platform_constants import SwapdConstants

constants = SwapdConstants


def probe_url(username: str) -> str:
    return SwapdConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SwapdConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SwapdConstants, "AVATAR_KEYS", ()), getattr(SwapdConstants, "COVER_KEYS", ()))
