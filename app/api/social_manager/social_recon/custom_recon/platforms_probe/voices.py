...
def probe_url(username: str) -> str:
    return VoicesConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(VoicesConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(VoicesConstants, "AVATAR_KEYS", ()), getattr(VoicesConstants, "COVER_KEYS", ()))
