...
def probe_url(username: str) -> str:
    return IFunnyConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(IFunnyConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(IFunnyConstants, "AVATAR_KEYS", ()), getattr(IFunnyConstants, "COVER_KEYS", ()))
