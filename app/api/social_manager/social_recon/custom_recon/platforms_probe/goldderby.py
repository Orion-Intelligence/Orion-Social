...
def probe_url(username: str) -> str:
    return GoldderbyConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(GoldderbyConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(GoldderbyConstants, "AVATAR_KEYS", ()), getattr(GoldderbyConstants, "COVER_KEYS", ()))
