...
from api.social_manager.social_recon.constants.platform_constants import TinkoffInvestConstants

constants = TinkoffInvestConstants


def probe_url(username: str) -> str:
    return TinkoffInvestConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TinkoffInvestConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TinkoffInvestConstants, "AVATAR_KEYS", ()), getattr(TinkoffInvestConstants, "COVER_KEYS", ()))
...
ROUTES = (
    (r"invest/social/profile/(?P<id>[^/]+)(?:/.*)?", "profile"),
)
