...
from api.social_manager.social_recon.constants.platform_constants import QQConstants

constants = QQConstants


def probe_url(username: str) -> str:
    return QQConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(QQConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(QQConstants, "AVATAR_KEYS", ()), getattr(QQConstants, "COVER_KEYS", ()))
