...
from api.social_manager.social_recon.constants.platform_constants import DiscussCircleciComConstants

constants = DiscussCircleciComConstants


def probe_url(username: str) -> str:
    return DiscussCircleciComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DiscussCircleciComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DiscussCircleciComConstants, "AVATAR_KEYS", ()), getattr(DiscussCircleciComConstants, "COVER_KEYS", ()))
