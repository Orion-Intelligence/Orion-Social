...
from api.social_manager.social_recon.constants.platform_constants import SketchfabComConstants

constants = SketchfabComConstants


def probe_url(username: str) -> str:
    return SketchfabComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SketchfabComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SketchfabComConstants, "AVATAR_KEYS", ()), getattr(SketchfabComConstants, "COVER_KEYS", ()))
