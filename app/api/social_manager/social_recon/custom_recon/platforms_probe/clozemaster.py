...
def probe_url(username: str) -> str:
    return ClozemasterConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ClozemasterConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ClozemasterConstants, "AVATAR_KEYS", ()), getattr(ClozemasterConstants, "COVER_KEYS", ()))
