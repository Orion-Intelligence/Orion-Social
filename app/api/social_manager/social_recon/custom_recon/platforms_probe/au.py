...
def probe_url(username: str) -> str:
    return AuConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(AuConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(AuConstants, "AVATAR_KEYS", ()), getattr(AuConstants, "COVER_KEYS", ()))
