...
from api.social_manager.social_recon.constants.platform_constants import ListedToConstants

constants = ListedToConstants


def probe_url(username: str) -> str:
    return ListedToConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ListedToConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ListedToConstants, "AVATAR_KEYS", ()), getattr(ListedToConstants, "COVER_KEYS", ()))
