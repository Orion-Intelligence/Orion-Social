...
from api.social_manager.social_recon.constants.platform_constants import KwejkConstants

constants = KwejkConstants


def probe_url(username: str) -> str:
    return KwejkConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(KwejkConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(KwejkConstants, "AVATAR_KEYS", ()), getattr(KwejkConstants, "COVER_KEYS", ()))
