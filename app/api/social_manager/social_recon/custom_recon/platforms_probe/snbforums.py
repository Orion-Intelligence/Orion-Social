...
def probe_url(username: str) -> str:
    return SnbforumsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SnbforumsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SnbforumsConstants, "AVATAR_KEYS", ()), getattr(SnbforumsConstants, "COVER_KEYS", ()))
