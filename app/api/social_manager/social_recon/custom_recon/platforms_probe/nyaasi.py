...
from api.social_manager.social_recon.constants.platform_constants import NyaaSiConstants

constants = NyaaSiConstants


def probe_url(username: str) -> str:
    return NyaaSiConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(NyaaSiConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(NyaaSiConstants, "AVATAR_KEYS", ()), getattr(NyaaSiConstants, "COVER_KEYS", ()))
