...
from api.social_manager.social_recon.constants.platform_constants import ArhrockConstants

constants = ArhrockConstants


def probe_url(username: str) -> str:
    return ArhrockConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ArhrockConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ArhrockConstants, "AVATAR_KEYS", ()), getattr(ArhrockConstants, "COVER_KEYS", ()))
