...
from api.social_manager.social_recon.constants.platform_constants import CSLordsConstants

constants = CSLordsConstants


def probe_url(username: str) -> str:
    return CSLordsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CSLordsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CSLordsConstants, "AVATAR_KEYS", ()), getattr(CSLordsConstants, "COVER_KEYS", ()))
