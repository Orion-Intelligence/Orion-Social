...
def probe_url(username: str) -> str:
    return GogConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(GogConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(GogConstants, "AVATAR_KEYS", ()), getattr(GogConstants, "COVER_KEYS", ()))
