...
def probe_url(username: str) -> str:
    return SportsRuConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SportsRuConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SportsRuConstants, "AVATAR_KEYS", ()), getattr(SportsRuConstants, "COVER_KEYS", ()))
