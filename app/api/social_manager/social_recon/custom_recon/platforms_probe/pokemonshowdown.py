...
def probe_url(username: str) -> str:
    return PokemonShowdownConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PokemonShowdownConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PokemonShowdownConstants, "AVATAR_KEYS", ()), getattr(PokemonShowdownConstants, "COVER_KEYS", ()))
