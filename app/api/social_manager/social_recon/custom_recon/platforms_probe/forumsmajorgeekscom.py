...
from api.social_manager.social_recon.constants.platform_constants import ForumsMajorgeeksComConstants

constants = ForumsMajorgeeksComConstants


def probe_url(username: str) -> str:
    return ForumsMajorgeeksComConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(ForumsMajorgeeksComConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(ForumsMajorgeeksComConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumsMajorgeeksComConstants, "AVATAR_KEYS", ()), getattr(ForumsMajorgeeksComConstants, "COVER_KEYS", ()))
