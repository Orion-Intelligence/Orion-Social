...
from api.social_manager.social_recon.constants.platform_constants import HouseMixesComConstants

constants = HouseMixesComConstants


def probe_url(username: str) -> str:
    return HouseMixesComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(HouseMixesComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(HouseMixesComConstants, "AVATAR_KEYS", ()), getattr(HouseMixesComConstants, "COVER_KEYS", ()))
