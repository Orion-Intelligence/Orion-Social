...
def probe_url(username: str) -> str:
    return RottentomatoesConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(RottentomatoesConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(RottentomatoesConstants, "AVATAR_KEYS", ()), getattr(RottentomatoesConstants, "COVER_KEYS", ()))
