...
def probe_url(username: str) -> str:
    return DisqusConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DisqusConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DisqusConstants, "AVATAR_KEYS", ()), getattr(DisqusConstants, "COVER_KEYS", ()))
