...
from api.social_manager.social_recon.constants.platform_constants import CommunityRoonlabsComConstants

constants = CommunityRoonlabsComConstants


def probe_url(username: str) -> str:
    return CommunityRoonlabsComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CommunityRoonlabsComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CommunityRoonlabsComConstants, "AVATAR_KEYS", ()), getattr(CommunityRoonlabsComConstants, "COVER_KEYS", ()))
