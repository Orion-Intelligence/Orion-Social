...
def probe_url(username: str) -> str:
    return WykopConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WykopConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WykopConstants, "AVATAR_KEYS", ()), getattr(WykopConstants, "COVER_KEYS", ()))
