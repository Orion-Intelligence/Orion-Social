...
from api.social_manager.social_recon.constants.platform_constants import HackMDConstants

constants = HackMDConstants


def probe_url(username: str) -> str:
    return HackMDConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(HackMDConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(HackMDConstants, "AVATAR_KEYS", ()), getattr(HackMDConstants, "COVER_KEYS", ()))
