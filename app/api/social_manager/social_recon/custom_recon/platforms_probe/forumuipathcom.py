...
from api.social_manager.social_recon.constants.platform_constants import ForumUipathComConstants

constants = ForumUipathComConstants


def probe_url(username: str) -> str:
    return ForumUipathComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForumUipathComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumUipathComConstants, "AVATAR_KEYS", ()), getattr(ForumUipathComConstants, "COVER_KEYS", ()))
