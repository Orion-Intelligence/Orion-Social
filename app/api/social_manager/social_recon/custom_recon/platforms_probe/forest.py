...
from api.social_manager.social_recon.constants.platform_constants import ForestConstants

constants = ForestConstants


def probe_url(username: str) -> str:
    return ForestConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForestConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForestConstants, "AVATAR_KEYS", ()), getattr(ForestConstants, "COVER_KEYS", ()))
