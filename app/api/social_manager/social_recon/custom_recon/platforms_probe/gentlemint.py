...
from api.social_manager.social_recon.constants.platform_constants import GentlemintConstants

constants = GentlemintConstants


def probe_url(username: str) -> str:
    return GentlemintConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(GentlemintConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(GentlemintConstants, "AVATAR_KEYS", ()), getattr(GentlemintConstants, "COVER_KEYS", ()))
