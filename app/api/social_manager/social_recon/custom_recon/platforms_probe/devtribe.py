...
from api.social_manager.social_recon.constants.platform_constants import DevtribeConstants

constants = DevtribeConstants


def probe_url(username: str) -> str:
    return DevtribeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DevtribeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DevtribeConstants, "AVATAR_KEYS", ()), getattr(DevtribeConstants, "COVER_KEYS", ()))
