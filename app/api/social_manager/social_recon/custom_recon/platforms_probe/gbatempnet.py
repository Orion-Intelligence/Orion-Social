...
def probe_url(username: str) -> str:
    return GBAtempNetConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(GBAtempNetConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(GBAtempNetConstants, "AVATAR_KEYS", ()), getattr(GBAtempNetConstants, "COVER_KEYS", ()))
