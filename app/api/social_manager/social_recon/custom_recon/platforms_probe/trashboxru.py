...
from api.social_manager.social_recon.constants.platform_constants import TrashboxRuConstants

constants = TrashboxRuConstants


def probe_url(username: str) -> str:
    return TrashboxRuConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TrashboxRuConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TrashboxRuConstants, "AVATAR_KEYS", ()), getattr(TrashboxRuConstants, "COVER_KEYS", ()))
