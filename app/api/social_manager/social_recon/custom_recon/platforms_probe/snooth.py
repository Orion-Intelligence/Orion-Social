...
from api.social_manager.social_recon.constants.platform_constants import SnoothConstants

constants = SnoothConstants


def probe_url(username: str) -> str:
    return SnoothConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SnoothConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SnoothConstants, "AVATAR_KEYS", ()), getattr(SnoothConstants, "COVER_KEYS", ()))
