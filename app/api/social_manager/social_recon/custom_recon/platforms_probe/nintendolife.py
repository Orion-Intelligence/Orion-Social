...
def probe_url(username: str) -> str:
    return NintendoLifeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(NintendoLifeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(NintendoLifeConstants, "AVATAR_KEYS", ()), getattr(NintendoLifeConstants, "COVER_KEYS", ()))
