...
def probe_url(username: str) -> str:
    return ChangeOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ChangeOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ChangeOrgConstants, "AVATAR_KEYS", ()), getattr(ChangeOrgConstants, "COVER_KEYS", ()))
