...
def probe_url(username: str) -> str:
    return NPMConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(NPMConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(NPMConstants, "AVATAR_KEYS", ()), getattr(NPMConstants, "COVER_KEYS", ()))
