...
from api.social_manager.social_recon.constants.platform_constants import Spells8Constants

constants = Spells8Constants


def probe_url(username: str) -> str:
    return Spells8Constants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(Spells8Constants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(Spells8Constants, "AVATAR_KEYS", ()), getattr(Spells8Constants, "COVER_KEYS", ()))
