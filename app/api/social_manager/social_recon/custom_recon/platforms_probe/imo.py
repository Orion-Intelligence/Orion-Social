...
from api.social_manager.social_recon.constants.platform_constants import ImoConstants

constants = ImoConstants


def probe_url(username: str) -> str:
    return ImoConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ImoConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ImoConstants, "AVATAR_KEYS", ()), getattr(ImoConstants, "COVER_KEYS", ()))
