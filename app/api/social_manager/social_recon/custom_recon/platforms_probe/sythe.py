...
from api.social_manager.social_recon.constants.platform_constants import SytheConstants

constants = SytheConstants


def probe_url(username: str) -> str:
    return SytheConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SytheConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SytheConstants, "AVATAR_KEYS", ()), getattr(SytheConstants, "COVER_KEYS", ()))
