...
from api.social_manager.social_recon.constants.platform_constants import PreisjaegerConstants

constants = PreisjaegerConstants


def probe_url(username: str) -> str:
    return PreisjaegerConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PreisjaegerConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PreisjaegerConstants, "AVATAR_KEYS", ()), getattr(PreisjaegerConstants, "COVER_KEYS", ()))
