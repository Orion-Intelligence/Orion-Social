...
def probe_url(username: str) -> str:
    return SpatialConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SpatialConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SpatialConstants, "AVATAR_KEYS", ()), getattr(SpatialConstants, "COVER_KEYS", ()))
