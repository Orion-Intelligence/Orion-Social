...
def probe_url(username: str) -> str:
    return JSFiddleConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(JSFiddleConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(JSFiddleConstants, "AVATAR_KEYS", ()), getattr(JSFiddleConstants, "COVER_KEYS", ()))
