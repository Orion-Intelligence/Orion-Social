...
from api.social_manager.social_recon.constants.platform_constants import NiftygatewayConstants

constants = NiftygatewayConstants


def probe_url(username: str) -> str:
    return NiftygatewayConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(NiftygatewayConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(NiftygatewayConstants, "AVATAR_KEYS", ()), getattr(NiftygatewayConstants, "COVER_KEYS", ()))
