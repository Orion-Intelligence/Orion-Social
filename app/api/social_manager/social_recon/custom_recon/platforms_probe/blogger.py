...
from api.social_manager.social_recon.constants.platform_constants import BloggerConstants

constants = BloggerConstants


def probe_url(username: str) -> str:
    return BloggerConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(BloggerConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(BloggerConstants, "AVATAR_KEYS", ()), getattr(BloggerConstants, "COVER_KEYS", ()))
...
HOSTS = ("blogspot.com",)
SUBDOMAIN = ("blogspot.com", "profile")
ROUTES = ()
