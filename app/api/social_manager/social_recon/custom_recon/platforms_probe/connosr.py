...
from api.social_manager.social_recon.constants.platform_constants import ConnosrConstants

constants = ConnosrConstants


def probe_url(username: str) -> str:
    return ConnosrConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ConnosrConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ConnosrConstants, "AVATAR_KEYS", ()), getattr(ConnosrConstants, "COVER_KEYS", ()))
