...
def probe_url(username: str) -> str:
    return MetalArchivesConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MetalArchivesConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MetalArchivesConstants, "AVATAR_KEYS", ()), getattr(MetalArchivesConstants, "COVER_KEYS", ()))
