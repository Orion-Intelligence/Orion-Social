...
from api.social_manager.social_recon.constants.platform_constants import FfmBioConstants

constants = FfmBioConstants


def probe_url(username: str) -> str:
    return FfmBioConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FfmBioConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FfmBioConstants, "AVATAR_KEYS", ()), getattr(FfmBioConstants, "COVER_KEYS", ()))
