...
from api.social_manager.social_recon.constants.platform_constants import ForumGolangbridgeOrgConstants

constants = ForumGolangbridgeOrgConstants


def probe_url(username: str) -> str:
    return ForumGolangbridgeOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForumGolangbridgeOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumGolangbridgeOrgConstants, "AVATAR_KEYS", ()), getattr(ForumGolangbridgeOrgConstants, "COVER_KEYS", ()))
