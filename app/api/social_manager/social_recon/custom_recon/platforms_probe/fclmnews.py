...
from api.social_manager.social_recon.constants.platform_constants import FclmnewsConstants

constants = FclmnewsConstants


def probe_url(username: str) -> str:
    return FclmnewsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FclmnewsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FclmnewsConstants, "AVATAR_KEYS", ()), getattr(FclmnewsConstants, "COVER_KEYS", ()))
