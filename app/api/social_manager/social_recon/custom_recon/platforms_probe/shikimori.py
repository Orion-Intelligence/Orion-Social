...
from api.social_manager.social_recon.constants.platform_constants import ShikimoriConstants

constants = ShikimoriConstants


def probe_url(username: str) -> str:
    return ShikimoriConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ShikimoriConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ShikimoriConstants, "AVATAR_KEYS", ()), getattr(ShikimoriConstants, "COVER_KEYS", ()))
