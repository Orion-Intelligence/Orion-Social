...
from api.social_manager.social_recon.constants.platform_constants import DonatePayConstants

constants = DonatePayConstants


def probe_url(username: str) -> str:
    return DonatePayConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DonatePayConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DonatePayConstants, "AVATAR_KEYS", ()), getattr(DonatePayConstants, "COVER_KEYS", ()))
