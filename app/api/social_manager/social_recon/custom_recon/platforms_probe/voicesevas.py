...
from api.social_manager.social_recon.constants.platform_constants import VoicesevasConstants

constants = VoicesevasConstants


def probe_url(username: str) -> str:
    return VoicesevasConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(VoicesevasConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(VoicesevasConstants, "AVATAR_KEYS", ()), getattr(VoicesevasConstants, "COVER_KEYS", ()))
