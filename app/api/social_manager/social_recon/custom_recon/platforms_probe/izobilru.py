...
from api.social_manager.social_recon.constants.platform_constants import IzobilRuConstants

constants = IzobilRuConstants


def probe_url(username: str) -> str:
    return IzobilRuConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(IzobilRuConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(IzobilRuConstants, "AVATAR_KEYS", ()), getattr(IzobilRuConstants, "COVER_KEYS", ()))
