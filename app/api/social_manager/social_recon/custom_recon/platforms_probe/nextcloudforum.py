...
def probe_url(username: str) -> str:
    return NextcloudForumConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(NextcloudForumConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(NextcloudForumConstants, "AVATAR_KEYS", ()), getattr(NextcloudForumConstants, "COVER_KEYS", ()))
