...
from api.social_manager.social_recon.constants.platform_constants import ForumGitlabComConstants

constants = ForumGitlabComConstants


def probe_url(username: str) -> str:
    return ForumGitlabComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForumGitlabComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumGitlabComConstants, "AVATAR_KEYS", ()), getattr(ForumGitlabComConstants, "COVER_KEYS", ()))
