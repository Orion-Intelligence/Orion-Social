...
from api.social_manager.social_recon.constants.platform_constants import CodeSnippetWikiConstants

constants = CodeSnippetWikiConstants


def probe_url(username: str) -> str:
    return CodeSnippetWikiConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CodeSnippetWikiConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CodeSnippetWikiConstants, "AVATAR_KEYS", ()), getattr(CodeSnippetWikiConstants, "COVER_KEYS", ()))
