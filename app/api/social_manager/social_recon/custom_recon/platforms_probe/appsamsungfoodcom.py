...
from api.social_manager.social_recon.constants.platform_constants import AppSamsungfoodComConstants

constants = AppSamsungfoodComConstants


def probe_url(username: str) -> str:
    return AppSamsungfoodComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(AppSamsungfoodComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(AppSamsungfoodComConstants, "AVATAR_KEYS", ()), getattr(AppSamsungfoodComConstants, "COVER_KEYS", ()))
