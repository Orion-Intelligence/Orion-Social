...
from api.social_manager.social_recon.constants.platform_constants import SEOForumConstants

constants = SEOForumConstants


def probe_url(username: str) -> str:
    return SEOForumConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SEOForumConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SEOForumConstants, "AVATAR_KEYS", ()), getattr(SEOForumConstants, "COVER_KEYS", ()))
