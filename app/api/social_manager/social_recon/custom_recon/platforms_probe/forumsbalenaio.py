...
from api.social_manager.social_recon.constants.platform_constants import ForumsBalenaIoConstants

constants = ForumsBalenaIoConstants


def probe_url(username: str) -> str:
    return ForumsBalenaIoConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForumsBalenaIoConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumsBalenaIoConstants, "AVATAR_KEYS", ()), getattr(ForumsBalenaIoConstants, "COVER_KEYS", ()))
