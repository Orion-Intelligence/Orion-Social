...
from api.social_manager.social_recon.constants.platform_constants import TradingViewConstants

constants = TradingViewConstants


def probe_url(username: str) -> str:
    return TradingViewConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TradingViewConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TradingViewConstants, "AVATAR_KEYS", ()), getattr(TradingViewConstants, "COVER_KEYS", ()))
