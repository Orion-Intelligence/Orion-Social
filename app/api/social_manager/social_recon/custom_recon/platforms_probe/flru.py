...
from api.social_manager.social_recon.constants.platform_constants import FLRuConstants

constants = FLRuConstants


def probe_url(username: str) -> str:
    return FLRuConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FLRuConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FLRuConstants, "AVATAR_KEYS", ()), getattr(FLRuConstants, "COVER_KEYS", ()))
