...
def probe_url(username: str) -> str:
    return RubyForumConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(RubyForumConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(RubyForumConstants, "AVATAR_KEYS", ()), getattr(RubyForumConstants, "COVER_KEYS", ()))
