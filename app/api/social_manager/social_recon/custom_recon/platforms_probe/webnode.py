...
from api.social_manager.social_recon.constants.platform_constants import WebNodeConstants

constants = WebNodeConstants


def probe_url(username: str) -> str:
    return WebNodeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WebNodeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WebNodeConstants, "AVATAR_KEYS", ()), getattr(WebNodeConstants, "COVER_KEYS", ()))
