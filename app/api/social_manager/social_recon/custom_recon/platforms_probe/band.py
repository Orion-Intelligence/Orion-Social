...
from api.social_manager.social_recon.constants.platform_constants import BandConstants

constants = BandConstants


def probe_url(username: str) -> str:
    return BandConstants.PROFILE_URL.format(username=username)
...
ROUTES = (
    ("@(?P<id>[^/]+)", "profile"),
    (r"band/(?P<id>\d+)/post/(?P<post>\d+)", "post"),
    (r"band/(?P<id>\d+)", "group"),
)
