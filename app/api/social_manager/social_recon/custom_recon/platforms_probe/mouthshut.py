...
from api.social_manager.social_recon.constants.platform_constants import MouthshutConstants

constants = MouthshutConstants


def probe_url(username: str) -> str:
    return MouthshutConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MouthshutConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MouthshutConstants, "AVATAR_KEYS", ()), getattr(MouthshutConstants, "COVER_KEYS", ()))
