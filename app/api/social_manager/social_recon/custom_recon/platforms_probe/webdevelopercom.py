...
def probe_url(username: str) -> str:
    return WebdeveloperComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WebdeveloperComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WebdeveloperComConstants, "AVATAR_KEYS", ()), getattr(WebdeveloperComConstants, "COVER_KEYS", ()))
