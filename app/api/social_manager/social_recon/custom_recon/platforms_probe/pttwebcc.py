...
from api.social_manager.social_recon.constants.platform_constants import PttwebCcConstants

constants = PttwebCcConstants


def probe_url(username: str) -> str:
    return PttwebCcConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PttwebCcConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PttwebCcConstants, "AVATAR_KEYS", ()), getattr(PttwebCcConstants, "COVER_KEYS", ()))
