...
def probe_url(username: str) -> str:
    return TriplineConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TriplineConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TriplineConstants, "AVATAR_KEYS", ()), getattr(TriplineConstants, "COVER_KEYS", ()))
