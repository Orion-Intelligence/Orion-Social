...
from api.social_manager.social_recon.constants.platform_constants import JAlbumNetConstants

constants = JAlbumNetConstants


def probe_url(username: str) -> str:
    return JAlbumNetConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(JAlbumNetConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(JAlbumNetConstants, "AVATAR_KEYS", ()), getattr(JAlbumNetConstants, "COVER_KEYS", ()))
