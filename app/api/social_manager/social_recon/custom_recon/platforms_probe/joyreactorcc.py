...
def probe_url(username: str) -> str:
    return JoyreactorCcConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(JoyreactorCcConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(JoyreactorCcConstants, "AVATAR_KEYS", ()), getattr(JoyreactorCcConstants, "COVER_KEYS", ()))
