...
def probe_url(username: str) -> str:
    return SamlibConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SamlibConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SamlibConstants, "AVATAR_KEYS", ()), getattr(SamlibConstants, "COVER_KEYS", ()))
