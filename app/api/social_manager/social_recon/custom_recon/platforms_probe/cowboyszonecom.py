...
def probe_url(username: str) -> str:
    return CowboyszoneComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CowboyszoneComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CowboyszoneComConstants, "AVATAR_KEYS", ()), getattr(CowboyszoneComConstants, "COVER_KEYS", ()))
