...
from api.social_manager.social_recon.constants.platform_constants import EintrachtConstants

constants = EintrachtConstants


def probe_url(username: str) -> str:
    return EintrachtConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(EintrachtConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(EintrachtConstants, "AVATAR_KEYS", ()), getattr(EintrachtConstants, "COVER_KEYS", ()))
