...
def probe_url(username: str) -> str:
    return FotkiConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FotkiConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FotkiConstants, "AVATAR_KEYS", ()), getattr(FotkiConstants, "COVER_KEYS", ()))
