...
def probe_url(username: str) -> str:
    return DigitalPointConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DigitalPointConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DigitalPointConstants, "AVATAR_KEYS", ()), getattr(DigitalPointConstants, "COVER_KEYS", ()))
