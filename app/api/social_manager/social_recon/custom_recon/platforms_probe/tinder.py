...
def probe_url(username: str) -> str:
    return TinderConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TinderConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TinderConstants, "AVATAR_KEYS", ()), getattr(TinderConstants, "COVER_KEYS", ()))
