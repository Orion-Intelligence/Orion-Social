...
from api.social_manager.social_recon.constants.platform_constants import FosstodonConstants

constants = FosstodonConstants


def probe_url(username: str) -> str:
    return FosstodonConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FosstodonConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FosstodonConstants, "AVATAR_KEYS", ()), getattr(FosstodonConstants, "COVER_KEYS", ()))
