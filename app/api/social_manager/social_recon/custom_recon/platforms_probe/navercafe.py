...
def probe_url(username: str) -> str:
    return NaverCafeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(NaverCafeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(NaverCafeConstants, "AVATAR_KEYS", ()), getattr(NaverCafeConstants, "COVER_KEYS", ()))
