...
from api.social_manager.social_recon.constants.platform_constants import FlickrConstants

constants = FlickrConstants


def probe_url(username: str) -> str:
    return FlickrConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in FlickrConstants.GENERIC:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, FlickrConstants.AVATAR_KEYS, FlickrConstants.COVER_KEYS)
    ...
    match = re.search(FlickrConstants.PHOTO_COUNT, body)
    if match:
        info["posts"] = match.group(1)
