...
from api.social_manager.social_recon.constants.platform_constants import StellerCoConstants

constants = StellerCoConstants


def probe_url(username: str) -> str:
    return StellerCoConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(StellerCoConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(StellerCoConstants, "AVATAR_KEYS", ()), getattr(StellerCoConstants, "COVER_KEYS", ()))
