...
from api.social_manager.social_recon.constants.platform_constants import ExposureConstants

constants = ExposureConstants


def probe_url(username: str) -> str:
    return ExposureConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ExposureConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ExposureConstants, "AVATAR_KEYS", ()), getattr(ExposureConstants, "COVER_KEYS", ()))
