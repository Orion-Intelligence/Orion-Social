...
def probe_url(username: str) -> str:
    return SysadminsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SysadminsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SysadminsConstants, "AVATAR_KEYS", ()), getattr(SysadminsConstants, "COVER_KEYS", ()))
