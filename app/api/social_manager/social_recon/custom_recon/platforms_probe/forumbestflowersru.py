...
from api.social_manager.social_recon.constants.platform_constants import ForumBestflowersRuConstants

constants = ForumBestflowersRuConstants


def probe_url(username: str) -> str:
    return ForumBestflowersRuConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForumBestflowersRuConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumBestflowersRuConstants, "AVATAR_KEYS", ()), getattr(ForumBestflowersRuConstants, "COVER_KEYS", ()))
