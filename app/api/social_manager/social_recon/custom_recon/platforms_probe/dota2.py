...
from api.social_manager.social_recon.constants.platform_constants import Dota2Constants

constants = Dota2Constants


def probe_url(username: str) -> str:
    return Dota2Constants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(Dota2Constants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(Dota2Constants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(Dota2Constants, "AVATAR_KEYS", ()), getattr(Dota2Constants, "COVER_KEYS", ()))
...
ROUTES = (
    (r"forum/search/?\?(?:.*&)?keywords=(?P<id>[^&]+)(?:&.*)?", "profile"),
)
