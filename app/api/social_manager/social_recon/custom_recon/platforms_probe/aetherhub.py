...
from api.social_manager.social_recon.constants.platform_constants import AetherhubConstants

constants = AetherhubConstants


def probe_url(username: str) -> str:
    return AetherhubConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(AetherhubConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(AetherhubConstants, "AVATAR_KEYS", ()), getattr(AetherhubConstants, "COVER_KEYS", ()))
