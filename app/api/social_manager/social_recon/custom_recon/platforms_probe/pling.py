...
def probe_url(username: str) -> str:
    return PlingConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PlingConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PlingConstants, "AVATAR_KEYS", ()), getattr(PlingConstants, "COVER_KEYS", ()))
