...
from api.social_manager.social_recon.constants.platform_constants import ValorantForumsConstants

constants = ValorantForumsConstants


def probe_url(username: str) -> str:
    return ValorantForumsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ValorantForumsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ValorantForumsConstants, "AVATAR_KEYS", ()), getattr(ValorantForumsConstants, "COVER_KEYS", ()))
