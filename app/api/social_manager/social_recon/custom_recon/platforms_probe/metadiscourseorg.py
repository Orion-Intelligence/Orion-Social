...
from api.social_manager.social_recon.constants.platform_constants import MetaDiscourseOrgConstants

constants = MetaDiscourseOrgConstants


def probe_url(username: str) -> str:
    return MetaDiscourseOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MetaDiscourseOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MetaDiscourseOrgConstants, "AVATAR_KEYS", ()), getattr(MetaDiscourseOrgConstants, "COVER_KEYS", ()))
