...
from api.social_manager.social_recon.constants.platform_constants import VelogConstants

constants = VelogConstants


def probe_url(username: str) -> str:
    return VelogConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(VelogConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(VelogConstants, "AVATAR_KEYS", ()), getattr(VelogConstants, "COVER_KEYS", ()))
