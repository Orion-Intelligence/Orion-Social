...
def probe_url(username: str) -> str:
    return MydramalistConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MydramalistConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MydramalistConstants, "AVATAR_KEYS", ()), getattr(MydramalistConstants, "COVER_KEYS", ()))
