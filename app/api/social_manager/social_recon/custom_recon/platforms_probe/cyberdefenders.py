...
from api.social_manager.social_recon.constants.platform_constants import CyberDefendersConstants

constants = CyberDefendersConstants


def probe_url(username: str) -> str:
    return CyberDefendersConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CyberDefendersConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CyberDefendersConstants, "AVATAR_KEYS", ()), getattr(CyberDefendersConstants, "COVER_KEYS", ()))
