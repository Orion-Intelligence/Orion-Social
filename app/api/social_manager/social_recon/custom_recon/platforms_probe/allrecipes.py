...
def probe_url(username: str) -> str:
    return AllRecipesConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(AllRecipesConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(AllRecipesConstants, "AVATAR_KEYS", ()), getattr(AllRecipesConstants, "COVER_KEYS", ()))
