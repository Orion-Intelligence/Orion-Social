...
def probe_url(username: str) -> str:
    return CodecademyConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CodecademyConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CodecademyConstants, "AVATAR_KEYS", ()), getattr(CodecademyConstants, "COVER_KEYS", ()))
