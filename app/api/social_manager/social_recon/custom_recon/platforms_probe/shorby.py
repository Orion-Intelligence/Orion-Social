...
from api.social_manager.social_recon.constants.platform_constants import ShorByConstants

constants = ShorByConstants


def probe_url(username: str) -> str:
    return ShorByConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ShorByConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ShorByConstants, "AVATAR_KEYS", ()), getattr(ShorByConstants, "COVER_KEYS", ()))
