...
from api.social_manager.social_recon.constants.platform_constants import OverclockersConstants

constants = OverclockersConstants


def probe_url(username: str) -> str:
    return OverclockersConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(OverclockersConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(OverclockersConstants, "AVATAR_KEYS", ()), getattr(OverclockersConstants, "COVER_KEYS", ()))
