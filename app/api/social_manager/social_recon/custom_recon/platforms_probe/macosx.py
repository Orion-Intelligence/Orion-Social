...
def probe_url(username: str) -> str:
    return MacosxConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MacosxConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MacosxConstants, "AVATAR_KEYS", ()), getattr(MacosxConstants, "COVER_KEYS", ()))
