...
def probe_url(username: str) -> str:
    return SlidesConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SlidesConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SlidesConstants, "AVATAR_KEYS", ()), getattr(SlidesConstants, "COVER_KEYS", ()))
