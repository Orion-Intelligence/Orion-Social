...
def probe_url(username: str) -> str:
    return NhattaoComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(NhattaoComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(NhattaoComConstants, "AVATAR_KEYS", ()), getattr(NhattaoComConstants, "COVER_KEYS", ()))
