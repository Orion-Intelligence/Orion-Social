...
from api.social_manager.social_recon.constants.platform_constants import BoardTtvchannelComConstants

constants = BoardTtvchannelComConstants


def probe_url(username: str) -> str:
    return BoardTtvchannelComConstants.API_URL.format(username=username)
...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    if not isinstance(user, dict) or not user.get("username"):
        return VerdictConstants.UNKNOWN, {}
    base = BoardTtvchannelComConstants.PROFILE_URL.split("/u/")[0]
