...
def probe_url(username: str) -> str:
    return RailforumsCoUkConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(RailforumsCoUkConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(RailforumsCoUkConstants, "AVATAR_KEYS", ()), getattr(RailforumsCoUkConstants, "COVER_KEYS", ()))
