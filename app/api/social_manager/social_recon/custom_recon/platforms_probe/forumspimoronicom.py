...
from api.social_manager.social_recon.constants.platform_constants import ForumsPimoroniComConstants

constants = ForumsPimoroniComConstants


def probe_url(username: str) -> str:
    return ForumsPimoroniComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForumsPimoroniComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumsPimoroniComConstants, "AVATAR_KEYS", ()), getattr(ForumsPimoroniComConstants, "COVER_KEYS", ()))
