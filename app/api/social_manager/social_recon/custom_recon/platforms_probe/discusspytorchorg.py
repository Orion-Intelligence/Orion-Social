...
from api.social_manager.social_recon.constants.platform_constants import DiscussPytorchOrgConstants

constants = DiscussPytorchOrgConstants


def probe_url(username: str) -> str:
    return DiscussPytorchOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DiscussPytorchOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DiscussPytorchOrgConstants, "AVATAR_KEYS", ()), getattr(DiscussPytorchOrgConstants, "COVER_KEYS", ()))
