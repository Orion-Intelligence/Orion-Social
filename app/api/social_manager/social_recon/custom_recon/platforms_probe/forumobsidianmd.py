...
from api.social_manager.social_recon.constants.platform_constants import ForumObsidianMdConstants

constants = ForumObsidianMdConstants


def probe_url(username: str) -> str:
    return ForumObsidianMdConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForumObsidianMdConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumObsidianMdConstants, "AVATAR_KEYS", ()), getattr(ForumObsidianMdConstants, "COVER_KEYS", ()))
