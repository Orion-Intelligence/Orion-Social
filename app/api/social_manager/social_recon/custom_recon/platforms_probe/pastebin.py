...
def probe_url(username: str) -> str:
    return PastebinConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PastebinConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PastebinConstants, "AVATAR_KEYS", ()), getattr(PastebinConstants, "COVER_KEYS", ()))
