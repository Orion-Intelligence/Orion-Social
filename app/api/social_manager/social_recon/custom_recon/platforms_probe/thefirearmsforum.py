...
def probe_url(username: str) -> str:
    return ThefirearmsforumConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ThefirearmsforumConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ThefirearmsforumConstants, "AVATAR_KEYS", ()), getattr(ThefirearmsforumConstants, "COVER_KEYS", ()))
