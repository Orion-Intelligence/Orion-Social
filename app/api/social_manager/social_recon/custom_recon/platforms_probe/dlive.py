...
def probe_url(username: str) -> str:
    return DLiveConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DLiveConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DLiveConstants, "AVATAR_KEYS", ()), getattr(DLiveConstants, "COVER_KEYS", ()))
