...
def probe_url(username: str) -> str:
    return WeasylConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WeasylConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WeasylConstants, "AVATAR_KEYS", ()), getattr(WeasylConstants, "COVER_KEYS", ()))
