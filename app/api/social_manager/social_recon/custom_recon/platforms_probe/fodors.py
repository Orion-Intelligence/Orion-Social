...
def probe_url(username: str) -> str:
    return FodorsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FodorsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FodorsConstants, "AVATAR_KEYS", ()), getattr(FodorsConstants, "COVER_KEYS", ()))
