...
def probe_url(username: str) -> str:
    return HomebrewtalkComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(HomebrewtalkComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(HomebrewtalkComConstants, "AVATAR_KEYS", ()), getattr(HomebrewtalkComConstants, "COVER_KEYS", ()))
