...
def probe_url(username: str) -> str:
    return NationStatesNationConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(NationStatesNationConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(NationStatesNationConstants, "AVATAR_KEYS", ()), getattr(NationStatesNationConstants, "COVER_KEYS", ()))
