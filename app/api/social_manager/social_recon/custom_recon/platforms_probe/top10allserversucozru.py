...
from api.social_manager.social_recon.constants.platform_constants import Top10allserversUcozRuConstants

constants = Top10allserversUcozRuConstants


def probe_url(username: str) -> str:
    return Top10allserversUcozRuConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(Top10allserversUcozRuConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(Top10allserversUcozRuConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(Top10allserversUcozRuConstants, "AVATAR_KEYS", ()), getattr(Top10allserversUcozRuConstants, "COVER_KEYS", ()))
