...
def probe_url(username: str) -> str:
    return LobstersConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(LobstersConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(LobstersConstants, "AVATAR_KEYS", ()), getattr(LobstersConstants, "COVER_KEYS", ()))
