...
from api.social_manager.social_recon.constants.platform_constants import PartyvibeConstants

constants = PartyvibeConstants


def probe_url(username: str) -> str:
    return PartyvibeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PartyvibeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PartyvibeConstants, "AVATAR_KEYS", ()), getattr(PartyvibeConstants, "COVER_KEYS", ()))
