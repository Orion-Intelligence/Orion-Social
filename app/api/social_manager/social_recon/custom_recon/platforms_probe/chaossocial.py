...
from api.social_manager.social_recon.constants.platform_constants import ChaosSocialConstants

constants = ChaosSocialConstants


def probe_url(username: str) -> str:
    return ChaosSocialConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ChaosSocialConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ChaosSocialConstants, "AVATAR_KEYS", ()), getattr(ChaosSocialConstants, "COVER_KEYS", ()))
