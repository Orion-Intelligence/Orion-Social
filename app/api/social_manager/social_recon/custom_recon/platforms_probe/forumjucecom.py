...
from api.social_manager.social_recon.constants.platform_constants import ForumJuceComConstants

constants = ForumJuceComConstants


def probe_url(username: str) -> str:
    return ForumJuceComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForumJuceComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumJuceComConstants, "AVATAR_KEYS", ()), getattr(ForumJuceComConstants, "COVER_KEYS", ()))
