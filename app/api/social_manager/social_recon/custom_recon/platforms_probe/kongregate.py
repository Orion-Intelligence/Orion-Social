...
def probe_url(username: str) -> str:
    return KongregateConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(KongregateConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(KongregateConstants, "AVATAR_KEYS", ()), getattr(KongregateConstants, "COVER_KEYS", ()))
