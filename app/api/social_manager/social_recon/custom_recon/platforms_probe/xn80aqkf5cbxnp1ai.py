...
from api.social_manager.social_recon.constants.platform_constants import Xn80aqkf5cbXnP1aiConstants

constants = Xn80aqkf5cbXnP1aiConstants


def probe_url(username: str) -> str:
    return Xn80aqkf5cbXnP1aiConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(Xn80aqkf5cbXnP1aiConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(Xn80aqkf5cbXnP1aiConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(Xn80aqkf5cbXnP1aiConstants, "AVATAR_KEYS", ()), getattr(Xn80aqkf5cbXnP1aiConstants, "COVER_KEYS", ()))
