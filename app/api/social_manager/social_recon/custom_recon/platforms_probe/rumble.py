...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in ("rumble", "404 not found"):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, RumbleConstants.AVATAR_KEYS, RumbleConstants.COVER_KEYS)
    info["display_name"] = parse.clean(info.get("display_name") or heading)
    if info.get("description"):
        info["description"] = parse.clean(info["description"])
    followers = re.search(r"([\d.,]+[KMB]?)\s+Followers", body)
    if followers:
