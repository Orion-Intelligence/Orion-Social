...
from api.social_manager.social_recon.constants.platform_constants import SpletnikConstants

constants = SpletnikConstants


def probe_url(username: str) -> str:
    return SpletnikConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SpletnikConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SpletnikConstants, "AVATAR_KEYS", ()), getattr(SpletnikConstants, "COVER_KEYS", ()))
