...
from api.social_manager.social_recon.constants.platform_constants import WeedmapsConstants

constants = WeedmapsConstants


def probe_url(username: str) -> str:
    return WeedmapsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WeedmapsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WeedmapsConstants, "AVATAR_KEYS", ()), getattr(WeedmapsConstants, "COVER_KEYS", ()))
