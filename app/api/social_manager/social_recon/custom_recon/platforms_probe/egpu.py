...
def probe_url(username: str) -> str:
    return EGPUConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(EGPUConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(EGPUConstants, "AVATAR_KEYS", ()), getattr(EGPUConstants, "COVER_KEYS", ()))
