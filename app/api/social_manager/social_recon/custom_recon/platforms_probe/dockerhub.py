...
from api.social_manager.social_recon.constants.platform_constants import DockerHubConstants

constants = DockerHubConstants


def probe_url(username: str) -> str:
    return DockerHubConstants.API_URL.format(username=username)
...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    if not (isinstance(payload, dict) and payload.get("id") and (payload.get("username") or payload.get("orgname"))):
        return VerdictConstants.UNKNOWN, {}
    info = {
        "display_name": parse.clean(payload.get("full_name") or payload.get("username") or payload.get("orgname")),
        "username": parse.text(payload.get("username") or payload.get("orgname")),
        "avatar": parse.text(payload.get("gravatar_url")),
        "id": parse.text(payload.get("id")),
        "created_at": parse.text(payload.get("date_joined")),
        "location": parse.clean(payload.get("location")),
        "company": parse.clean(payload.get("company")),
        "website": parse.text(payload.get("profile_url")),
