...
def probe_url(username: str) -> str:
    return GloriaTvConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(GloriaTvConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(GloriaTvConstants, "AVATAR_KEYS", ()), getattr(GloriaTvConstants, "COVER_KEYS", ()))
