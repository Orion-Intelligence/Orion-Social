...
def probe_url(username: str) -> str:
    return MeetMeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MeetMeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MeetMeConstants, "AVATAR_KEYS", ()), getattr(MeetMeConstants, "COVER_KEYS", ()))
