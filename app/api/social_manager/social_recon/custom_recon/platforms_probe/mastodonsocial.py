...
from api.social_manager.social_recon.constants.platform_constants import MastodonSocialConstants

constants = MastodonSocialConstants


def probe_url(username: str) -> str:
    return MastodonSocialConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MastodonSocialConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MastodonSocialConstants, "AVATAR_KEYS", ()), getattr(MastodonSocialConstants, "COVER_KEYS", ()))
