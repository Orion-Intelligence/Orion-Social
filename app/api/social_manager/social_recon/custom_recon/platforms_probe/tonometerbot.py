...
from api.social_manager.social_recon.constants.platform_constants import TonometerbotConstants

constants = TonometerbotConstants


def probe_url(username: str) -> str:
    return TonometerbotConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TonometerbotConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TonometerbotConstants, "AVATAR_KEYS", ()), getattr(TonometerbotConstants, "COVER_KEYS", ()))
