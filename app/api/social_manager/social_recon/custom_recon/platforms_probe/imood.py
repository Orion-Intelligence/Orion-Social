...
def probe_url(username: str) -> str:
    return ImoodConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ImoodConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ImoodConstants, "AVATAR_KEYS", ()), getattr(ImoodConstants, "COVER_KEYS", ()))
