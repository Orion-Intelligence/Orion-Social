...
def probe_url(username: str) -> str:
    return ArmorgamesConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ArmorgamesConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ArmorgamesConstants, "AVATAR_KEYS", ()), getattr(ArmorgamesConstants, "COVER_KEYS", ()))
