...
def probe_url(username: str) -> str:
    return GoodgameRuConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(GoodgameRuConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(GoodgameRuConstants, "AVATAR_KEYS", ()), getattr(GoodgameRuConstants, "COVER_KEYS", ()))
