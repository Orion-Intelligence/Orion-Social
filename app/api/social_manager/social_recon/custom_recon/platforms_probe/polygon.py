...
def probe_url(username: str) -> str:
    return PolygonConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PolygonConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PolygonConstants, "AVATAR_KEYS", ()), getattr(PolygonConstants, "COVER_KEYS", ()))
