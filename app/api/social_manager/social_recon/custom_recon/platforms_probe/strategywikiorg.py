...
from api.social_manager.social_recon.constants.platform_constants import StrategywikiOrgConstants

constants = StrategywikiOrgConstants


def probe_url(username: str) -> str:
    return StrategywikiOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(StrategywikiOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(StrategywikiOrgConstants, "AVATAR_KEYS", ()), getattr(StrategywikiOrgConstants, "COVER_KEYS", ()))
