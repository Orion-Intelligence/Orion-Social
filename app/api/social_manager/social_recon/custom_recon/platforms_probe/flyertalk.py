...
def probe_url(username: str) -> str:
    return FlyertalkConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FlyertalkConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FlyertalkConstants, "AVATAR_KEYS", ()), getattr(FlyertalkConstants, "COVER_KEYS", ()))
