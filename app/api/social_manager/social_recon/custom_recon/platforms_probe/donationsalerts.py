...
from api.social_manager.social_recon.constants.platform_constants import DonationsAlertsConstants

constants = DonationsAlertsConstants


def probe_url(username: str) -> str:
    return DonationsAlertsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DonationsAlertsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DonationsAlertsConstants, "AVATAR_KEYS", ()), getattr(DonationsAlertsConstants, "COVER_KEYS", ()))
