...
def probe_url(username: str) -> str:
    return NucastleCoUkConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(NucastleCoUkConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(NucastleCoUkConstants, "AVATAR_KEYS", ()), getattr(NucastleCoUkConstants, "COVER_KEYS", ()))
