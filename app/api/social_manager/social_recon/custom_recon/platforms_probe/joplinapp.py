...
from api.social_manager.social_recon.constants.platform_constants import JoplinAppConstants

constants = JoplinAppConstants


def probe_url(username: str) -> str:
    return JoplinAppConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(JoplinAppConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(JoplinAppConstants, "AVATAR_KEYS", ()), getattr(JoplinAppConstants, "COVER_KEYS", ()))
