...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in RubyGemsConstants.GENERIC:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, RubyGemsConstants.AVATAR_KEYS, RubyGemsConstants.COVER_KEYS)
    match = re.search(r"Profile of (.+?)\s*\|", heading)
    name = parse.clean(match.group(1)) if match else parse.clean(heading.split(" |")[0])
    if name:
