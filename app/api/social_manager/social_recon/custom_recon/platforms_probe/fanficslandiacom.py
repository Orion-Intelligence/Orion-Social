...
def probe_url(username: str) -> str:
    return FanficslandiaComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FanficslandiaComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FanficslandiaComConstants, "AVATAR_KEYS", ()), getattr(FanficslandiaComConstants, "COVER_KEYS", ()))
