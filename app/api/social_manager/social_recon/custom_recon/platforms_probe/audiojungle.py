...
def probe_url(username: str) -> str:
    return AudiojungleConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(AudiojungleConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(AudiojungleConstants, "AVATAR_KEYS", ()), getattr(AudiojungleConstants, "COVER_KEYS", ()))
