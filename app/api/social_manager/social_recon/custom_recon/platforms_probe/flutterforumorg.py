...
from api.social_manager.social_recon.constants.platform_constants import FlutterforumOrgConstants

constants = FlutterforumOrgConstants


def probe_url(username: str) -> str:
    return FlutterforumOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FlutterforumOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FlutterforumOrgConstants, "AVATAR_KEYS", ()), getattr(FlutterforumOrgConstants, "COVER_KEYS", ()))
