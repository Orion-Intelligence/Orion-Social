...
from api.social_manager.social_recon.constants.platform_constants import IphonesRuConstants

constants = IphonesRuConstants


def probe_url(username: str) -> str:
    return IphonesRuConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(IphonesRuConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(IphonesRuConstants, "AVATAR_KEYS", ()), getattr(IphonesRuConstants, "COVER_KEYS", ()))
