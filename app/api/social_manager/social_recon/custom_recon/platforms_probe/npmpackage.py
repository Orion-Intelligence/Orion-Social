...
from api.social_manager.social_recon.constants.platform_constants import NPMPackageConstants

constants = NPMPackageConstants


def probe_url(username: str) -> str:
    return NPMPackageConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(NPMPackageConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(NPMPackageConstants, "AVATAR_KEYS", ()), getattr(NPMPackageConstants, "COVER_KEYS", ()))
