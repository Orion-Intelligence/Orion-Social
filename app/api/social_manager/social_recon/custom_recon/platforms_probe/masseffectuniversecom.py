...
from api.social_manager.social_recon.constants.platform_constants import MasseffectUniverseComConstants

constants = MasseffectUniverseComConstants


def probe_url(username: str) -> str:
    return MasseffectUniverseComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MasseffectUniverseComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MasseffectUniverseComConstants, "AVATAR_KEYS", ()), getattr(MasseffectUniverseComConstants, "COVER_KEYS", ()))
