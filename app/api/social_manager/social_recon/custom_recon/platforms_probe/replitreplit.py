...
def probe_url(username: str) -> str:
    return ReplItConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ReplItConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ReplItConstants, "AVATAR_KEYS", ()), getattr(ReplItConstants, "COVER_KEYS", ()))
