...
def probe_url(username: str) -> str:
    return ReibertConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ReibertConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ReibertConstants, "AVATAR_KEYS", ()), getattr(ReibertConstants, "COVER_KEYS", ()))
