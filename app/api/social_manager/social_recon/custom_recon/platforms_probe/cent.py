...
def probe_url(username: str) -> str:
    return CentConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CentConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CentConstants, "AVATAR_KEYS", ()), getattr(CentConstants, "COVER_KEYS", ()))
