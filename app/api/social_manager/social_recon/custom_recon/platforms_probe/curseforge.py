...
from api.social_manager.social_recon.constants.platform_constants import CurseForgeConstants

constants = CurseForgeConstants


def probe_url(username: str) -> str:
    return CurseForgeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CurseForgeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CurseForgeConstants, "AVATAR_KEYS", ()), getattr(CurseForgeConstants, "COVER_KEYS", ()))
...
ROUTES = (
    (r"members/(?P<id>[^/]+)/projects", "profile"),
)
