...
def probe_url(username: str) -> str:
    return SwedroidSeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SwedroidSeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SwedroidSeConstants, "AVATAR_KEYS", ()), getattr(SwedroidSeConstants, "COVER_KEYS", ()))
