...
from api.social_manager.social_recon.constants.platform_constants import RcloneForumConstants

constants = RcloneForumConstants


def probe_url(username: str) -> str:
    return RcloneForumConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(RcloneForumConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(RcloneForumConstants, "AVATAR_KEYS", ()), getattr(RcloneForumConstants, "COVER_KEYS", ()))
