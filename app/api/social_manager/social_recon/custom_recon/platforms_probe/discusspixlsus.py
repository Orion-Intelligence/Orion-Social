...
from api.social_manager.social_recon.constants.platform_constants import DiscussPixlsUsConstants

constants = DiscussPixlsUsConstants


def probe_url(username: str) -> str:
    return DiscussPixlsUsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DiscussPixlsUsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DiscussPixlsUsConstants, "AVATAR_KEYS", ()), getattr(DiscussPixlsUsConstants, "COVER_KEYS", ()))
