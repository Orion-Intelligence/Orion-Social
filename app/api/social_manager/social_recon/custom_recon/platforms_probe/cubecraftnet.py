...
def probe_url(username: str) -> str:
    return CubecraftNetConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CubecraftNetConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CubecraftNetConstants, "AVATAR_KEYS", ()), getattr(CubecraftNetConstants, "COVER_KEYS", ()))
