...
from api.social_manager.social_recon.constants.platform_constants import XShakerConstants

constants = XShakerConstants


def probe_url(username: str) -> str:
    return XShakerConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(XShakerConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(XShakerConstants, "AVATAR_KEYS", ()), getattr(XShakerConstants, "COVER_KEYS", ()))
