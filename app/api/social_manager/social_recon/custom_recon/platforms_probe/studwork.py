...
from api.social_manager.social_recon.constants.platform_constants import StudworkConstants

constants = StudworkConstants


def probe_url(username: str) -> str:
    return StudworkConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(StudworkConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(StudworkConstants, "AVATAR_KEYS", ()), getattr(StudworkConstants, "COVER_KEYS", ()))
