...
def probe_url(username: str) -> str:
    return TyperacerConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TyperacerConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TyperacerConstants, "AVATAR_KEYS", ()), getattr(TyperacerConstants, "COVER_KEYS", ()))
