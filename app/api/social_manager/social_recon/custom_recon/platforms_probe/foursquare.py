...
def probe_url(username: str) -> str:
    return FoursquareConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FoursquareConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FoursquareConstants, "AVATAR_KEYS", ()), getattr(FoursquareConstants, "COVER_KEYS", ()))
