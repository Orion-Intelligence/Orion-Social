...
def probe_url(username: str) -> str:
    return CfdOnlineConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CfdOnlineConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CfdOnlineConstants, "AVATAR_KEYS", ()), getattr(CfdOnlineConstants, "COVER_KEYS", ()))
