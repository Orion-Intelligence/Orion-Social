...
from api.social_manager.social_recon.constants.platform_constants import TunaConstants

constants = TunaConstants


def probe_url(username: str) -> str:
    return TunaConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TunaConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TunaConstants, "AVATAR_KEYS", ()), getattr(TunaConstants, "COVER_KEYS", ()))
