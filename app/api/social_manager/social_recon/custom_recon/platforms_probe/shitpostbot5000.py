...
from api.social_manager.social_recon.constants.platform_constants import ShitpostBot5000Constants

constants = ShitpostBot5000Constants


def probe_url(username: str) -> str:
    return ShitpostBot5000Constants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ShitpostBot5000Constants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ShitpostBot5000Constants, "AVATAR_KEYS", ()), getattr(ShitpostBot5000Constants, "COVER_KEYS", ()))
