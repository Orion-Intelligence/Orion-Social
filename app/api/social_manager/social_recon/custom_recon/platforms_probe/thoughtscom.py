...
def probe_url(username: str) -> str:
    return ThoughtsComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ThoughtsComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ThoughtsComConstants, "AVATAR_KEYS", ()), getattr(ThoughtsComConstants, "COVER_KEYS", ()))
