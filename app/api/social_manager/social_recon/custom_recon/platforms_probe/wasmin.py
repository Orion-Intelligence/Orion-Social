...
from api.social_manager.social_recon.constants.platform_constants import WasmInConstants

constants = WasmInConstants


def probe_url(username: str) -> str:
    return WasmInConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WasmInConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WasmInConstants, "AVATAR_KEYS", ()), getattr(WasmInConstants, "COVER_KEYS", ()))
