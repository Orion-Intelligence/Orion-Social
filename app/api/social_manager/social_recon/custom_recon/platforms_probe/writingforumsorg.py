...
def probe_url(username: str) -> str:
    return WritingforumsOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WritingforumsOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WritingforumsOrgConstants, "AVATAR_KEYS", ()), getattr(WritingforumsOrgConstants, "COVER_KEYS", ()))
