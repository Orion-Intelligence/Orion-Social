...
from api.social_manager.social_recon.constants.platform_constants import MixConstants

constants = MixConstants


def probe_url(username: str) -> str:
    return MixConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MixConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MixConstants, "AVATAR_KEYS", ()), getattr(MixConstants, "COVER_KEYS", ()))
