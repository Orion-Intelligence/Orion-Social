...
from api.social_manager.social_recon.constants.platform_constants import DealabsConstants

constants = DealabsConstants


def probe_url(username: str) -> str:
    return DealabsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DealabsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DealabsConstants, "AVATAR_KEYS", ()), getattr(DealabsConstants, "COVER_KEYS", ()))
