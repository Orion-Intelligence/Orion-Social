...
from api.social_manager.social_recon.constants.platform_constants import Suomi24Constants

constants = Suomi24Constants


def probe_url(username: str) -> str:
    return Suomi24Constants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(Suomi24Constants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(Suomi24Constants, "AVATAR_KEYS", ()), getattr(Suomi24Constants, "COVER_KEYS", ()))
