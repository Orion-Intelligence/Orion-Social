...
from api.social_manager.social_recon.constants.platform_constants import InfourokConstants

constants = InfourokConstants


def probe_url(username: str) -> str:
    return InfourokConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(InfourokConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(InfourokConstants, "AVATAR_KEYS", ()), getattr(InfourokConstants, "COVER_KEYS", ()))
