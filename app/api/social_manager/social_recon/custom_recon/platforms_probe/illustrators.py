...
from api.social_manager.social_recon.constants.platform_constants import IllustratorsConstants

constants = IllustratorsConstants


def probe_url(username: str) -> str:
    return IllustratorsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(IllustratorsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(IllustratorsConstants, "AVATAR_KEYS", ()), getattr(IllustratorsConstants, "COVER_KEYS", ()))
