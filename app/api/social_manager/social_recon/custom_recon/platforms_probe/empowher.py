...
from api.social_manager.social_recon.constants.platform_constants import EmpowherConstants

constants = EmpowherConstants


def probe_url(username: str) -> str:
    return EmpowherConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(EmpowherConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(EmpowherConstants, "AVATAR_KEYS", ()), getattr(EmpowherConstants, "COVER_KEYS", ()))
