...
from api.social_manager.social_recon.constants.platform_constants import VgtimesConstants

constants = VgtimesConstants


def probe_url(username: str) -> str:
    return VgtimesConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(VgtimesConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(VgtimesConstants, "AVATAR_KEYS", ()), getattr(VgtimesConstants, "COVER_KEYS", ()))
