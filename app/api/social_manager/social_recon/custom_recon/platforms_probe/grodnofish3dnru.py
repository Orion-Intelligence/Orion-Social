...
from api.social_manager.social_recon.constants.platform_constants import Grodnofish3dnRuConstants

constants = Grodnofish3dnRuConstants


def probe_url(username: str) -> str:
    return Grodnofish3dnRuConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(Grodnofish3dnRuConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(Grodnofish3dnRuConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(Grodnofish3dnRuConstants, "AVATAR_KEYS", ()), getattr(Grodnofish3dnRuConstants, "COVER_KEYS", ()))
