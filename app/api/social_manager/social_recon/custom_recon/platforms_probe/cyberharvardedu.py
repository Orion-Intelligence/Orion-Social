...
from api.social_manager.social_recon.constants.platform_constants import CyberHarvardEduConstants

constants = CyberHarvardEduConstants


def probe_url(username: str) -> str:
    return CyberHarvardEduConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CyberHarvardEduConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CyberHarvardEduConstants, "AVATAR_KEYS", ()), getattr(CyberHarvardEduConstants, "COVER_KEYS", ()))
