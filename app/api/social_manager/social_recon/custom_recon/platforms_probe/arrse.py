...
def probe_url(username: str) -> str:
    return ArrseConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ArrseConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ArrseConstants, "AVATAR_KEYS", ()), getattr(ArrseConstants, "COVER_KEYS", ()))
