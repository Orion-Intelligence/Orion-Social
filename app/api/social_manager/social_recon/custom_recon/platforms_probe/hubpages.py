...
def probe_url(username: str) -> str:
    return HubPagesConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(HubPagesConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(HubPagesConstants, "AVATAR_KEYS", ()), getattr(HubPagesConstants, "COVER_KEYS", ()))
