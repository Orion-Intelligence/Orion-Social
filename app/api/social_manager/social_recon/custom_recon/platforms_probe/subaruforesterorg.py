...
def probe_url(username: str) -> str:
    return SubaruforesterOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SubaruforesterOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SubaruforesterOrgConstants, "AVATAR_KEYS", ()), getattr(SubaruforesterOrgConstants, "COVER_KEYS", ()))
