...
from api.social_manager.social_recon.constants.platform_constants import VirgoolConstants

constants = VirgoolConstants


def probe_url(username: str) -> str:
    return VirgoolConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(VirgoolConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(VirgoolConstants, "AVATAR_KEYS", ()), getattr(VirgoolConstants, "COVER_KEYS", ()))
