...
def probe_url(username: str) -> str:
    return PokecommunityConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PokecommunityConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PokecommunityConstants, "AVATAR_KEYS", ()), getattr(PokecommunityConstants, "COVER_KEYS", ()))
