...
def probe_url(username: str) -> str:
    return EthresearConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(EthresearConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(EthresearConstants, "AVATAR_KEYS", ()), getattr(EthresearConstants, "COVER_KEYS", ()))
