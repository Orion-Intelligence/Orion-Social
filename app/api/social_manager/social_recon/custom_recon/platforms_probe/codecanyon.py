...
def probe_url(username: str) -> str:
    return CodecanyonConstants.PROFILE_URL.format(username=username)


def fetch(username: str) -> tuple[int, str, str]:
    return http_client.fetch(probe_url(username), impersonate=CodecanyonConstants.IMPERSONATE)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CodecanyonConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CodecanyonConstants, "AVATAR_KEYS", ()), getattr(CodecanyonConstants, "COVER_KEYS", ()))
