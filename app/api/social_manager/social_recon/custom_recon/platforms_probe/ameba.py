...
from api.social_manager.social_recon.constants.platform_constants import AmebaConstants

constants = AmebaConstants


def probe_url(username: str) -> str:
    return AmebaConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(AmebaConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(AmebaConstants, "AVATAR_KEYS", ()), getattr(AmebaConstants, "COVER_KEYS", ()))
    ...
    if info.get("display_name"):
        info["display_name"] = parse.clean(re.sub(r"さんのプロフィールページ$|のプロフィールページ$", "", info["display_name"]))
