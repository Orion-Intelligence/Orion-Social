...
from api.social_manager.social_recon.constants.platform_constants import CTFtimeConstants

constants = CTFtimeConstants


def probe_url(username: str) -> str:
    return CTFtimeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CTFtimeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CTFtimeConstants, "AVATAR_KEYS", ()), getattr(CTFtimeConstants, "COVER_KEYS", ()))
