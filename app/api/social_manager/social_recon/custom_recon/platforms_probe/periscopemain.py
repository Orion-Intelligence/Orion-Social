...
from api.social_manager.social_recon.constants.platform_constants import PeriscopeMainConstants

constants = PeriscopeMainConstants


def probe_url(username: str) -> str:
    return PeriscopeMainConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PeriscopeMainConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PeriscopeMainConstants, "AVATAR_KEYS", ()), getattr(PeriscopeMainConstants, "COVER_KEYS", ()))
