...
from api.social_manager.social_recon.constants.platform_constants import StereoConstants

constants = StereoConstants


def probe_url(username: str) -> str:
    return StereoConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(StereoConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(StereoConstants, "AVATAR_KEYS", ()), getattr(StereoConstants, "COVER_KEYS", ()))
