...
def probe_url(username: str) -> str:
    return DatingRuConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DatingRuConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DatingRuConstants, "AVATAR_KEYS", ()), getattr(DatingRuConstants, "COVER_KEYS", ()))
