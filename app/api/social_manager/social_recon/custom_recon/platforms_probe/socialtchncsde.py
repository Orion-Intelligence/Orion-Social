...
from api.social_manager.social_recon.constants.platform_constants import SocialTchncsDeConstants

constants = SocialTchncsDeConstants


def probe_url(username: str) -> str:
    return SocialTchncsDeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SocialTchncsDeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SocialTchncsDeConstants, "AVATAR_KEYS", ()), getattr(SocialTchncsDeConstants, "COVER_KEYS", ()))
