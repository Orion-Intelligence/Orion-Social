...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    info = parse.social_info(body, LastFmConstants.AVATAR_KEYS, LastFmConstants.COVER_KEYS)
    name = re.match(LastFmConstants.NAME_RE, parse.text(meta.get("og:title")))
    if name:
        info["display_name"] = parse.clean(name.group(1))
    description = parse.text(meta.get("og:description"))
    scrobbles = re.search(LastFmConstants.SCROBBLES, description)
    if scrobbles:
        info["scrobbles"] = scrobbles.group(1)
    top = re.search(LastFmConstants.TOP_ARTISTS, description)
    if top:
        info["top_artists"] = parse.clean(top.group(1))
    info["description"] = ""
