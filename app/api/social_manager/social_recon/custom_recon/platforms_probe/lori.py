...
from api.social_manager.social_recon.constants.platform_constants import LoriConstants

constants = LoriConstants


def probe_url(username: str) -> str:
    return LoriConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(LoriConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(LoriConstants, "AVATAR_KEYS", ()), getattr(LoriConstants, "COVER_KEYS", ()))
