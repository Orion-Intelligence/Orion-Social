...
from api.social_manager.social_recon.constants.platform_constants import FlirticEeConstants

constants = FlirticEeConstants


def probe_url(username: str) -> str:
    return FlirticEeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FlirticEeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FlirticEeConstants, "AVATAR_KEYS", ()), getattr(FlirticEeConstants, "COVER_KEYS", ()))
