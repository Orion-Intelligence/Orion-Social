...
def probe_url(username: str) -> str:
    return ModDBConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ModDBConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ModDBConstants, "AVATAR_KEYS", ()), getattr(ModDBConstants, "COVER_KEYS", ()))
