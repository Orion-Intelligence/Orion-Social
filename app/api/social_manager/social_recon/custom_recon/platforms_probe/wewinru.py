...
from api.social_manager.social_recon.constants.platform_constants import WewinRuConstants

constants = WewinRuConstants


def probe_url(username: str) -> str:
    return WewinRuConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WewinRuConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WewinRuConstants, "AVATAR_KEYS", ()), getattr(WewinRuConstants, "COVER_KEYS", ()))
