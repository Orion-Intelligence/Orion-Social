...
from api.social_manager.social_recon.constants.platform_constants import NhlConstants

constants = NhlConstants


def probe_url(username: str) -> str:
    return NhlConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(NhlConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(NhlConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(NhlConstants, "AVATAR_KEYS", ()), getattr(NhlConstants, "COVER_KEYS", ()))
...
ROUTES = (
    (r"talks/profile\.php/?\?(?:.*&)?u=(?P<id>[^&]+)(?:&.*)?", "profile"),
)
