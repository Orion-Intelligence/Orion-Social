...
def probe_url(username: str) -> str:
    return NothingCommunityConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(NothingCommunityConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(NothingCommunityConstants, "AVATAR_KEYS", ()), getattr(NothingCommunityConstants, "COVER_KEYS", ()))
