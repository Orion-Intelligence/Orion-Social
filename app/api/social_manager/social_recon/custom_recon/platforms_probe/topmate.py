...
def probe_url(username: str) -> str:
    return TopmateConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TopmateConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TopmateConstants, "AVATAR_KEYS", ()), getattr(TopmateConstants, "COVER_KEYS", ()))
