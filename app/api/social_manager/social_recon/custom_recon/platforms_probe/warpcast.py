...
def probe_url(username: str) -> str:
    return WarpcastConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WarpcastConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WarpcastConstants, "AVATAR_KEYS", ()), getattr(WarpcastConstants, "COVER_KEYS", ()))
