...
def probe_url(username: str) -> str:
    return ClubhouseConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ClubhouseConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ClubhouseConstants, "AVATAR_KEYS", ()), getattr(ClubhouseConstants, "COVER_KEYS", ()))
