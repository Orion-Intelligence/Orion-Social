...
from api.social_manager.social_recon.constants.platform_constants import SteemitConstants

constants = SteemitConstants


def probe_url(username: str) -> str:
    return SteemitConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SteemitConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SteemitConstants, "AVATAR_KEYS", ()), getattr(SteemitConstants, "COVER_KEYS", ()))
