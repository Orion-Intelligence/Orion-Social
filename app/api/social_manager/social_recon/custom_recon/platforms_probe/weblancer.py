...
def probe_url(username: str) -> str:
    return WeblancerConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WeblancerConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WeblancerConstants, "AVATAR_KEYS", ()), getattr(WeblancerConstants, "COVER_KEYS", ()))
