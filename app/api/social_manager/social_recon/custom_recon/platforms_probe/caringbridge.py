...
from api.social_manager.social_recon.constants.platform_constants import CaringbridgeConstants

constants = CaringbridgeConstants


def probe_url(username: str) -> str:
    return CaringbridgeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CaringbridgeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CaringbridgeConstants, "AVATAR_KEYS", ()), getattr(CaringbridgeConstants, "COVER_KEYS", ()))
