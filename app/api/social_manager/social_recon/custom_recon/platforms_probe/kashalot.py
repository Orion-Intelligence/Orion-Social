...
from api.social_manager.social_recon.constants.platform_constants import KashalotConstants

constants = KashalotConstants


def probe_url(username: str) -> str:
    return KashalotConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(KashalotConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(KashalotConstants, "AVATAR_KEYS", ()), getattr(KashalotConstants, "COVER_KEYS", ()))
