...
from api.social_manager.social_recon.constants.platform_constants import ForumsTNationComConstants

constants = ForumsTNationComConstants


def probe_url(username: str) -> str:
    return ForumsTNationComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForumsTNationComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumsTNationComConstants, "AVATAR_KEYS", ()), getattr(ForumsTNationComConstants, "COVER_KEYS", ()))
