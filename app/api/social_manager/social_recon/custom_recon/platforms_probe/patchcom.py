...
from api.social_manager.social_recon.constants.platform_constants import PatchComConstants

constants = PatchComConstants


def probe_url(username: str) -> str:
    return PatchComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PatchComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PatchComConstants, "AVATAR_KEYS", ()), getattr(PatchComConstants, "COVER_KEYS", ()))
