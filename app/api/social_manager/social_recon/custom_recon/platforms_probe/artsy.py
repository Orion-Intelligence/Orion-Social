...
from api.social_manager.social_recon.constants.platform_constants import ArtsyConstants

constants = ArtsyConstants


def probe_url(username: str) -> str:
    return ArtsyConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ArtsyConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ArtsyConstants, "AVATAR_KEYS", ()), getattr(ArtsyConstants, "COVER_KEYS", ()))
