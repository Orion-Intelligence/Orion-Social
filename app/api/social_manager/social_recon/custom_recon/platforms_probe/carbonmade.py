...
from api.social_manager.social_recon.constants.platform_constants import CarbonmadeConstants

constants = CarbonmadeConstants


def probe_url(username: str) -> str:
    return CarbonmadeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CarbonmadeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CarbonmadeConstants, "AVATAR_KEYS", ()), getattr(CarbonmadeConstants, "COVER_KEYS", ()))
...
HOSTS = ("carbonmade.com",)
SUBDOMAIN = ("carbonmade.com", "profile")
ROUTES = ()
