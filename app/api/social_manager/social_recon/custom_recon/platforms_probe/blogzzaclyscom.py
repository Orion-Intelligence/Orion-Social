...
from api.social_manager.social_recon.constants.platform_constants import BlogzZaclysComConstants

constants = BlogzZaclysComConstants


def probe_url(username: str) -> str:
    return BlogzZaclysComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(BlogzZaclysComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(BlogzZaclysComConstants, "AVATAR_KEYS", ()), getattr(BlogzZaclysComConstants, "COVER_KEYS", ()))
