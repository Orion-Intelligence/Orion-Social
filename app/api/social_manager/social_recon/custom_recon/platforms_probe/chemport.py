...
def probe_url(username: str) -> str:
    return ChemportConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ChemportConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ChemportConstants, "AVATAR_KEYS", ()), getattr(ChemportConstants, "COVER_KEYS", ()))
