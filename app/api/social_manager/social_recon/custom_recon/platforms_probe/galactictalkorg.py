...
from api.social_manager.social_recon.constants.platform_constants import GalactictalkOrgConstants

constants = GalactictalkOrgConstants


def probe_url(username: str) -> str:
    return GalactictalkOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(GalactictalkOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(GalactictalkOrgConstants, "AVATAR_KEYS", ()), getattr(GalactictalkOrgConstants, "COVER_KEYS", ()))
