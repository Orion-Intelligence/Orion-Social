...
def probe_url(username: str) -> str:
    return PeriscopeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PeriscopeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PeriscopeConstants, "AVATAR_KEYS", ()), getattr(PeriscopeConstants, "COVER_KEYS", ()))
