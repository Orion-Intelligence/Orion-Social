...
def probe_url(username: str) -> str:
    return TexasguntalkConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TexasguntalkConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TexasguntalkConstants, "AVATAR_KEYS", ()), getattr(TexasguntalkConstants, "COVER_KEYS", ()))
