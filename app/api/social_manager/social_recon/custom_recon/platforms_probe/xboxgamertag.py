...
from api.social_manager.social_recon.constants.platform_constants import XboxGamertagConstants

constants = XboxGamertagConstants


def probe_url(username: str) -> str:
    return XboxGamertagConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(XboxGamertagConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(XboxGamertagConstants, "AVATAR_KEYS", ()), getattr(XboxGamertagConstants, "COVER_KEYS", ()))
