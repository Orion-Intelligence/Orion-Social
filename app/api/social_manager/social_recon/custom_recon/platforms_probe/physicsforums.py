...
from api.social_manager.social_recon.constants.platform_constants import PhysicsforumsConstants

constants = PhysicsforumsConstants


def probe_url(username: str) -> str:
    return PhysicsforumsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PhysicsforumsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PhysicsforumsConstants, "AVATAR_KEYS", ()), getattr(PhysicsforumsConstants, "COVER_KEYS", ()))
