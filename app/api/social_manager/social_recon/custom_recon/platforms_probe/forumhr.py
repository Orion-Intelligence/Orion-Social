...
def probe_url(username: str) -> str:
    return ForumHrConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForumHrConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumHrConstants, "AVATAR_KEYS", ()), getattr(ForumHrConstants, "COVER_KEYS", ()))
