...
def probe_url(username: str) -> str:
    return FreesoundConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FreesoundConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FreesoundConstants, "AVATAR_KEYS", ()), getattr(FreesoundConstants, "COVER_KEYS", ()))
