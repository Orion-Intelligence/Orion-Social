...
from api.social_manager.social_recon.constants.platform_constants import ForumsWyzecamComConstants

constants = ForumsWyzecamComConstants


def probe_url(username: str) -> str:
    return ForumsWyzecamComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForumsWyzecamComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumsWyzecamComConstants, "AVATAR_KEYS", ()), getattr(ForumsWyzecamComConstants, "COVER_KEYS", ()))
