...
from api.social_manager.social_recon.constants.platform_constants import UbuntuMateCommunityConstants

constants = UbuntuMateCommunityConstants


def probe_url(username: str) -> str:
    return UbuntuMateCommunityConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(UbuntuMateCommunityConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(UbuntuMateCommunityConstants, "AVATAR_KEYS", ()), getattr(UbuntuMateCommunityConstants, "COVER_KEYS", ()))
