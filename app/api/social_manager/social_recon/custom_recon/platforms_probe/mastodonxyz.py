...
from api.social_manager.social_recon.constants.platform_constants import MastodonXyzConstants

constants = MastodonXyzConstants


def probe_url(username: str) -> str:
    return MastodonXyzConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MastodonXyzConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MastodonXyzConstants, "AVATAR_KEYS", ()), getattr(MastodonXyzConstants, "COVER_KEYS", ()))
