...
def probe_url(username: str) -> str:
    return CSSBattleConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CSSBattleConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CSSBattleConstants, "AVATAR_KEYS", ()), getattr(CSSBattleConstants, "COVER_KEYS", ()))
