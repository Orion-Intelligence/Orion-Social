...
from api.social_manager.social_recon.constants.platform_constants import ForumCfxReConstants

constants = ForumCfxReConstants


def probe_url(username: str) -> str:
    return ForumCfxReConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForumCfxReConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumCfxReConstants, "AVATAR_KEYS", ()), getattr(ForumCfxReConstants, "COVER_KEYS", ()))
