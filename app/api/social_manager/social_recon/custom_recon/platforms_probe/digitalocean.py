...
def probe_url(username: str) -> str:
    return DigitalOceanConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DigitalOceanConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DigitalOceanConstants, "AVATAR_KEYS", ()), getattr(DigitalOceanConstants, "COVER_KEYS", ()))
