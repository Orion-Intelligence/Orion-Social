...
def probe_url(username: str) -> str:
    return FunnyjunkConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FunnyjunkConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FunnyjunkConstants, "AVATAR_KEYS", ()), getattr(FunnyjunkConstants, "COVER_KEYS", ()))
