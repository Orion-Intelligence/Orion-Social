...
from api.social_manager.social_recon.constants.platform_constants import NickNameRuConstants

constants = NickNameRuConstants


def probe_url(username: str) -> str:
    return NickNameRuConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(NickNameRuConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(NickNameRuConstants, "AVATAR_KEYS", ()), getattr(NickNameRuConstants, "COVER_KEYS", ()))
