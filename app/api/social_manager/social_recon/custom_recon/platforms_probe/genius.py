...
def probe_url(username: str) -> str:
    return GeniusConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(GeniusConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(GeniusConstants, "AVATAR_KEYS", ()), getattr(GeniusConstants, "COVER_KEYS", ()))
