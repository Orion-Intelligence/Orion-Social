...
from api.social_manager.social_recon.constants.platform_constants import TamTamConstants

constants = TamTamConstants


def probe_url(username: str) -> str:
    return TamTamConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(TamTamConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(TamTamConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TamTamConstants, "AVATAR_KEYS", ()), getattr(TamTamConstants, "COVER_KEYS", ()))
