...
def probe_url(username: str) -> str:
    return LineConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(LineConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(LineConstants, "AVATAR_KEYS", ()), getattr(LineConstants, "COVER_KEYS", ()))
