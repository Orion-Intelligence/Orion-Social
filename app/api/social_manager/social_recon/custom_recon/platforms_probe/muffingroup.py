...
from api.social_manager.social_recon.constants.platform_constants import MuffinGroupConstants

constants = MuffinGroupConstants


def probe_url(username: str) -> str:
    return MuffinGroupConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MuffinGroupConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MuffinGroupConstants, "AVATAR_KEYS", ()), getattr(MuffinGroupConstants, "COVER_KEYS", ()))
...
ROUTES = (
    (r"betheme/profile/(?P<id>[^/]+)(?:/.*)?", "profile"),
)
