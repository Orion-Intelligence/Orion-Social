...
from api.social_manager.social_recon.constants.platform_constants import ForumBonsaimiraiComConstants

constants = ForumBonsaimiraiComConstants


def probe_url(username: str) -> str:
    return ForumBonsaimiraiComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForumBonsaimiraiComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumBonsaimiraiComConstants, "AVATAR_KEYS", ()), getattr(ForumBonsaimiraiComConstants, "COVER_KEYS", ()))
