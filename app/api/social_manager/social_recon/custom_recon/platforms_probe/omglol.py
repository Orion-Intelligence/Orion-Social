...
from api.social_manager.social_recon.constants.platform_constants import OmgLolConstants

constants = OmgLolConstants


def probe_url(username: str) -> str:
    return OmgLolConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(OmgLolConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(OmgLolConstants, "AVATAR_KEYS", ()), getattr(OmgLolConstants, "COVER_KEYS", ()))
