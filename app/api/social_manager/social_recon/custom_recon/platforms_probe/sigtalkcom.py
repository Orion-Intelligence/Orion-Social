...
from api.social_manager.social_recon.constants.platform_constants import SigtalkComConstants

constants = SigtalkComConstants


def probe_url(username: str) -> str:
    return SigtalkComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SigtalkComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SigtalkComConstants, "AVATAR_KEYS", ()), getattr(SigtalkComConstants, "COVER_KEYS", ()))
