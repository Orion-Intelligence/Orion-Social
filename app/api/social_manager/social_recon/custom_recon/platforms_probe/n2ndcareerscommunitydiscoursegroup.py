...
from api.social_manager.social_recon.constants.platform_constants import N2ndcareersCommunityDiscourseGroupConstants

constants = N2ndcareersCommunityDiscourseGroupConstants


def probe_url(username: str) -> str:
    return N2ndcareersCommunityDiscourseGroupConstants.API_URL.format(username=username)
...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    if avatar.startswith("/"):
        avatar = f"https://2ndcareers-community.discourse.group{avatar}".replace("{size}", "240")
    info = {
