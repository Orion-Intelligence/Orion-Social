...
from api.social_manager.social_recon.constants.platform_constants import PolarstepsConstants

constants = PolarstepsConstants


def probe_url(username: str) -> str:
    return PolarstepsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PolarstepsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PolarstepsConstants, "AVATAR_KEYS", ()), getattr(PolarstepsConstants, "COVER_KEYS", ()))
