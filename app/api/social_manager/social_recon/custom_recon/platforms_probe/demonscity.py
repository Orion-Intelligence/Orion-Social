...
from api.social_manager.social_recon.constants.platform_constants import DemonscityConstants

constants = DemonscityConstants


def probe_url(username: str) -> str:
    return DemonscityConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DemonscityConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DemonscityConstants, "AVATAR_KEYS", ()), getattr(DemonscityConstants, "COVER_KEYS", ()))
