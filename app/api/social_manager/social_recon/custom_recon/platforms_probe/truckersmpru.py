...
from api.social_manager.social_recon.constants.platform_constants import TruckersMPRuConstants

constants = TruckersMPRuConstants


def probe_url(username: str) -> str:
    return TruckersMPRuConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TruckersMPRuConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TruckersMPRuConstants, "AVATAR_KEYS", ()), getattr(TruckersMPRuConstants, "COVER_KEYS", ()))
