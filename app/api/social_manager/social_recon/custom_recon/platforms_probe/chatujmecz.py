...
def probe_url(username: str) -> str:
    return ChatujmeCzConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ChatujmeCzConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ChatujmeCzConstants, "AVATAR_KEYS", ()), getattr(ChatujmeCzConstants, "COVER_KEYS", ()))
