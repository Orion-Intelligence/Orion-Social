...
def probe_url(username: str) -> str:
    return ProfiRuConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ProfiRuConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ProfiRuConstants, "AVATAR_KEYS", ()), getattr(ProfiRuConstants, "COVER_KEYS", ()))
