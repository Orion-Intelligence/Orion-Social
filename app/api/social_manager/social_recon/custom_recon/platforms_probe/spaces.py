...
def probe_url(username: str) -> str:
    return SpacesConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SpacesConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SpacesConstants, "AVATAR_KEYS", ()), getattr(SpacesConstants, "COVER_KEYS", ()))
