...
from api.social_manager.social_recon.constants.platform_constants import JbzdConstants

constants = JbzdConstants


def probe_url(username: str) -> str:
    return JbzdConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(JbzdConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(JbzdConstants, "AVATAR_KEYS", ()), getattr(JbzdConstants, "COVER_KEYS", ()))
