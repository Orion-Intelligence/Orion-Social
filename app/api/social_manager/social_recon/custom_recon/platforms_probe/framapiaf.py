...
from api.social_manager.social_recon.constants.platform_constants import FramapiafConstants

constants = FramapiafConstants


def probe_url(username: str) -> str:
    return FramapiafConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FramapiafConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FramapiafConstants, "AVATAR_KEYS", ()), getattr(FramapiafConstants, "COVER_KEYS", ()))
