...
def probe_url(username: str) -> str:
    return LaracastConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(LaracastConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(LaracastConstants, "AVATAR_KEYS", ()), getattr(LaracastConstants, "COVER_KEYS", ()))
