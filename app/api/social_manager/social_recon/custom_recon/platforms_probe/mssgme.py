...
from api.social_manager.social_recon.constants.platform_constants import MssgMeConstants

constants = MssgMeConstants


def probe_url(username: str) -> str:
    return MssgMeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MssgMeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MssgMeConstants, "AVATAR_KEYS", ()), getattr(MssgMeConstants, "COVER_KEYS", ()))
