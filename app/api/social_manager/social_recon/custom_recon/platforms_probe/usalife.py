...
from api.social_manager.social_recon.constants.platform_constants import UsaLifeConstants

constants = UsaLifeConstants


def probe_url(username: str) -> str:
    return UsaLifeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(UsaLifeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(UsaLifeConstants, "AVATAR_KEYS", ()), getattr(UsaLifeConstants, "COVER_KEYS", ()))
