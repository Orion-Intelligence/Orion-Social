...
def probe_url(username: str) -> str:
    return AllTheLyricsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(AllTheLyricsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(AllTheLyricsConstants, "AVATAR_KEYS", ()), getattr(AllTheLyricsConstants, "COVER_KEYS", ()))
