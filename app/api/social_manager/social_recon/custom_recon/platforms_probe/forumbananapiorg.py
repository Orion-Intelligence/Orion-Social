...
from api.social_manager.social_recon.constants.platform_constants import ForumBananaPiOrgConstants

constants = ForumBananaPiOrgConstants


def probe_url(username: str) -> str:
    return ForumBananaPiOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForumBananaPiOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumBananaPiOrgConstants, "AVATAR_KEYS", ()), getattr(ForumBananaPiOrgConstants, "COVER_KEYS", ()))
