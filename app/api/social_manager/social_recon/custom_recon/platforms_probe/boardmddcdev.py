...
from api.social_manager.social_recon.constants.platform_constants import BoardMddcDevConstants

constants = BoardMddcDevConstants


def probe_url(username: str) -> str:
    return BoardMddcDevConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(BoardMddcDevConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(BoardMddcDevConstants, "AVATAR_KEYS", ()), getattr(BoardMddcDevConstants, "COVER_KEYS", ()))
