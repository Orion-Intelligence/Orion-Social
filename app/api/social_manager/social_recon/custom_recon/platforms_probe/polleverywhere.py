...
from api.social_manager.social_recon.constants.platform_constants import PolleverywhereConstants

constants = PolleverywhereConstants


def probe_url(username: str) -> str:
    return PolleverywhereConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(PolleverywhereConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(PolleverywhereConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PolleverywhereConstants, "AVATAR_KEYS", ()), getattr(PolleverywhereConstants, "COVER_KEYS", ()))
...
ROUTES = (
    (r"proxy/api/users/(?P<id>[^/]+)(?:/.*)?", "profile"),
)
