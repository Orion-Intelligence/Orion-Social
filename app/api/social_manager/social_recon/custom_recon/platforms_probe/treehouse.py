...
def probe_url(username: str) -> str:
    return TreehouseConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TreehouseConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TreehouseConstants, "AVATAR_KEYS", ()), getattr(TreehouseConstants, "COVER_KEYS", ()))
