...
from api.social_manager.social_recon.constants.platform_constants import SecretKompas3dSuConstants

constants = SecretKompas3dSuConstants


def probe_url(username: str) -> str:
    return SecretKompas3dSuConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(SecretKompas3dSuConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(SecretKompas3dSuConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SecretKompas3dSuConstants, "AVATAR_KEYS", ()), getattr(SecretKompas3dSuConstants, "COVER_KEYS", ()))
