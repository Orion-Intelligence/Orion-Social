...
from api.social_manager.social_recon.constants.platform_constants import HomskComConstants

constants = HomskComConstants


def probe_url(username: str) -> str:
    return HomskComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(HomskComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(HomskComConstants, "AVATAR_KEYS", ()), getattr(HomskComConstants, "COVER_KEYS", ()))
