...
from api.social_manager.social_recon.constants.platform_constants import MyMiniFactoryConstants

constants = MyMiniFactoryConstants


def probe_url(username: str) -> str:
    return MyMiniFactoryConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MyMiniFactoryConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MyMiniFactoryConstants, "AVATAR_KEYS", ()), getattr(MyMiniFactoryConstants, "COVER_KEYS", ()))
