...
def probe_url(username: str) -> str:
    return SoupConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SoupConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SoupConstants, "AVATAR_KEYS", ()), getattr(SoupConstants, "COVER_KEYS", ()))
