...
from api.social_manager.social_recon.constants.platform_constants import ChessConstants

constants = ChessConstants


def probe_url(username: str) -> str:
    return ChessConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ChessConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ChessConstants, "AVATAR_KEYS", ()), getattr(ChessConstants, "COVER_KEYS", ()))
