...
from api.social_manager.social_recon.constants.platform_constants import ForumFibraClickConstants

constants = ForumFibraClickConstants


def probe_url(username: str) -> str:
    return ForumFibraClickConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForumFibraClickConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumFibraClickConstants, "AVATAR_KEYS", ()), getattr(ForumFibraClickConstants, "COVER_KEYS", ()))
