...
def probe_url(username: str) -> str:
    return NitroTypeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(NitroTypeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(NitroTypeConstants, "AVATAR_KEYS", ()), getattr(NitroTypeConstants, "COVER_KEYS", ()))
