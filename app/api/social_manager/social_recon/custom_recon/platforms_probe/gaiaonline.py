...
def probe_url(username: str) -> str:
    return GaiaOnlineConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(GaiaOnlineConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(GaiaOnlineConstants, "AVATAR_KEYS", ()), getattr(GaiaOnlineConstants, "COVER_KEYS", ()))
