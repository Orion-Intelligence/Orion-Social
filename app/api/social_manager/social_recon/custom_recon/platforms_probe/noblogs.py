...
def probe_url(username: str) -> str:
    return NoblogsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(NoblogsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(NoblogsConstants, "AVATAR_KEYS", ()), getattr(NoblogsConstants, "COVER_KEYS", ()))
