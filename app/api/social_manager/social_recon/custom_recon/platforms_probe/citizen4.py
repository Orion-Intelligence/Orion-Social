...
from api.social_manager.social_recon.constants.platform_constants import Citizen4Constants

constants = Citizen4Constants


def probe_url(username: str) -> str:
    return Citizen4Constants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(Citizen4Constants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(Citizen4Constants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(Citizen4Constants, "AVATAR_KEYS", ()), getattr(Citizen4Constants, "COVER_KEYS", ()))
...
ROUTES = (
    (r"profile/(?P<id>[^/]+)/profile", "profile"),
)
