...
from api.social_manager.social_recon.constants.platform_constants import TosterConstants

constants = TosterConstants


def probe_url(username: str) -> str:
    return TosterConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TosterConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TosterConstants, "AVATAR_KEYS", ()), getattr(TosterConstants, "COVER_KEYS", ()))
