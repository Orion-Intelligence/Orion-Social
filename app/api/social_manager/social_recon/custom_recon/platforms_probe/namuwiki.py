...
def probe_url(username: str) -> str:
    return NamuwikiConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(NamuwikiConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(NamuwikiConstants, "AVATAR_KEYS", ()), getattr(NamuwikiConstants, "COVER_KEYS", ()))
