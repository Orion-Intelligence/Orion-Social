...
def probe_url(username: str) -> str:
    return UpworkConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(UpworkConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(UpworkConstants, "AVATAR_KEYS", ()), getattr(UpworkConstants, "COVER_KEYS", ()))
