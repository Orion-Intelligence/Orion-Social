...
def probe_url(username: str) -> str:
    return TouristlinkConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TouristlinkConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TouristlinkConstants, "AVATAR_KEYS", ()), getattr(TouristlinkConstants, "COVER_KEYS", ()))
...
ROUTES = (
    ("(?:city|attraction|hotel)/(?P<id>[^/]+)(?:/.*)?", "page"),
    ("user/(?P<id>[^/]+)(?:/.*)?", "profile"),
)
