...
from api.social_manager.social_recon.constants.platform_constants import BookandreaderConstants

constants = BookandreaderConstants


def probe_url(username: str) -> str:
    return BookandreaderConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(BookandreaderConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(BookandreaderConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(BookandreaderConstants, "AVATAR_KEYS", ()), getattr(BookandreaderConstants, "COVER_KEYS", ()))
