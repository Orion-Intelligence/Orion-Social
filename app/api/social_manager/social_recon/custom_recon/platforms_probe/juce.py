...
from api.social_manager.social_recon.constants.platform_constants import JuceConstants

constants = JuceConstants


def probe_url(username: str) -> str:
    return JuceConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(JuceConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(JuceConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(JuceConstants, "AVATAR_KEYS", ()), getattr(JuceConstants, "COVER_KEYS", ()))
