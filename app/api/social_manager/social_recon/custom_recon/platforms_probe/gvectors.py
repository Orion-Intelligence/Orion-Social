...
def probe_url(username: str) -> str:
    return GvectorsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(GvectorsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(GvectorsConstants, "AVATAR_KEYS", ()), getattr(GvectorsConstants, "COVER_KEYS", ()))
