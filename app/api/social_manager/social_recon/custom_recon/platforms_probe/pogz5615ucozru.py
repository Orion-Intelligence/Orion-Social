...
from api.social_manager.social_recon.constants.platform_constants import Pogz5615UcozRuConstants

constants = Pogz5615UcozRuConstants


def probe_url(username: str) -> str:
    return Pogz5615UcozRuConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(Pogz5615UcozRuConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(Pogz5615UcozRuConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(Pogz5615UcozRuConstants, "AVATAR_KEYS", ()), getattr(Pogz5615UcozRuConstants, "COVER_KEYS", ()))
