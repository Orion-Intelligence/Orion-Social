...
def probe_url(username: str) -> str:
    return GuruShotsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(GuruShotsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(GuruShotsConstants, "AVATAR_KEYS", ()), getattr(GuruShotsConstants, "COVER_KEYS", ()))
