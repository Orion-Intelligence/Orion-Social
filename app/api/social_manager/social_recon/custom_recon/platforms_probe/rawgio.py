...
from api.social_manager.social_recon.constants.platform_constants import RawgIoConstants

constants = RawgIoConstants


def probe_url(username: str) -> str:
    return RawgIoConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(RawgIoConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(RawgIoConstants, "AVATAR_KEYS", ()), getattr(RawgIoConstants, "COVER_KEYS", ()))
