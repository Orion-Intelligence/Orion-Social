...
def probe_url(username: str) -> str:
    return PronounsPageConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PronounsPageConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PronounsPageConstants, "AVATAR_KEYS", ()), getattr(PronounsPageConstants, "COVER_KEYS", ()))
