...
from api.social_manager.social_recon.constants.platform_constants import EnUncyclopediaCoConstants

constants = EnUncyclopediaCoConstants


def probe_url(username: str) -> str:
    return EnUncyclopediaCoConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(EnUncyclopediaCoConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(EnUncyclopediaCoConstants, "AVATAR_KEYS", ()), getattr(EnUncyclopediaCoConstants, "COVER_KEYS", ()))
