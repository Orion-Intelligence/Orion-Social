...
from api.social_manager.social_recon.constants.platform_constants import LinuxfoundationConstants

constants = LinuxfoundationConstants


def probe_url(username: str) -> str:
    return LinuxfoundationConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(LinuxfoundationConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(LinuxfoundationConstants, "AVATAR_KEYS", ()), getattr(LinuxfoundationConstants, "COVER_KEYS", ()))
