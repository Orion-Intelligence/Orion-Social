...
def probe_url(username: str) -> str:
    return DuolingoConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DuolingoConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DuolingoConstants, "AVATAR_KEYS", ()), getattr(DuolingoConstants, "COVER_KEYS", ()))
