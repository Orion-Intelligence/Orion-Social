...
def probe_url(username: str) -> str:
    return LightstalkingComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(LightstalkingComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(LightstalkingComConstants, "AVATAR_KEYS", ()), getattr(LightstalkingComConstants, "COVER_KEYS", ()))
