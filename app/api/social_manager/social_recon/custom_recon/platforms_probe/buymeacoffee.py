...
def probe_url(username: str) -> str:
    return BuyMeACoffeeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(BuyMeACoffeeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(BuyMeACoffeeConstants, "AVATAR_KEYS", ()), getattr(BuyMeACoffeeConstants, "COVER_KEYS", ()))
