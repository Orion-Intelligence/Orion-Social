...
def probe_url(username: str) -> str:
    return ForumsDromRuConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForumsDromRuConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumsDromRuConstants, "AVATAR_KEYS", ()), getattr(ForumsDromRuConstants, "COVER_KEYS", ()))
