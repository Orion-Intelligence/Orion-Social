...
from api.social_manager.social_recon.constants.platform_constants import So4ineniyaUcozRuConstants

constants = So4ineniyaUcozRuConstants


def probe_url(username: str) -> str:
    return So4ineniyaUcozRuConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(So4ineniyaUcozRuConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(So4ineniyaUcozRuConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(So4ineniyaUcozRuConstants, "AVATAR_KEYS", ()), getattr(So4ineniyaUcozRuConstants, "COVER_KEYS", ()))
