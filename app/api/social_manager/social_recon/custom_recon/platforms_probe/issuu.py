...
from api.social_manager.social_recon.constants.platform_constants import IssuuConstants

constants = IssuuConstants


def probe_url(username: str) -> str:
    return IssuuConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(IssuuConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(IssuuConstants, "AVATAR_KEYS", ()), getattr(IssuuConstants, "COVER_KEYS", ()))
