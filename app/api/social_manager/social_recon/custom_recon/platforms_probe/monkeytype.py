...
def probe_url(username: str) -> str:
    return MonkeytypeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MonkeytypeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MonkeytypeConstants, "AVATAR_KEYS", ()), getattr(MonkeytypeConstants, "COVER_KEYS", ()))
