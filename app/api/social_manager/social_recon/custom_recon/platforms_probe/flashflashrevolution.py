...
from api.social_manager.social_recon.constants.platform_constants import FlashflashrevolutionConstants

constants = FlashflashrevolutionConstants


def probe_url(username: str) -> str:
    return FlashflashrevolutionConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FlashflashrevolutionConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FlashflashrevolutionConstants, "AVATAR_KEYS", ()), getattr(FlashflashrevolutionConstants, "COVER_KEYS", ()))
