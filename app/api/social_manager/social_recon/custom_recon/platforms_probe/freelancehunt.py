...
def probe_url(username: str) -> str:
    return FreelancehuntConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FreelancehuntConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FreelancehuntConstants, "AVATAR_KEYS", ()), getattr(FreelancehuntConstants, "COVER_KEYS", ()))
