...
from api.social_manager.social_recon.constants.platform_constants import VjudgeConstants

constants = VjudgeConstants


def probe_url(username: str) -> str:
    return VjudgeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(VjudgeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(VjudgeConstants, "AVATAR_KEYS", ()), getattr(VjudgeConstants, "COVER_KEYS", ()))
