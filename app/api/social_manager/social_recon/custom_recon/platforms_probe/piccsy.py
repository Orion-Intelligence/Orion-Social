...
from api.social_manager.social_recon.constants.platform_constants import PiccsyConstants

constants = PiccsyConstants


def probe_url(username: str) -> str:
    return PiccsyConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PiccsyConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PiccsyConstants, "AVATAR_KEYS", ()), getattr(PiccsyConstants, "COVER_KEYS", ()))
