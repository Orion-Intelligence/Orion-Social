...
from api.social_manager.social_recon.constants.platform_constants import PicturepushComConstants

constants = PicturepushComConstants


def probe_url(username: str) -> str:
    return PicturepushComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PicturepushComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PicturepushComConstants, "AVATAR_KEYS", ()), getattr(PicturepushComConstants, "COVER_KEYS", ()))
...
HOSTS = ("picturepush.com",)
SUBDOMAIN = ("picturepush.com", "profile")
ROUTES = ()
