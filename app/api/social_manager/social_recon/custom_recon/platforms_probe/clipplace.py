...
from api.social_manager.social_recon.constants.platform_constants import ClipPlaceConstants

constants = ClipPlaceConstants


def probe_url(username: str) -> str:
    return ClipPlaceConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ClipPlaceConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ClipPlaceConstants, "AVATAR_KEYS", ()), getattr(ClipPlaceConstants, "COVER_KEYS", ()))
