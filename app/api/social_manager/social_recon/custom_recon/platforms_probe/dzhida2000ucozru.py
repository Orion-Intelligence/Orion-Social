...
from api.social_manager.social_recon.constants.platform_constants import Dzhida2000UcozRuConstants

constants = Dzhida2000UcozRuConstants


def probe_url(username: str) -> str:
    return Dzhida2000UcozRuConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(Dzhida2000UcozRuConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(Dzhida2000UcozRuConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(Dzhida2000UcozRuConstants, "AVATAR_KEYS", ()), getattr(Dzhida2000UcozRuConstants, "COVER_KEYS", ()))
