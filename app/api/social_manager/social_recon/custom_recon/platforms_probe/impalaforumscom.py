...
def probe_url(username: str) -> str:
    return ImpalaforumsComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ImpalaforumsComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ImpalaforumsComConstants, "AVATAR_KEYS", ()), getattr(ImpalaforumsComConstants, "COVER_KEYS", ()))
