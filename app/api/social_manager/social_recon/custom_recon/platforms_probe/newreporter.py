...
from api.social_manager.social_recon.constants.platform_constants import NewreporterConstants

constants = NewreporterConstants


def probe_url(username: str) -> str:
    return NewreporterConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(NewreporterConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(NewreporterConstants, "AVATAR_KEYS", ()), getattr(NewreporterConstants, "COVER_KEYS", ()))
