...
from api.social_manager.social_recon.constants.platform_constants import ManyLinkConstants

constants = ManyLinkConstants


def probe_url(username: str) -> str:
    return ManyLinkConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ManyLinkConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ManyLinkConstants, "AVATAR_KEYS", ()), getattr(ManyLinkConstants, "COVER_KEYS", ()))
