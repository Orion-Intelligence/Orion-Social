...
from api.social_manager.social_recon.constants.platform_constants import ManutdConstants

constants = ManutdConstants


def probe_url(username: str) -> str:
    return ManutdConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ManutdConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ManutdConstants, "AVATAR_KEYS", ()), getattr(ManutdConstants, "COVER_KEYS", ()))
