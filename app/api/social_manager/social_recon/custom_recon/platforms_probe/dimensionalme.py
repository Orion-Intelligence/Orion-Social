...
from api.social_manager.social_recon.constants.platform_constants import DimensionalMeConstants

constants = DimensionalMeConstants


def probe_url(username: str) -> str:
    return DimensionalMeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DimensionalMeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DimensionalMeConstants, "AVATAR_KEYS", ()), getattr(DimensionalMeConstants, "COVER_KEYS", ()))
