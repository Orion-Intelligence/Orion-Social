...
from api.social_manager.social_recon.constants.platform_constants import G2gComConstants

constants = G2gComConstants


def probe_url(username: str) -> str:
    return G2gComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(G2gComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(G2gComConstants, "AVATAR_KEYS", ()), getattr(G2gComConstants, "COVER_KEYS", ()))
