...
from api.social_manager.social_recon.constants.platform_constants import MyMailRuListRuConstants

constants = MyMailRuListRuConstants


def probe_url(username: str) -> str:
    return MyMailRuListRuConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MyMailRuListRuConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MyMailRuListRuConstants, "AVATAR_KEYS", ()), getattr(MyMailRuListRuConstants, "COVER_KEYS", ()))
