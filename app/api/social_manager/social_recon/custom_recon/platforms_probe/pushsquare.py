...
def probe_url(username: str) -> str:
    return PushSquareConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PushSquareConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PushSquareConstants, "AVATAR_KEYS", ()), getattr(PushSquareConstants, "COVER_KEYS", ()))
