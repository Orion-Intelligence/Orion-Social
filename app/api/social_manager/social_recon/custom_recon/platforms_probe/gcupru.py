...
from api.social_manager.social_recon.constants.platform_constants import GcupRuConstants

constants = GcupRuConstants


def probe_url(username: str) -> str:
    return GcupRuConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(GcupRuConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(GcupRuConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(GcupRuConstants, "AVATAR_KEYS", ()), getattr(GcupRuConstants, "COVER_KEYS", ()))
