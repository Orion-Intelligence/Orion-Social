...
def probe_url(username: str) -> str:
    return ThelionConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ThelionConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ThelionConstants, "AVATAR_KEYS", ()), getattr(ThelionConstants, "COVER_KEYS", ()))
