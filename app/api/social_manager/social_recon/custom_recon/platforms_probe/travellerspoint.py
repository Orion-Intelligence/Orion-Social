...
from api.social_manager.social_recon.constants.platform_constants import TravellersPointConstants

constants = TravellersPointConstants


def probe_url(username: str) -> str:
    return TravellersPointConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TravellersPointConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TravellersPointConstants, "AVATAR_KEYS", ()), getattr(TravellersPointConstants, "COVER_KEYS", ()))
