...
from api.social_manager.social_recon.constants.platform_constants import AwfulSystemsConstants

constants = AwfulSystemsConstants


def probe_url(username: str) -> str:
    return AwfulSystemsConstants.PROFILE_URL.format(username=username)


def fetch(username: str) -> tuple[int, str, str]:
    return http_client.fetch(AwfulSystemsConstants.API_URL.format(username=username))
...
def evaluate_resource(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if community:
        code, payload, _ = http_client.fetch(AwfulSystemsConstants.COMMUNITY_API.format(id=community.group(1)))
