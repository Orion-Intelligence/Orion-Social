...
def probe_url(username: str) -> str:
    return TripAdvisorConstants.PROFILE_URL.format(username=username)


def fetch(username: str) -> tuple[int, str, str]:
    return http_client.fetch(probe_url(username), impersonate=TripAdvisorConstants.IMPERSONATE)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TripAdvisorConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TripAdvisorConstants, "AVATAR_KEYS", ()), getattr(TripAdvisorConstants, "COVER_KEYS", ()))
...
ROUTES = (
    ("(?P<id>Hotel_Review-[^/]+)", "page"),
    ("(?P<id>Restaurant_Review-[^/]+)", "page"),
    ("(?P<id>Attraction_Review-[^/]+)", "page"),
