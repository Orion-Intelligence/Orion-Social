...
from api.social_manager.social_recon.constants.platform_constants import MinfinComUaConstants

constants = MinfinComUaConstants


def probe_url(username: str) -> str:
    return MinfinComUaConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MinfinComUaConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MinfinComUaConstants, "AVATAR_KEYS", ()), getattr(MinfinComUaConstants, "COVER_KEYS", ()))
