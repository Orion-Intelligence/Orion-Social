...
def probe_url(username: str) -> str:
    return MyinstantsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MyinstantsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MyinstantsConstants, "AVATAR_KEYS", ()), getattr(MyinstantsConstants, "COVER_KEYS", ()))
