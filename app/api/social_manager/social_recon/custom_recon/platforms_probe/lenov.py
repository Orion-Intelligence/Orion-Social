...
from api.social_manager.social_recon.constants.platform_constants import LenovConstants

constants = LenovConstants


def probe_url(username: str) -> str:
    return LenovConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(LenovConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(LenovConstants, "AVATAR_KEYS", ()), getattr(LenovConstants, "COVER_KEYS", ()))
