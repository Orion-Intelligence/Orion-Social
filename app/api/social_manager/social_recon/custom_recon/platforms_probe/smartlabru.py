...
from api.social_manager.social_recon.constants.platform_constants import SmartLabRuConstants

constants = SmartLabRuConstants


def probe_url(username: str) -> str:
    return SmartLabRuConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SmartLabRuConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SmartLabRuConstants, "AVATAR_KEYS", ()), getattr(SmartLabRuConstants, "COVER_KEYS", ()))
