...
from api.social_manager.social_recon.constants.platform_constants import DasaugeConstants

constants = DasaugeConstants


def probe_url(username: str) -> str:
    return DasaugeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DasaugeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DasaugeConstants, "AVATAR_KEYS", ()), getattr(DasaugeConstants, "COVER_KEYS", ()))
