...
def probe_url(username: str) -> str:
    return BlastConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(BlastConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(BlastConstants, "AVATAR_KEYS", ()), getattr(BlastConstants, "COVER_KEYS", ()))
