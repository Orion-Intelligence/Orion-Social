...
def probe_url(username: str) -> str:
    return FCRubinConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FCRubinConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FCRubinConstants, "AVATAR_KEYS", ()), getattr(FCRubinConstants, "COVER_KEYS", ()))
