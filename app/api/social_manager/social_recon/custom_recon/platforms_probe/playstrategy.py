...
from api.social_manager.social_recon.constants.platform_constants import PlaystrategyConstants

constants = PlaystrategyConstants


def probe_url(username: str) -> str:
    return PlaystrategyConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PlaystrategyConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PlaystrategyConstants, "AVATAR_KEYS", ()), getattr(PlaystrategyConstants, "COVER_KEYS", ()))
