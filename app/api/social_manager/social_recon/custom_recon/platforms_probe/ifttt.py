...
from api.social_manager.social_recon.constants.platform_constants import IFTTTConstants

constants = IFTTTConstants


def probe_url(username: str) -> str:
    return IFTTTConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(IFTTTConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(IFTTTConstants, "AVATAR_KEYS", ()), getattr(IFTTTConstants, "COVER_KEYS", ()))
