...
from api.social_manager.social_recon.constants.platform_constants import NNRUConstants

constants = NNRUConstants


def probe_url(username: str) -> str:
    return NNRUConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(NNRUConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(NNRUConstants, "AVATAR_KEYS", ()), getattr(NNRUConstants, "COVER_KEYS", ()))
