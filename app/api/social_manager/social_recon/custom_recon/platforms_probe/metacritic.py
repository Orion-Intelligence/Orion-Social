...
from api.social_manager.social_recon.constants.platform_constants import MetacriticConstants

constants = MetacriticConstants


def probe_url(username: str) -> str:
    return MetacriticConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MetacriticConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MetacriticConstants, "AVATAR_KEYS", ()), getattr(MetacriticConstants, "COVER_KEYS", ()))
