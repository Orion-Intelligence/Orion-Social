...
from api.social_manager.social_recon.constants.platform_constants import TerrariaForumsConstants

constants = TerrariaForumsConstants


def probe_url(username: str) -> str:
    return TerrariaForumsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TerrariaForumsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TerrariaForumsConstants, "AVATAR_KEYS", ()), getattr(TerrariaForumsConstants, "COVER_KEYS", ()))
...
ROUTES = (
    (r"index\.php/?\?(?:.*&)?c\[users\]=(?P<id>[^&]+)(?:&.*)?", "profile"),
)
