...
from api.social_manager.social_recon.constants.platform_constants import FreelancerComConstants

constants = FreelancerComConstants


def probe_url(username: str) -> str:
    return FreelancerComConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(FreelancerComConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(FreelancerComConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FreelancerComConstants, "AVATAR_KEYS", ()), getattr(FreelancerComConstants, "COVER_KEYS", ()))
...
ROUTES = (
    (r"api/users/0\.1/users/?\?(?:.*&)?usernames%5B%5D=(?P<id>[^&]+)(?:&.*)?", "profile"),
)
