...
from api.social_manager.social_recon.constants.platform_constants import CytoidIoConstants

constants = CytoidIoConstants


def probe_url(username: str) -> str:
    return CytoidIoConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CytoidIoConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CytoidIoConstants, "AVATAR_KEYS", ()), getattr(CytoidIoConstants, "COVER_KEYS", ()))
