...
def probe_url(username: str) -> str:
    return SpeakerdeckConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SpeakerdeckConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SpeakerdeckConstants, "AVATAR_KEYS", ()), getattr(SpeakerdeckConstants, "COVER_KEYS", ()))
