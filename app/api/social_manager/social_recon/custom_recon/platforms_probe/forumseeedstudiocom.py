...
from api.social_manager.social_recon.constants.platform_constants import ForumSeeedstudioComConstants

constants = ForumSeeedstudioComConstants


def probe_url(username: str) -> str:
    return ForumSeeedstudioComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForumSeeedstudioComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumSeeedstudioComConstants, "AVATAR_KEYS", ()), getattr(ForumSeeedstudioComConstants, "COVER_KEYS", ()))
