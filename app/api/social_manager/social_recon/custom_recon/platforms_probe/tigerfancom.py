...
def probe_url(username: str) -> str:
    return TigerfanComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TigerfanComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TigerfanComConstants, "AVATAR_KEYS", ()), getattr(TigerfanComConstants, "COVER_KEYS", ()))
