...
from api.social_manager.social_recon.constants.platform_constants import TrepupComConstants

constants = TrepupComConstants


def probe_url(username: str) -> str:
    return TrepupComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TrepupComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TrepupComConstants, "AVATAR_KEYS", ()), getattr(TrepupComConstants, "COVER_KEYS", ()))
