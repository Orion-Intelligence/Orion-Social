...
def probe_url(username: str) -> str:
    return RealmeyeConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(RealmeyeConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(RealmeyeConstants, "AVATAR_KEYS", ()), getattr(RealmeyeConstants, "COVER_KEYS", ()))
