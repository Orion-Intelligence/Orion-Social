...
def probe_url(username: str) -> str:
    return VTwinforumComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(VTwinforumComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(VTwinforumComConstants, "AVATAR_KEYS", ()), getattr(VTwinforumComConstants, "COVER_KEYS", ()))
