...
from api.social_manager.social_recon.constants.platform_constants import DiscourseMcStanOrgConstants

constants = DiscourseMcStanOrgConstants


def probe_url(username: str) -> str:
    return DiscourseMcStanOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DiscourseMcStanOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DiscourseMcStanOrgConstants, "AVATAR_KEYS", ()), getattr(DiscourseMcStanOrgConstants, "COVER_KEYS", ()))
