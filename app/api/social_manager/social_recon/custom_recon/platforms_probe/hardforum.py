...
def probe_url(username: str) -> str:
    return HardforumConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(HardforumConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(HardforumConstants, "AVATAR_KEYS", ()), getattr(HardforumConstants, "COVER_KEYS", ()))
