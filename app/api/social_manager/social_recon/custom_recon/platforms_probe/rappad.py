...
def probe_url(username: str) -> str:
    return RappadConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(RappadConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(RappadConstants, "AVATAR_KEYS", ()), getattr(RappadConstants, "COVER_KEYS", ()))
