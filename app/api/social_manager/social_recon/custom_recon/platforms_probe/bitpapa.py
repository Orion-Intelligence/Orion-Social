...
from api.social_manager.social_recon.constants.platform_constants import BitPapaConstants

constants = BitPapaConstants


def probe_url(username: str) -> str:
    return BitPapaConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(BitPapaConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(BitPapaConstants, "AVATAR_KEYS", ()), getattr(BitPapaConstants, "COVER_KEYS", ()))
