...
def probe_url(username: str) -> str:
    return OfficeForumsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(OfficeForumsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(OfficeForumsConstants, "AVATAR_KEYS", ()), getattr(OfficeForumsConstants, "COVER_KEYS", ()))
