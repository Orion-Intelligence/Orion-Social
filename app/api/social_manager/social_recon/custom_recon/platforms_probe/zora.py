...
from api.social_manager.social_recon.constants.platform_constants import ZoraConstants

constants = ZoraConstants


def probe_url(username: str) -> str:
    return ZoraConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ZoraConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ZoraConstants, "AVATAR_KEYS", ()), getattr(ZoraConstants, "COVER_KEYS", ()))
