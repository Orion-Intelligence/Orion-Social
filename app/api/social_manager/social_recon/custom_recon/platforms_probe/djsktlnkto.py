...
from api.social_manager.social_recon.constants.platform_constants import DjsktLnkToConstants

constants = DjsktLnkToConstants


def probe_url(username: str) -> str:
    return DjsktLnkToConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DjsktLnkToConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DjsktLnkToConstants, "AVATAR_KEYS", ()), getattr(DjsktLnkToConstants, "COVER_KEYS", ()))
