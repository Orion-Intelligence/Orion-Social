...
from api.social_manager.social_recon.constants.platform_constants import VolkswagenLvivUaConstants

constants = VolkswagenLvivUaConstants


def probe_url(username: str) -> str:
    return VolkswagenLvivUaConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(VolkswagenLvivUaConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(VolkswagenLvivUaConstants, "AVATAR_KEYS", ()), getattr(VolkswagenLvivUaConstants, "COVER_KEYS", ()))
