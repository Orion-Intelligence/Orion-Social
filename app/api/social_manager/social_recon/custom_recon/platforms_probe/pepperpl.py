...
from api.social_manager.social_recon.constants.platform_constants import PepperPLConstants

constants = PepperPLConstants


def probe_url(username: str) -> str:
    return PepperPLConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PepperPLConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PepperPLConstants, "AVATAR_KEYS", ()), getattr(PepperPLConstants, "COVER_KEYS", ()))
