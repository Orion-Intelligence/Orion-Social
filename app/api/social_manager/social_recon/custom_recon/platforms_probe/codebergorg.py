...
def probe_url(username: str) -> str:
    return CodebergOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CodebergOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CodebergOrgConstants, "AVATAR_KEYS", ()), getattr(CodebergOrgConstants, "COVER_KEYS", ()))
