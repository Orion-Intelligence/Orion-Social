...
from api.social_manager.social_recon.constants.platform_constants import UsersRustLangOrgConstants

constants = UsersRustLangOrgConstants


def probe_url(username: str) -> str:
    return UsersRustLangOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(UsersRustLangOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(UsersRustLangOrgConstants, "AVATAR_KEYS", ()), getattr(UsersRustLangOrgConstants, "COVER_KEYS", ()))
