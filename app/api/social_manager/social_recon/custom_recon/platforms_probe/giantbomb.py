...
def probe_url(username: str) -> str:
    return GiantbombConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(GiantbombConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(GiantbombConstants, "AVATAR_KEYS", ()), getattr(GiantbombConstants, "COVER_KEYS", ()))
