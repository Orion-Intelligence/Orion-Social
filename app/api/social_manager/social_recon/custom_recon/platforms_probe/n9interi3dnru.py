...
from api.social_manager.social_recon.constants.platform_constants import N9interi3dnRuConstants

constants = N9interi3dnRuConstants


def probe_url(username: str) -> str:
    return N9interi3dnRuConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(N9interi3dnRuConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(N9interi3dnRuConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(N9interi3dnRuConstants, "AVATAR_KEYS", ()), getattr(N9interi3dnRuConstants, "COVER_KEYS", ()))
