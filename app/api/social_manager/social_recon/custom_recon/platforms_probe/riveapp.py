...
def probe_url(username: str) -> str:
    return RiveAppConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(RiveAppConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(RiveAppConstants, "AVATAR_KEYS", ()), getattr(RiveAppConstants, "COVER_KEYS", ()))
