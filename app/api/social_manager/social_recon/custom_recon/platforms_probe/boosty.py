...
from api.social_manager.social_recon.constants.platform_constants import BoostyConstants

constants = BoostyConstants


def probe_url(username: str) -> str:
    return BoostyConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(BoostyConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(BoostyConstants, "AVATAR_KEYS", ()), getattr(BoostyConstants, "COVER_KEYS", ()))
