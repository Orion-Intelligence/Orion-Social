...
from api.social_manager.social_recon.constants.platform_constants import MyMailRuYaRuConstants

constants = MyMailRuYaRuConstants


def probe_url(username: str) -> str:
    return MyMailRuYaRuConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MyMailRuYaRuConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MyMailRuYaRuConstants, "AVATAR_KEYS", ()), getattr(MyMailRuYaRuConstants, "COVER_KEYS", ()))
