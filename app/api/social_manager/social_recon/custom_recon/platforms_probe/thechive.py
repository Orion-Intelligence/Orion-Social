...
def probe_url(username: str) -> str:
    return ThechiveConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ThechiveConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ThechiveConstants, "AVATAR_KEYS", ()), getattr(ThechiveConstants, "COVER_KEYS", ()))
