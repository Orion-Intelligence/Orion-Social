...
def probe_url(username: str) -> str:
    return ViewBugConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ViewBugConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ViewBugConstants, "AVATAR_KEYS", ()), getattr(ViewBugConstants, "COVER_KEYS", ()))
