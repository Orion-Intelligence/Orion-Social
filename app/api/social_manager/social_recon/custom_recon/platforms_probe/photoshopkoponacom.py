...
from api.social_manager.social_recon.constants.platform_constants import PhotoshopKoponaComConstants

constants = PhotoshopKoponaComConstants


def probe_url(username: str) -> str:
    return PhotoshopKoponaComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PhotoshopKoponaComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PhotoshopKoponaComConstants, "AVATAR_KEYS", ()), getattr(PhotoshopKoponaComConstants, "COVER_KEYS", ()))
