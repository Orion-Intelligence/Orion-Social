...
def probe_url(username: str) -> str:
    return LkforumConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(LkforumConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(LkforumConstants, "AVATAR_KEYS", ()), getattr(LkforumConstants, "COVER_KEYS", ()))
