...
from api.social_manager.social_recon.constants.platform_constants import Mql5ComConstants

constants = Mql5ComConstants


def probe_url(username: str) -> str:
    return Mql5ComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(Mql5ComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(Mql5ComConstants, "AVATAR_KEYS", ()), getattr(Mql5ComConstants, "COVER_KEYS", ()))
