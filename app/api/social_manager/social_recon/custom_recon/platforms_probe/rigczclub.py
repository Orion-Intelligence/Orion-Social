...
from api.social_manager.social_recon.constants.platform_constants import RigczClubConstants

constants = RigczClubConstants


def probe_url(username: str) -> str:
    return RigczClubConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(RigczClubConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(RigczClubConstants, "AVATAR_KEYS", ()), getattr(RigczClubConstants, "COVER_KEYS", ()))
