...
from api.social_manager.social_recon.constants.platform_constants import BitpapaComConstants

constants = BitpapaComConstants


def probe_url(username: str) -> str:
    return BitpapaComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(BitpapaComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(BitpapaComConstants, "AVATAR_KEYS", ()), getattr(BitpapaComConstants, "COVER_KEYS", ()))
