...
from api.social_manager.social_recon.constants.platform_constants import ModxProConstants

constants = ModxProConstants


def probe_url(username: str) -> str:
    return ModxProConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ModxProConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ModxProConstants, "AVATAR_KEYS", ()), getattr(ModxProConstants, "COVER_KEYS", ()))
