...
def probe_url(username: str) -> str:
    return ItemFixConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ItemFixConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ItemFixConstants, "AVATAR_KEYS", ()), getattr(ItemFixConstants, "COVER_KEYS", ()))
