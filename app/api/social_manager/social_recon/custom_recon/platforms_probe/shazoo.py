...
from api.social_manager.social_recon.constants.platform_constants import ShazooConstants

constants = ShazooConstants


def probe_url(username: str) -> str:
    return ShazooConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ShazooConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ShazooConstants, "AVATAR_KEYS", ()), getattr(ShazooConstants, "COVER_KEYS", ()))
