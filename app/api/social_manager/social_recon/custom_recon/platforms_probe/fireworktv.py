...
from api.social_manager.social_recon.constants.platform_constants import FireworktvConstants

constants = FireworktvConstants


def probe_url(username: str) -> str:
    return FireworktvConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FireworktvConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FireworktvConstants, "AVATAR_KEYS", ()), getattr(FireworktvConstants, "COVER_KEYS", ()))
