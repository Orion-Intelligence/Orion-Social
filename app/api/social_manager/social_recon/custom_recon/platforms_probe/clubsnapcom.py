...
def probe_url(username: str) -> str:
    return ClubsnapComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ClubsnapComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ClubsnapComConstants, "AVATAR_KEYS", ()), getattr(ClubsnapComConstants, "COVER_KEYS", ()))
