...
from api.social_manager.social_recon.constants.platform_constants import CrevadoConstants

constants = CrevadoConstants


def probe_url(username: str) -> str:
    return CrevadoConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CrevadoConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CrevadoConstants, "AVATAR_KEYS", ()), getattr(CrevadoConstants, "COVER_KEYS", ()))
