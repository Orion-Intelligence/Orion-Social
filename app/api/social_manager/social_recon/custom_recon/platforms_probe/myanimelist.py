...
from api.social_manager.social_recon.constants.platform_constants import MyAnimeListConstants

constants = MyAnimeListConstants


def probe_url(username: str) -> str:
    return MyAnimeListConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in MyAnimeListConstants.GENERIC:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, MyAnimeListConstants.AVATAR_KEYS, MyAnimeListConstants.COVER_KEYS)
    name = parse.clean(parse.text(parse.meta(body).get("og:title") or heading).split("'s Profile")[0])
    if name:
