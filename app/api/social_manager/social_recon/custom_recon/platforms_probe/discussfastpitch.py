...
def probe_url(username: str) -> str:
    return DiscussfastpitchConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DiscussfastpitchConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DiscussfastpitchConstants, "AVATAR_KEYS", ()), getattr(DiscussfastpitchConstants, "COVER_KEYS", ()))
