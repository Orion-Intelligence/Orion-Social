...
from api.social_manager.social_recon.constants.platform_constants import MyMailRuMailRuConstants

constants = MyMailRuMailRuConstants


def probe_url(username: str) -> str:
    return MyMailRuMailRuConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MyMailRuMailRuConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MyMailRuMailRuConstants, "AVATAR_KEYS", ()), getattr(MyMailRuMailRuConstants, "COVER_KEYS", ()))
