...
from api.social_manager.social_recon.constants.platform_constants import PromoDJConstants

constants = PromoDJConstants


def probe_url(username: str) -> str:
    return PromoDJConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PromoDJConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PromoDJConstants, "AVATAR_KEYS", ()), getattr(PromoDJConstants, "COVER_KEYS", ()))
