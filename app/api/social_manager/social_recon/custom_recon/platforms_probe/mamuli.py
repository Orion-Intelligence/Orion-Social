...
from api.social_manager.social_recon.constants.platform_constants import MamuliConstants

constants = MamuliConstants


def probe_url(username: str) -> str:
    return MamuliConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MamuliConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MamuliConstants, "AVATAR_KEYS", ()), getattr(MamuliConstants, "COVER_KEYS", ()))
