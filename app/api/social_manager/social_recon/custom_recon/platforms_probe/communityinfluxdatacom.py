...
from api.social_manager.social_recon.constants.platform_constants import CommunityInfluxdataComConstants

constants = CommunityInfluxdataComConstants


def probe_url(username: str) -> str:
    return CommunityInfluxdataComConstants.API_URL.format(username=username)
...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    if not isinstance(user, dict) or not user.get("username"):
        return VerdictConstants.UNKNOWN, {}
    base = CommunityInfluxdataComConstants.PROFILE_URL.split("/u/")[0]
