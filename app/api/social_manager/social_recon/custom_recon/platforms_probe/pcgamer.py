...
def probe_url(username: str) -> str:
    return PCGamerConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PCGamerConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PCGamerConstants, "AVATAR_KEYS", ()), getattr(PCGamerConstants, "COVER_KEYS", ()))
