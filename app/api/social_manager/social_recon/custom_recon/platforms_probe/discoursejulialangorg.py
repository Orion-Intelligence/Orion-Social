...
from api.social_manager.social_recon.constants.platform_constants import DiscourseJulialangOrgConstants

constants = DiscourseJulialangOrgConstants


def probe_url(username: str) -> str:
    return DiscourseJulialangOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DiscourseJulialangOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DiscourseJulialangOrgConstants, "AVATAR_KEYS", ()), getattr(DiscourseJulialangOrgConstants, "COVER_KEYS", ()))
