...
from api.social_manager.social_recon.constants.platform_constants import WikiTeamfortressComConstants

constants = WikiTeamfortressComConstants


def probe_url(username: str) -> str:
    return WikiTeamfortressComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(WikiTeamfortressComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(WikiTeamfortressComConstants, "AVATAR_KEYS", ()), getattr(WikiTeamfortressComConstants, "COVER_KEYS", ()))
