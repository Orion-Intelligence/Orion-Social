...
def probe_url(username: str) -> str:
    return ComedyConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ComedyConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ComedyConstants, "AVATAR_KEYS", ()), getattr(ComedyConstants, "COVER_KEYS", ()))
