...
from api.social_manager.social_recon.constants.platform_constants import CapitalcityCombatsConstants

constants = CapitalcityCombatsConstants


def probe_url(username: str) -> str:
    return CapitalcityCombatsConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CapitalcityCombatsConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CapitalcityCombatsConstants, "AVATAR_KEYS", ()), getattr(CapitalcityCombatsConstants, "COVER_KEYS", ()))
...
ROUTES = (
    (r"inf\.pl/?\?(?:.*&)?\{username\}=(?P<id>[^&]+)(?:&.*)?", "profile"),
)
