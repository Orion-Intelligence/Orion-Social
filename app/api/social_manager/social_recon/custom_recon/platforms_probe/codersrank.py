...
def probe_url(username: str) -> str:
    return CodersRankConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CodersRankConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CodersRankConstants, "AVATAR_KEYS", ()), getattr(CodersRankConstants, "COVER_KEYS", ()))
