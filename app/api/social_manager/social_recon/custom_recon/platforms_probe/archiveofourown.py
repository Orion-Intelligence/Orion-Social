...
def probe_url(username: str) -> str:
    return ArchiveOfOurOwnConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ArchiveOfOurOwnConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ArchiveOfOurOwnConstants, "AVATAR_KEYS", ()), getattr(ArchiveOfOurOwnConstants, "COVER_KEYS", ()))
...
ROUTES = (
    ("collections/(?P<id>[^/]+)(?:/.*)?", "group"),
    (r"series/(?P<id>\d+)(?:/.*)?", "group"),
    (r"series/(?P<id>\d+)", "group"),
    ("users/(?P<id>[^/]+)(?:/.*)?", "profile"),
)
