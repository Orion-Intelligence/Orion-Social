...
from api.social_manager.social_recon.constants.platform_constants import DiscussHuelComConstants

constants = DiscussHuelComConstants


def probe_url(username: str) -> str:
    return DiscussHuelComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(DiscussHuelComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(DiscussHuelComConstants, "AVATAR_KEYS", ()), getattr(DiscussHuelComConstants, "COVER_KEYS", ()))
