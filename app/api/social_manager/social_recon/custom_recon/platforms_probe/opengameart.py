...
def probe_url(username: str) -> str:
    return OpenGameArtConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(OpenGameArtConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(OpenGameArtConstants, "AVATAR_KEYS", ()), getattr(OpenGameArtConstants, "COVER_KEYS", ()))
