...
def probe_url(username: str) -> str:
    return ForumJizniConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForumJizniConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumJizniConstants, "AVATAR_KEYS", ()), getattr(ForumJizniConstants, "COVER_KEYS", ()))
