...
def probe_url(username: str) -> str:
    return OakleyforumComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(OakleyforumComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(OakleyforumComConstants, "AVATAR_KEYS", ()), getattr(OakleyforumComConstants, "COVER_KEYS", ()))
