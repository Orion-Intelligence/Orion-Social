...
from api.social_manager.social_recon.constants.platform_constants import XINGConstants

constants = XINGConstants


def probe_url(username: str) -> str:
    return XINGConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    if "login.xing.com" in (_final_url or ""):
        return VerdictConstants.UNKNOWN, {}
    heading = parse.title(body)
    if not heading or heading.casefold() in XINGConstants.GENERIC:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, XINGConstants.AVATAR_KEYS, XINGConstants.COVER_KEYS)
