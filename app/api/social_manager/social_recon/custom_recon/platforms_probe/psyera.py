...
from api.social_manager.social_recon.constants.platform_constants import PsyeraConstants

constants = PsyeraConstants


def probe_url(username: str) -> str:
    return PsyeraConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PsyeraConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PsyeraConstants, "AVATAR_KEYS", ()), getattr(PsyeraConstants, "COVER_KEYS", ()))
