...
from api.social_manager.social_recon.constants.platform_constants import FlipsnackComConstants

constants = FlipsnackComConstants


def probe_url(username: str) -> str:
    return FlipsnackComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(FlipsnackComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(FlipsnackComConstants, "AVATAR_KEYS", ()), getattr(FlipsnackComConstants, "COVER_KEYS", ()))
