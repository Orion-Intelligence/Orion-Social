...
def probe_url(username: str) -> str:
    return TVTropesConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TVTropesConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TVTropesConstants, "AVATAR_KEYS", ()), getattr(TVTropesConstants, "COVER_KEYS", ()))
...
ROUTES = (
    (r"pmwiki/pmwiki\.php/(?P<id>Main/[^/]+)", "page"),
