...
from api.social_manager.social_recon.constants.platform_constants import VSCOConstants

constants = VSCOConstants


def probe_url(username: str) -> str:
    return VSCOConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in VSCOConstants.GENERIC:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, VSCOConstants.AVATAR_KEYS, VSCOConstants.COVER_KEYS)
    title = parse.text(parse.meta(body).get("og:title"))
    info["display_name"] = parse.clean(title.split(" (@")[0]) or info.get("display_name", "")
    handle = re.search(r"\(@([^)]+)\)", title)
    if handle:
