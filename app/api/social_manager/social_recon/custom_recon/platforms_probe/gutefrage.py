...
def probe_url(username: str) -> str:
    return GutefrageConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(GutefrageConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(GutefrageConstants, "AVATAR_KEYS", ()), getattr(GutefrageConstants, "COVER_KEYS", ()))
