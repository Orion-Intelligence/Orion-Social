...
def probe_url(username: str) -> str:
    return SporcleConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SporcleConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SporcleConstants, "AVATAR_KEYS", ()), getattr(SporcleConstants, "COVER_KEYS", ()))
