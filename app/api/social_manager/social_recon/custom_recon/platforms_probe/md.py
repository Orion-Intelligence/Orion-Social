...
def probe_url(username: str) -> str:
    return MdConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MdConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MdConstants, "AVATAR_KEYS", ()), getattr(MdConstants, "COVER_KEYS", ()))
