...
def probe_url(username: str) -> str:
    return GeocachingConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(GeocachingConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(GeocachingConstants, "AVATAR_KEYS", ()), getattr(GeocachingConstants, "COVER_KEYS", ()))
