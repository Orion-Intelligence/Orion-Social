...
def probe_url(username: str) -> str:
    return SvtperformanceComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SvtperformanceComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SvtperformanceComConstants, "AVATAR_KEYS", ()), getattr(SvtperformanceComConstants, "COVER_KEYS", ()))
