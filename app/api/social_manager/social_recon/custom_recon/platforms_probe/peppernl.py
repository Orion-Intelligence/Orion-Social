...
from api.social_manager.social_recon.constants.platform_constants import PepperNLConstants

constants = PepperNLConstants


def probe_url(username: str) -> str:
    return PepperNLConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(PepperNLConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PepperNLConstants, "AVATAR_KEYS", ()), getattr(PepperNLConstants, "COVER_KEYS", ()))
