...
from api.social_manager.social_recon.constants.platform_constants import Vch34693dnRuConstants

constants = Vch34693dnRuConstants


def probe_url(username: str) -> str:
    return Vch34693dnRuConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(Vch34693dnRuConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(Vch34693dnRuConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(Vch34693dnRuConstants, "AVATAR_KEYS", ()), getattr(Vch34693dnRuConstants, "COVER_KEYS", ()))
