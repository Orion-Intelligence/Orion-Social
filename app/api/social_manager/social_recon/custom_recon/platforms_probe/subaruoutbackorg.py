...
def probe_url(username: str) -> str:
    return SubaruoutbackOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(SubaruoutbackOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(SubaruoutbackOrgConstants, "AVATAR_KEYS", ()), getattr(SubaruoutbackOrgConstants, "COVER_KEYS", ()))
