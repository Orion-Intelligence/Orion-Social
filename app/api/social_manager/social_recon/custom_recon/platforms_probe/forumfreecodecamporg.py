...
from api.social_manager.social_recon.constants.platform_constants import ForumFreecodecampOrgConstants

constants = ForumFreecodecampOrgConstants


def probe_url(username: str) -> str:
    return ForumFreecodecampOrgConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(ForumFreecodecampOrgConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(ForumFreecodecampOrgConstants, "AVATAR_KEYS", ()), getattr(ForumFreecodecampOrgConstants, "COVER_KEYS", ()))
