...
from api.social_manager.social_recon.constants.platform_constants import NookipediaComConstants

constants = NookipediaComConstants


def probe_url(username: str) -> str:
    return NookipediaComConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(NookipediaComConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(NookipediaComConstants, "AVATAR_KEYS", ()), getattr(NookipediaComConstants, "COVER_KEYS", ()))
