...
from api.social_manager.social_recon.constants.platform_constants import MylotConstants

constants = MylotConstants


def probe_url(username: str) -> str:
    return MylotConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(MylotConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(MylotConstants, "AVATAR_KEYS", ()), getattr(MylotConstants, "COVER_KEYS", ()))
