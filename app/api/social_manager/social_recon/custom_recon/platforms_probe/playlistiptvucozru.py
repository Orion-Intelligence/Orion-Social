...
from api.social_manager.social_recon.constants.platform_constants import PlaylistIptvUcozRuConstants

constants = PlaylistIptvUcozRuConstants


def probe_url(username: str) -> str:
    return PlaylistIptvUcozRuConstants.PROFILE_URL.format(username=username)


def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    if status in (404, 410):
        return VerdictConstants.ABSENT, {}
    absence = getattr(PlaylistIptvUcozRuConstants, "ABSENCE", ())
    if absence and any(marker in body for marker in absence):
        return VerdictConstants.ABSENT, {}
    presence = getattr(PlaylistIptvUcozRuConstants, "PRESENCE", ())
    if presence:
        if not any(marker in body for marker in presence):
            return VerdictConstants.UNKNOWN, {}
    elif status != 200:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(PlaylistIptvUcozRuConstants, "AVATAR_KEYS", ()), getattr(PlaylistIptvUcozRuConstants, "COVER_KEYS", ()))
