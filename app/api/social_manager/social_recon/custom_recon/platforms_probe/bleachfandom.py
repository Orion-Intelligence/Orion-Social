...
from api.social_manager.social_recon.constants.platform_constants import BleachFandomConstants

constants = BleachFandomConstants


def probe_url(username: str) -> str:
    return BleachFandomConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(BleachFandomConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(BleachFandomConstants, "AVATAR_KEYS", ()), getattr(BleachFandomConstants, "COVER_KEYS", ()))
