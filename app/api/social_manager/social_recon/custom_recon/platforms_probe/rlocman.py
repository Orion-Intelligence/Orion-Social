...
def probe_url(username: str) -> str:
    return RlocmanConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(RlocmanConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(RlocmanConstants, "AVATAR_KEYS", ()), getattr(RlocmanConstants, "COVER_KEYS", ()))
