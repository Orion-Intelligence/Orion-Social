...
from api.social_manager.social_recon.constants.platform_constants import CnetConstants

constants = CnetConstants


def probe_url(username: str) -> str:
    return CnetConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(CnetConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(CnetConstants, "AVATAR_KEYS", ()), getattr(CnetConstants, "COVER_KEYS", ()))
    ...
    if info.get("display_name") and info["display_name"].casefold() in getattr(CnetConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
