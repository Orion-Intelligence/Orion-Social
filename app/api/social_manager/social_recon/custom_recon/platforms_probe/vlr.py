...
def probe_url(username: str) -> str:
    return VLRConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(VLRConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(VLRConstants, "AVATAR_KEYS", ()), getattr(VLRConstants, "COVER_KEYS", ()))
