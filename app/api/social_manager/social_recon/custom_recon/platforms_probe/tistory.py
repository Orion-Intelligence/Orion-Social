...
from api.social_manager.social_recon.constants.platform_constants import TistoryConstants

constants = TistoryConstants


def probe_url(username: str) -> str:
    return TistoryConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TistoryConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TistoryConstants, "AVATAR_KEYS", ()), getattr(TistoryConstants, "COVER_KEYS", ()))
