...
def probe_url(username: str) -> str:
    return TruthbookConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in getattr(TruthbookConstants, "GENERIC", set()):
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, getattr(TruthbookConstants, "AVATAR_KEYS", ()), getattr(TruthbookConstants, "COVER_KEYS", ()))
...
ROUTES = (
    (r"viewforum\.php\?(?:.*&)?f=(?P<id>\d+)(?:&.*)?", "group"),
