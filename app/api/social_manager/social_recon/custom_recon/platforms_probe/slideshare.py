...
from api.social_manager.social_recon.constants.platform_constants import SlideShareConstants

constants = SlideShareConstants


def probe_url(username: str) -> str:
    return SlideShareConstants.PROFILE_URL.format(username=username)
...
def evaluate(status: int, body: str, _final_url: str) -> tuple[str, dict]:
    ...
    if not heading or heading.casefold() in SlideShareConstants.GENERIC:
        return VerdictConstants.UNKNOWN, {}
    info = parse.social_info(body, SlideShareConstants.AVATAR_KEYS, SlideShareConstants.COVER_KEYS)
    parts = parse.clean(heading).split(", ", 1)
    info["display_name"] = parts[0]
    if len(parts) > 1 and not info.get("description"):
